...
import { createBugbeeClient } from "@bugbee-ai/sdk"
...
const { client, server } = createBugbee()
...
try {
  ...
  console.log("bugbee session", session.id)
  if (shareId) {
...
function createBugbee() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`bugbee`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createBugbeeClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/bugbee|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/bugbee` or `/oc`")
  }
}
...
async function assertBugbeeConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to bugbee server")
  }
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.bugbee.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("bugbee-github-action")
    response = await fetch("https://api.bugbee.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/bugbee" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/bugbee") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to bugbee...")
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `bugbee/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/bugbee-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[bugbee session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
