...
Move the canonical Bugbee terminal application from
`packages/bugbee/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/bugbee`,
`packages/cli`, or `@bugbee-ai/core`.

The SDK is the TUI's Bugbee boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/bugbee`.
- Do not replace private `packages/bugbee` imports with `@bugbee-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@bugbee-ai/tui`.
...
- Bugbee domain data displayed by the TUI
...
```text
packages/bugbee/src/cli/cmd/tui
```

Its private dependency on `packages/bugbee` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/bugbee/src/*`.
...
Status: Completed. The private `@bugbee-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@bugbee-ai/tui`.
...
- The package has no dependency on `bugbee`, `@bugbee-ai/cli`, or
  `@bugbee-ai/core`.
...
primitives now live in `@bugbee-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@bugbee-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Bugbee globals or process state;
...
Replace process-global Bugbee state with resolved TUI inputs.
...
`@bugbee-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/bugbee` initially.
...
canonical in `@bugbee-ai/tui`. Configured references resolve through the new
...
adapters rather than Bugbee domain implementations.

Make the SDK the only Bugbee domain boundary used by the TUI.
...
- Domain-facing TUI code imports Bugbee data and operations only from
  `@bugbee-ai/sdk`.
...
`@bugbee-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/bugbee`.
...
- The package has no imports from `packages/bugbee`, `packages/cli`, or
  `@bugbee-ai/core`.
...
public `@bugbee-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/bugbee`.
- Keep the legacy embedded worker and server startup in `packages/bugbee`.
- Change those adapters to load config, create transport inputs, and call the
  public `@bugbee-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/bugbee`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@bugbee-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@bugbee-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Bugbee domain state.
...
cd packages/bugbee && bun typecheck
...
rg '@bugbee-ai/core|packages/bugbee|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/bugbee/src packages/bugbee/test
```
...
- Build the current-platform `packages/bugbee` binary.
