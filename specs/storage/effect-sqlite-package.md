...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an bugbee storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/bugbee` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No bugbee paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@bugbee-ai/effect-drizzle-sqlite`
...
- Bugbee-specific path/channel selection stays in `packages/bugbee`.

## Bugbee Adoption Notes

These are not package requirements, but they matter for the later bugbee adoption PR.

The current `packages/bugbee/src/storage/db.ts` has two non-obvious semantics that the bugbee wrapper must preserve when it consumes this adapter:
...
The bugbee wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving bugbee to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@bugbee-ai/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@bugbee-ai/effect-drizzle-sqlite` as a dependency of `packages/bugbee`.
5. Port `packages/bugbee/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus bugbee-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of bugbee's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then bugbee can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay bugbee-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/bugbee`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not bugbee domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/bugbee` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing bugbee's current database runtime.
