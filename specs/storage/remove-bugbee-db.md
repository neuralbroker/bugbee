# Remove `packages/bugbee/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/bugbee/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the bugbee legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/bugbee/src/storage/db.ts` are concentrated in 22 source files:

- `packages/bugbee/src/account/repo.ts`
- `packages/bugbee/src/cli/cmd/db.ts`
- `packages/bugbee/src/cli/cmd/import.ts`
- `packages/bugbee/src/cli/cmd/stats.ts`
- `packages/bugbee/src/control-plane/workspace.ts`
- `packages/bugbee/src/index.ts`
- `packages/bugbee/src/node.ts`
- `packages/bugbee/src/permission/index.ts`
- `packages/bugbee/src/project/project.ts`
- `packages/bugbee/src/server/projectors.ts`
- `packages/bugbee/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/bugbee/src/server/shared/fence.ts`
- `packages/bugbee/src/session/message-v2.ts`
- `packages/bugbee/src/session/projectors.ts`
- `packages/bugbee/src/session/prompt.ts`
- `packages/bugbee/src/session/session.ts`
- `packages/bugbee/src/session/todo.ts`
- `packages/bugbee/src/share/share-next.ts`
- `packages/bugbee/src/storage/db.ts`
- `packages/bugbee/src/sync/index.ts`
- `packages/bugbee/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy bugbee database wrapper; `packages/bugbee/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the bugbee wrapper.
...
Status: Completed. `SyncEvent` and the bugbee projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy bugbee database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy bugbee database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy bugbee database wrapper.
...
All migration groups are complete or superseded. `packages/bugbee/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No bugbee data-migration group remains.

The previous bugbee `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate bugbee data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/bugbee`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/bugbee/src`
- `bun typecheck` from `packages/bugbee`
- Relevant package tests from `packages/bugbee`, not the repo root
