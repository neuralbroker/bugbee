...
V2 core discovers config documents named `bugbee.json` or `bugbee.jsonc` in the global config directory, ancestor project directories, and `.bugbee` config directories. The legacy `config.json` filename is not supported in V2.
...
| Field        | Current Purpose                                     | Status | Notes                                                                          |
| ------------ | --------------------------------------------------- | ------ | ------------------------------------------------------------------------------ |
| `shell`      | Default shell for terminal and shell tool execution | keep   | Port as effective config; shared shell choice is used throughout bugbee.     |
...
The configured `plugins` list represents package-loaded plugins only. Local plugin code remains discovered from plugin directories such as `.bugbee/plugins/`; v2 does not port arbitrary configured local paths or file URLs into this field.
...
Policy evaluation will consume authored config documents in reverse order while preserving statement order inside each document. The precedence of `.bugbee` policy sources remains open until `.bugbee` configuration is reviewed.
...
| Field | Current Purpose                       | Status   | Notes                                                                                                                                                      |
| ----- | ------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mcp` | MCP server definitions and enablement | redesign | Keep bugbee's explicit local/remote server entry format, nested under `mcp.servers`; use `disabled` for inactive entries and move timeout defaults here. |

Keep the bugbee MCP server entry format instead of adopting the common `mcpServers` copy/paste shape. Local servers remain explicit `type: "local"` entries with command arrays and `environment`; remote servers remain explicit `type: "remote"` entries with `url`, `headers`, and optional `oauth`. Nest the server map under `mcp.servers` so protocol-wide settings such as timeout defaults can live under the same subsystem.
