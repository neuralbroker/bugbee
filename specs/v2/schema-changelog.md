...
This document covers meaningful contract changes introduced on the `feat/bugbee-embedded-api` branch since its divergence from `origin/dev`. Mechanical file moves and internal refactors are omitted unless they changed stored data, replay behavior, public HTTP or SDK shapes, or model-facing tool contracts.
...
- Canonical tagged tool file sources in `@bugbee-ai/llm`.
...
- Spill oversized model-facing tool text into globally unique files under Bugbee's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `BUGBEE_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
