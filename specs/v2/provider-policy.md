...
Both `action` and `resource` use bugbee's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.bugbee` files is intentionally deferred until `.bugbee` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether bugbee permits an operation through its managed execution paths.
