...
The bugbee server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@bugbee-ai/llm` tool loop after replacing its
...
- plugins get global "bugbee" instance like in that post i showed
- bugbee instance has stuff like `bugbee.session.prompt()` or
  `bugbee.tool.register({...})`
...
- decide whether to preserve deprecated `@bugbee-ai/llm` orchestration exports
