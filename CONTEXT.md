# Bugbee Session Runtime

Bugbee sessions preserve durable conversational history while assembling the runtime context an agent needs to act correctly in its current environment.
...
A temporary file created under Bugbee's shared tool-output directory to retain complete output that was too large for Session history.
...
**Bugbee Client**:
The generated Promise and Effect APIs derived from the public `HttpApi`; **Embedded Bugbee** shares the Effect API through an in-memory `HttpClient` against the same router and handlers.
...
**Embedded Bugbee**:
A scoped in-process host that structurally extends the **Bugbee Client**, supplies an in-memory HTTP transport, and exposes additional same-process capabilities directly.
...
- The **PTY Environment** is a server concern rather than a Core PTY concern. PTY creation merges caller values, then the host overlay, then Core-forced terminal invariants such as `TERM` and `BUGBEE_TERMINAL`.
- Networked and **Embedded Bugbee** use the same **Bugbee Client** and preserve the full HTTP encoding, routing, middleware, and decoding boundary; only the `HttpClient` transport differs.
...
- Creating **Embedded Bugbee** is scoped. Closing its owning Scope releases the in-process server resources, database resources, registrations, and fibers.
- **Embedded Bugbee** exposes shared client capabilities and embedded-only capabilities on one object; consumers do not navigate through a nested `.client` property.
- The beta **Bugbee Client** currently uses plural consumer-facing capability groups such as `sessions`; whether the stable Session namespace should instead be singular `session` must be settled before stabilization. Internal server identifiers do not implicitly define public client names.
- Server's concrete `HttpApi` is authoritative for shared **Bugbee Client** capabilities. Codegen compiles its Session group directly; the Effect runtime uses an equivalent Protocol-only projection so generated artifacts remain independent of Core and Server.
...
- `@bugbee-ai/protocol` owns Session endpoint construction and middleware placement. Server supplies concrete middleware keys to produce the authoritative build-time API; the client projection supplies transport-only keys without importing Core or Server at runtime.
...
- Promise and Effect network clients ship from `@bugbee-ai/client` behind isolated root and `/effect` exports. The root has no runtime path to Effect; `/effect` imports only Effect, Schema, and Protocol.
- The Effect-native scoped host belongs to `@bugbee-ai/sdk-next`, which will assume the existing `@bugbee-ai/sdk` name after legacy consumers migrate. Client remains network-only and SDK depends one-way on Client.
...
- A capability intended for both networked and **Embedded Bugbee** belongs in the authoritative public `HttpApi`; embedded-only same-process capabilities extend **Embedded Bugbee** separately.
...
- The initial common Bugbee Client does not expose server-global event aggregation. `events.subscribe()` is bounded to the connected Bugbee instance or workspace; any future cross-instance administrative stream requires a separately designed API.
...
- The stable `sessions.list(...)` design returns a **Page** in both networked and **Embedded Bugbee**; embedded execution does not define a separate unbounded array-returning list operation. The beta client currently preserves the existing HTTP `{ data, cursor }` envelope until emitter-level Page projection is implemented.
...
- `sessions.create(...)` accepts an optional `location`. Omission resolves through the connected Bugbee instance's default or current location; an explicit value selects a known location. Networked and embedded transports use the same handler semantics.
...
- The **Embedded Bugbee** Layer delegates to the same scoped creation path; it does not define a second implementation.
...
- Ambient project instruction discovery honors `BUGBEE_DISABLE_PROJECT_CONFIG`; global instructions remain eligible.
...
- Preserve browser-safe `@bugbee-ai/client` and `@bugbee-ai/client/effect` bundles through import-boundary tests.
