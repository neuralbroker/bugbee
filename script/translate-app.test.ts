...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".bugbee/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".bugbee/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".bugbee/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "bugbee/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "bugbee/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "bugbee/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
