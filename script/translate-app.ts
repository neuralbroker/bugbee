...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "bugbee/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".bugbee/glossary/zh-cn.md"
  if (locale === "zht") return ".bugbee/glossary/zh-tw.md"
  return `.bugbee/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Bugbee did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Bugbee returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Bugbee session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Bugbee runs for 'all' (default: 4)
      --model <provider/id>  Bugbee model (default: bugbee/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Bugbee
...
`)
  ...
  if (failed.length) console.error(`\nBugbee failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.BUGBEE_DISABLE_PROJECT_CONFIG = "1"
  env.BUGBEE_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["bugbee", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.BUGBEE_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["bugbee", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.BUGBEE_CONFIG
  delete env.BUGBEE_CONFIG_DIR
  delete env.BUGBEE_CONFIG_CONTENT
  delete env.BUGBEE_PERMISSION
  delete env.BUGBEE_AUTO_SHARE
  return env
}
