...
const model = "bugbee/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/bugbee")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with bugbee...`)
  ...
  try {
    await $`bugbee run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  bugbee failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with bugbee")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with bugbee...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/bugbee`.",
