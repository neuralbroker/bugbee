...
import { createBugbee } from "@bugbee-ai/sdk"
...
`)
  ...
  const bugbee = await createBugbee({ port: 0 })
  ...
  try {
    ...
    const session = await bugbee.client.session.create()
    const result = await bugbee.client.session
      .prompt({
