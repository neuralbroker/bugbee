...
import { and, Database, eq, isNull, lt, or, sql } from "@bugbee-ai/console-core/drizzle/index.js"
import { KeyTable } from "@bugbee-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@bugbee-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@bugbee-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@bugbee-ai/console-core/util/date.js"
import { Identifier } from "@bugbee-ai/console-core/identifier.js"
import { Billing } from "@bugbee-ai/console-core/billing.js"
import { Actor } from "@bugbee-ai/console-core/actor.js"
import { WorkspaceTable } from "@bugbee-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@bugbee-ai/console-core/model.js"
import { Subscription } from "@bugbee-ai/console-core/subscription.js"
import { BlackData } from "@bugbee-ai/console-core/black.js"
import { UserTable } from "@bugbee-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@bugbee-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@bugbee-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@bugbee-ai/console-core/lite.js"
import { Resource } from "@bugbee-ai/console-resource"
...
import { Workspace } from "@bugbee-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-bugbee-session") ?? ""
    const requestId = input.request.headers.get("x-bugbee-request") ?? ""
    const ocClient = input.request.headers.get("x-bugbee-client") ?? ""
    const projectId = input.request.headers.get("x-bugbee-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://bugbee.dev/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-bugbee-request")
            headers.delete("x-bugbee-session")
            headers.delete("x-bugbee-project")
            headers.delete("x-bugbee-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-bugbee-endpoint-id")
        const resEndpointModelId = res.headers.get("x-bugbee-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://bugbee.dev/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://bugbee.dev/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://bugbee.dev/workspace/${authInfo.workspaceID}/members`
