...
import { createBugbeeClient } from "@bugbee-ai/sdk/v2/client"
import { ServerAuth } from "@bugbee-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createBugbeeClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@bugbee/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createBugbeeClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createBugbeeClient({ baseUrl: connection.url, headers: connection.headers })
    })
