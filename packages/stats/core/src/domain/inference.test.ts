...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve bugbee route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "bugbee")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "bugbee")).toBe("openai")
    expect(statProvider("big-pickle", "", "bugbee")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "bugbee"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep bugbee as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "bugbee"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "bugbee"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep bugbee or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "bugbee"), country: "US" })).toMatchObject([
