...
This is the living design and status document for `@bugbee-ai/codemode` and its existing V2 Bugbee adapter.
...
`packages/bugbee/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
