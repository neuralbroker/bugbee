...
```ts
import { CodeMode, Tool } from "@bugbee-ai/codemode"
...
```ts
import { CodeMode, OpenAPI } from "@bugbee-ai/codemode"
...
const runtime = CodeMode.make({ tools: { bugbee: api.tools } })
...
The catalog-entry budget defaults to 2,000 estimated tokens (characters / 4, the same heuristic Bugbee uses). It applies only to full tool entries shown in the catalog; fixed instructions and namespace summaries are not counted. Override it when constructing a runtime:
...
No limit has a default, on purpose: execution budgets are host policy, not library policy - a host that wants a bound sets one; a host that can interrupt the execution fiber (as Bugbee does on user cancel) may set no timeout, and a host with its own tool-output truncation (as Bugbee has) may leave `maxOutputBytes` unset. A host with neither should set `maxOutputBytes`, or oversized results silently flood model context.
...
```ts
import { toolError } from "@bugbee-ai/codemode"
