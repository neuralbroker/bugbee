...
describe("inlineCodeKind", () => {
  test("leaves code expressions as normal inline code", () => {
    ...
    expect(inlineCodeKind(`@bugbee-ai/app <StatusPopover />)`)).toBeUndefined()
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/bugbee`)).toBe("path")
    expect(inlineCodeKind(`@bugbee-ai/app`)).toBe("path")
