...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("bugbee-find")
  api.delete("bugbee-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("bugbee-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("bugbee-find", new Highlight(...rest))
    return true
  }
