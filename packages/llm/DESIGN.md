...
> current private `@bugbee-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@bugbee-ai/ai`
...
- Secondary audience: Bugbee and other durable agent runtimes
...
import { LLM } from "@bugbee-ai/ai"
import { OpenAI } from "@bugbee-ai/ai/providers/openai"
...
```ts
import { Message } from "@bugbee-ai/ai"
...
import { LLM, Tool } from "@bugbee-ai/ai"
...
Bugbee and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@bugbee-ai/ai/promise"
import { OpenAI } from "@bugbee-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@bugbee-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@bugbee-ai/ai/provider"
...
@bugbee-ai/ai/promise
  Promise/AsyncIterable LLM facade

@bugbee-ai/ai/schema
  serializable domain schemas

@bugbee-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@bugbee-ai/ai/providers/openai
@bugbee-ai/ai/providers/anthropic
@bugbee-ai/ai/providers/google
...
```
...
Bugbee should migrate to `generateTurn` / `streamTurn`, preserving its durable
