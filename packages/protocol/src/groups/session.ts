...
import { PromptInput } from "@bugbee-ai/schema/prompt-input"
import { Session } from "@bugbee-ai/schema/session"
import { Project } from "@bugbee-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@bugbee-ai/schema/schema"
...
import { SessionEvent } from "@bugbee-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Bugbee process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Bugbee process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
