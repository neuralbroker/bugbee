...
With Bugbee you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Bugbee uses our own `bugbee` theme.
...
Bugbee comes with several built-in themes.
...
- Want Bugbee to match their terminal's appearance
...
Bugbee supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/bugbee/themes/*.json` or `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.bugbee/themes/*.json`
4. **Current working directory** - Defined in `./.bugbee/themes/*.json`
