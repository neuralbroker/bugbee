---
title: ผู้ให้บริการ
description: ใช้ผู้ให้บริการ LLM ใน Bugbee
---

import config from "../../../../config.mjs"
export const console = config.console

Bugbee ใช้ [AI SDK](https://ai-sdk.dev/) และ [Models.dev](https://models.dev) เพื่อรองรับผู้ให้บริการ **75+ LLM** และรองรับการใช้งานโมเดลท้องถิ่น

หากต้องการเพิ่มผู้ให้บริการ คุณต้อง:

1. เพิ่มคีย์ API สำหรับผู้ให้บริการโดยใช้คำสั่ง `/connect`
2. กำหนดค่าผู้ให้บริการในการกำหนดค่า Bugbee ของคุณ
...
ใน `~/.local/share/bugbee/auth.json`.

---

### การกำหนดค่า

คุณสามารถปรับแต่งผู้ให้บริการได้ผ่านทางส่วน `provider` ใน Bugbee ของคุณ
กำหนดค่า
...
Bugbee Zen คือรายชื่อโมเดลที่จัดทำโดยทีมงาน Bugbee ที่ได้รับ
ทดสอบและตรวจสอบแล้วว่าทำงานได้ดีกับ Bugbee [เรียนรู้เพิ่มเติม](/docs/zen)

:::tip
หากคุณยังใหม่ เราขอแนะนำให้เริ่มต้นด้วย Bugbee Zen
:::

1. เรียกใช้คำสั่ง `/connect` ใน TUI เลือก bugbee และไปที่ [bugbee.dev/auth](https://bugbee.dev/auth)
...
มันทำงานเหมือนกับผู้ให้บริการรายอื่นใน Bugbee และเป็นทางเลือกในการใช้งานโดยสมบูรณ์

---

## Bugbee Go

Bugbee Go คือแผนการสมัครสมาชิกราคาประหยัดที่ให้การเข้าถึงโมเดลการเขียนโค้ดแบบเปิดยอดนิยมที่เชื่อถือได้ ซึ่งจัดเตรียมโดยทีมงาน Bugbee ที่ได้รับการทดสอบและตรวจสอบแล้วว่าทำงานได้ดีกับ Bugbee

1. เรียกใช้คำสั่ง `/connect` ใน TUI เลือก `Bugbee Go` และไปที่ [bugbee.dev/auth](https://bugbee.dev/zen)
...
วิธีใช้ Amazon Bedrock กับ Bugbee:
...
2. **กำหนดค่าการตรวจสอบสิทธิ์** โดยใช้วิธีใดวิธีหนึ่งต่อไปนี้:
   ...
   ตั้งค่าหนึ่งในตัวแปรสภาพแวดล้อมเหล่านี้ขณะเรียกใช้ bugbee:
   ...
   สำหรับการกำหนดค่าเฉพาะโครงการหรือแบบถาวร ให้ใช้ `bugbee.json`:
...
:::info
การใช้การสมัครสมาชิก Claude Pro/Max ใน Bugbee ไม่ได้รับการสนับสนุนอย่างเป็นทางการจาก [Anthropic](https://anthropic.com)
:::
...
คุณสามารถกำหนดค่า bugbee ให้ใช้โมเดลท้องถิ่นผ่าน [Atomic Chat](https://atomic.chat) ซึ่งเป็นแอปพลิเคชันเดสก์ท็อปที่เรียกใช้ LLM ในเครื่องภายใต้เซิร์ฟเวอร์ API ที่เข้ากันได้กับ OpenAI (ปลายทางเริ่มต้น `http://127.0.0.1:1337/v1`)
...
2. ไปที่ [Azure AI Foundry](https://ai.azure.com/) และปรับใช้โมเดล
   ...
   :::note
   ชื่อการปรับใช้จะต้องตรงกับชื่อรุ่นเพื่อให้ bugbee ทำงานได้อย่างถูกต้อง
   :::
...
5. รันคำสั่ง `/models` เพื่อเลือกรุ่น
   ...
   คุณยังสามารถเพิ่มโมเดลผ่านการกำหนดค่า bugbee ของคุณได้
...
เพื่อจัดเก็บโทเค็นในที่เก็บข้อมูลการรับรองความถูกต้องของ bugbee
:::

##### GitLab ที่โฮสต์เอง

:::note[บันทึกการปฏิบัติตาม]
Bugbee ใช้โมเดลขนาดเล็กสำหรับงาน AI บางอย่าง เช่น การสร้างชื่อเซสชัน
มีการกำหนดค่าให้ใช้ gpt-5-nano ตามค่าเริ่มต้น ซึ่งโฮสต์โดย Zen เพื่อล็อค Bugbee
หากต้องการใช้เฉพาะอินสแตนซ์ที่โฮสต์บน GitLab ของคุณเอง ให้เพิ่มสิ่งต่อไปนี้ในของคุณ
`bugbee.json` ไฟล์. ขอแนะนำให้ปิดใช้งานการแชร์เซสชันด้วย
...
เอกสารประกอบเพิ่มเติมเกี่ยวกับหน้าแรกของ [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth)

##### การกำหนดค่า

ปรับแต่งผ่าน `bugbee.json`:
...
วิธีใช้การสมัครสมาชิก GitHub Copilot ด้วย bugbee:
...
วิธีใช้ Google Vertex AI กับ Bugbee:
...
2. ตั้งค่าตัวแปรสภาพแวดล้อมที่ต้องการ:
   ...
   ตั้งค่าในขณะที่เรียกใช้ bugbee
...
ในกรณีที่คุณเห็นคุณสมบัติหรือรุ่นจาก Helicone ที่ไม่ได้กำหนดค่าโดยอัตโนมัติผ่าน bugbee คุณสามารถกำหนดค่าด้วยตนเองได้ตลอดเวลา
...
คุณลักษณะ [เซสชัน](https://docs.helicone.ai/features/sessions) ของ Helicone ช่วยให้คุณสามารถจัดกลุ่มคำขอ LLM ที่เกี่ยวข้องเข้าด้วยกันได้ ใช้ปลั๊กอิน [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) เพื่อบันทึกการสนทนา Bugbee แต่ละรายการเป็นเซสชันใน Helicone โดยอัตโนมัติ
...
ปลั๊กอินจะแทรกส่วนหัว `Helicone-Session-Id` และ `Helicone-Session-Name` ลงในคำขอของคุณ ในหน้าเซสชันของ Helicone คุณจะเห็นการสนทนา Bugbee แต่ละรายการแยกเป็นเซสชันแยกกัน
...
คุณสามารถกำหนดค่า bugbee เพื่อใช้โมเดลท้องถิ่นผ่านยูทิลิตี้ [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server
...
คุณสามารถกำหนดค่า bugbee เพื่อใช้โมเดลท้องถิ่นผ่าน LM Studio
...
คุณสามารถกำหนดค่า bugbee เพื่อใช้โมเดลท้องถิ่นผ่าน Ollama

:::tip
Ollama สามารถกำหนดค่าตัวเองสำหรับ Bugbee ได้โดยอัตโนมัติ ดู [เอกสารการบูรณาการ Ollama](https://docs.ollama.com/integrations/bugbee) เพื่อดูรายละเอียด
:::
...
วิธีใช้ Ollama Cloud กับ Bugbee:
...
3. คัดลอกคีย์ API เพื่อใช้ใน Bugbee
...
6. **สำคัญ**: ก่อนที่จะใช้โมเดลคลาวด์ใน Bugbee คุณต้องดึงข้อมูลโมเดลในเครื่อง:
...
Bugbee Zen คือรายการโมเดลที่ได้รับการทดสอบและตรวจสอบแล้วโดยทีมงาน Bugbee [เรียนรู้เพิ่มเติม](/docs/zen)

1. ลงชื่อเข้าใช้ **<a href={console}>Bugbee Zen</a>** และคลิก **สร้าง API คีย์**

2. เรียกใช้คำสั่ง `/connect` และค้นหา **Bugbee Zen**

   ```txt
   /connect
   ```

3. ป้อนคีย์ Bugbee API ของคุณ
...
4. โมเดล OpenRouter จำนวนมากถูกโหลดไว้ล่วงหน้าตามค่าเริ่มต้น รันคำสั่ง `/models` เพื่อเลือกโมเดลที่คุณต้องการ
   ...
   คุณยังสามารถเพิ่มโมเดลเพิ่มเติมผ่านการกำหนดค่า bugbee ของคุณได้
...
5. คุณยังสามารถปรับแต่งได้ผ่านการกำหนดค่า bugbee ของคุณ นี่คือตัวอย่างการระบุผู้ให้บริการ
...
วิธีใช้ [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) กับ Bugbee:
...
คุณยังสามารถปรับแต่งโมเดลผ่านการกำหนดค่า bugbee ของคุณได้ นี่คือตัวอย่างการระบุลำดับการกำหนดเส้นทางผู้ให้บริการ
...
:::tip
คุณสามารถใช้ผู้ให้บริการที่เข้ากันได้กับ OpenAI กับ bugbee ได้ ผู้ให้บริการ AI สมัยใหม่ส่วนใหญ่เสนอ API ที่เข้ากันได้กับ OpenAI
:::
...
4. สร้างหรืออัปเดตไฟล์ `bugbee.json` ของคุณในไดเร็กทอรีโครงการของคุณ:
...
ฟิลด์ `limit` ช่วยให้ Bugbee เข้าใจว่าคุณมีบริบทเหลืออยู่เท่าใด ผู้ให้บริการมาตรฐานจะดึงข้อมูลเหล่านี้จาก models.dev โดยอัตโนมัติ
...
1. **ตรวจสอบการตั้งค่าการรับรองความถูกต้อง**: รัน `bugbee auth list` เพื่อดูว่ามีการเพิ่มข้อมูลรับรอง
...
2. สำหรับผู้ให้บริการที่กำหนดเอง ให้ตรวจสอบการกำหนดค่า bugbee และ:
   - ตรวจสอบให้แน่ใจว่า ID ผู้ให้บริการที่ใช้ในคำสั่ง `/connect` ตรงกับ ID ในการกำหนดค่า bugbee ของคุณ
...
4. OpenRouter หลายรุ่นถูกโหลดไว้ล่วงหน้าตามค่าเริ่มต้น ให้รันคำสั่ง `/models` เพื่อเลือกรุ่นที่คุณต้องการ
   ...
   คุณยังสามารถเพิ่มโมเดลเพิ่มเติมผ่านการกำหนดค่า bugbee ของคุณ
...
วิธีใช้ [Scaleway Generative API](https://www.scaleway.com/en/docs/generative-apis/) กับ Bugbee:
...
คุณยังสามารถปรับแต่งโมเดลผ่านการกำหนดค่า bugbee ของคุณได้อีกด้วย นี่คือตัวอย่างการระบุลำดับการกำหนดเส้นทางของผู้ให้บริการ
...
:::tip
คุณสามารถใช้ผู้ให้บริการที่เข้ากันได้กับ OpenAI ด้วย bugbee ผู้ให้บริการ AI สมัยใหม่ส่วนใหญ่เสนอ API ที่เข้ากันได้กับ OpenAI
:::
...
4. สร้างหรืออัปเดตไฟล์ `bugbee.json` ในไดเรกทอรีโครงการของคุณ:
...
ช่อง `limit` ช่วยให้ Bugbee เข้าใจบริบทที่คุณเหลืออยู่ ผู้ให้บริการมาตรฐานจะดึงข้อมูลเหล่านี้จาก models.dev โดยอัตโนมัติ
...
1. **ตรวจสอบการตั้งค่าการรับรองความถูกต้อง**: เรียกใช้ `bugbee auth list` เพื่อดูว่ามีข้อมูลรับรองหรือไม่
...
2. สำหรับผู้ให้บริการแบบกำหนดเอง ให้ตรวจสอบการกำหนดค่า bugbee และ:
   - ตรวจสอบให้แน่ใจว่า ID ผู้ให้บริการที่ใช้ในคำสั่ง `/connect` ตรงกับ ID ในการกำหนดค่า bugbee ของคุณ
