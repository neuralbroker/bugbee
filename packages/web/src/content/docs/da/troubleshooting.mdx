...
For at fejlfinde problemer med Bugbee, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\bugbee\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `bugbee --log-level DEBUG`.

---

## Lagring

bugbee gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/bugbee/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\bugbee`
...
Bugbee Desktop kører en lokal Bugbee-server (`bugbee-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Bugbee`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/bugbee/bugbee.jsonc` (eller `~/.config/bugbee/bugbee.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/bugbee/bugbee.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\bugbee\bugbee.jsonc`
...
Bugbee kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/bugbee/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\bugbee\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Bugbee kan genopbygge den.

1. Afslut Bugbee Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/bugbee`
- **Linux**: slet `~/.cache/bugbee` (eller kør `rm -rf ~/.cache/bugbee`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\bugbee`

3. Start Bugbee Desktop på ny.

---

### Løs problemer med serverforbindelse

Bugbee Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `bugbee.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `BUGBEE_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `BUGBEE_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Bugbee Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Bugbee.

---

### Meddelelser vises ikke

Bugbee Desktop viser kun systemvarsler når:

- varsler er aktiveret for Bugbee i dine OS-indstillinger, og
...
1. Afslut Bugbee Desktop.
2. Find og slet disse filer (de findes i Bugbee Desktop-appens datamappe):

- `bugbee.settings.dat` (skrivebordsstandardserver URL)
- `bugbee.global.dat` og `bugbee.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Bugbee:
...
For at finde ud af hvilke modeller du har adgang til, kør `bugbee models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\bugbee`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. bugbee installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\bugbee`

2. Start bugbee på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge bugbee til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
bugbee vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
