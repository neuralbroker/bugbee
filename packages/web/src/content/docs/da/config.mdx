---
title: Konfiguration
description: Ved at bruge Bugbee JSON konfig.
---

Du kan konfigurere Bugbee ved hjælp af en JSON-konfigurationsfil.

---

## Format

Bugbee understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/bugbee`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/bugbee/bugbee.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`BUGBEE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`bugbee.json` i projekt) - projektspecifikke indstillinger
5. **`.bugbee` mapper** - agent, kommandoer, plugins
6. **Inline config** (`BUGBEE_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.bugbee` og `~/.config/bugbee` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/bugbee`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Bugbee-konfiguration i `~/.config/bugbee/bugbee.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `bugbee.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Bugbee starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `BUGBEE_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `BUGBEE_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.bugbee` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.bugbee` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `bugbee serve` og `bugbee web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Bugbee-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `bugbee.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Bugbee-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Bugbee at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Bugbee-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/bugbee/agents/` eller `.bugbee/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Bugbee falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`bugbee run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/bugbee/commands/` eller `.bugbee/commands/`. [Learn more here](/docs/commands).
...
Bugbee vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard bugbee **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Bugbee med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.bugbee/plugins/` eller `~/.config/bugbee/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Bugbee til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
