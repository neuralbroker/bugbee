---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

Bugbee bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Bugbee-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/bugbee/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Bugbee-konfigurationen.
...
Bugbee Zen er en liste over modeller leveret af Bugbee-teamet, som er blevet testet og verificeret til at fungere godt med Bugbee. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Bugbee Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg bugbee og gå til [bugbee.dev/auth](https://bugbee.dev/auth).
...
Det fungerer som alle andre udbydere i Bugbee og er helt valgfrit at bruge.

---

## Bugbee Go

Bugbee Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Bugbee-teamet, som er testet og verificeret til at fungere godt med Bugbee.

1. Kør kommandoen `/connect` i TUI, vælg `Bugbee Go`, og gå til [bugbee.dev/auth](https://bugbee.dev/zen).
...
Sådan bruger du Amazon Bedrock med Bugbee:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører bugbee:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `bugbee.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Bugbee understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere bugbee til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at bugbee skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem bugbee-konfigurationen.
...
gemme tokenet i bugbee auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Bugbee bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Bugbee
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`bugbee.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `bugbee.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Bugbee:
...
Sådan bruger du Google Vertex AI med Bugbee:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem bugbee, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) plugin for automatisk at logge hver Bugbee-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Bugbee-samtale opført som en separat session.
...
Du kan konfigurere bugbee til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere bugbee til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere bugbee til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Bugbee. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/bugbee) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Bugbee:
...
3. Kopier API-nøglen til brug i Bugbee.
...
6. **Vigtigt**: Før du bruger skymodeller i Bugbee, skal du hente modelinformationen lokalt:
...
Bugbee Zen er en liste over testede og verificerede modeller leveret af Bugbee-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Bugbee Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Bugbee Zen**.

   ```txt
   /connect
   ```

3. Indtast Bugbee API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem bugbee-konfigurationen.
...
5. Du kan også tilpasse dem gennem bugbee-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Bugbee:
...
Du kan også tilpasse modeller gennem bugbee-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med bugbee. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `bugbee.json`-filen i projektkataloget:
...
`limit`-felterne lader Bugbee forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `bugbee auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek bugbee-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i bugbee-konfigurationen.
