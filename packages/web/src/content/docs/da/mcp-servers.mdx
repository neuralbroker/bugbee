...
Du kan tilføje eksterne værktøjer til Bugbee ved hjælp af _Model Context Protocol_ eller MCP. Bugbee understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/bugbee`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Bugbee håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Bugbee:
...
Hvis serveren kræver godkendelse, vil Bugbee bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `bugbee mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Bugbee opbevare tokens sikkert i `~/.local/share/bugbee/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Bugbee-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Bugbee til din Sentry-konto.
