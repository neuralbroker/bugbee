...
Agentferdigheter lar Bugbee oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Bugbee søker etter disse stedene:

- Prosjektkonfigurasjon: `.bugbee/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/bugbee/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Bugbee opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.bugbee/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/bugbee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.bugbee/skills/git-release/SKILL.md` slik:
...
Bugbee viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `bugbee.json`:
...
**For innebygde agenter** (i `bugbee.json`):
