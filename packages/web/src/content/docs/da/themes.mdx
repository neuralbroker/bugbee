...
Med Bugbee kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Bugbee vårt eget `bugbee`-tema.
...
Bugbee kommer med flere innebygde temaer.
...
- Vil at Bugbee skal matche terminalens utseende
...
Bugbee støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/bugbee/themes/*.json` eller `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.bugbee/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.bugbee/themes/*.json`
