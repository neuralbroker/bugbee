---
title: Server
description: Interager med bugbee-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `bugbee serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en bugbee-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `bugbee.local` |
...
Indstil `BUGBEE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `bugbee`, eller indstil `BUGBEE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `bugbee serve` og `bugbee web`.
...
Når du kører `bugbee` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug bugbee-serveren til at interagere med bugbee programmatisk.
:::

Denne arkitektur lader bugbee understøtte flere klienter og lader dig interagere med bugbee programmatisk.

Du kan køre `bugbee serve` for at starte en selvstændig server. Hvis du har
bugbee TUI kørende, vil `bugbee serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Bugbee [IDE](/docs/ide) plugins.
...
Bugbee-serveren viser følgende API'er.
