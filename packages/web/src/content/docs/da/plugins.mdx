---
title: Plugins
description: Skriv dine egne plugins for at udvide Bugbee.
---

Plugins giver dig mulighed for at udvide Bugbee ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Bugbees standardadfærd.
...
- `.bugbee/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/bugbee/node_modules/`.
...
1. Global konfiguration (`~/.config/bugbee/bugbee.json`)
2. Projektkonfiguration (`bugbee.json`)
3. Global plugin-mappe (`~/.config/bugbee/plugins/`)
4. Projekt plugin bibliotek (`.bugbee/plugins/`)
...
Bugbee kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En bugbee SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide bugbee.
...
:::note
Hvis du bruger Bugbee desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå bugbee i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til bugbee:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som bugbee kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for bugbee sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://github.com/neuralbroker/bugbee/docs/sdk) for detaljer.
