---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Bugbee.
---

تتيح لك الإضافات توسيع Bugbee عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Bugbee.
...
- `.bugbee/plugins/` - إضافات على مستوى المشروع
- `~/.config/bugbee/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/bugbee/node_modules/`.
...
1. إعدادات عامة (`~/.config/bugbee/bugbee.json`)
2. إعدادات المشروع (`bugbee.json`)
3. دليل الإضافات العام (`~/.config/bugbee/plugins/`)
4. دليل إضافات المشروع (`.bugbee/plugins/`)
...
يشغّل Bugbee الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ bugbee للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع bugbee.
...
:::note
إذا كنت تستخدم تطبيق Bugbee لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع bugbee من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى bugbee:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ bugbee استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ bugbee إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://github.com/neuralbroker/bugbee/docs/sdk) للتفاصيل.
