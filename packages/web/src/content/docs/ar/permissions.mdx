...
يستخدم Bugbee إعداد `permission` لتحديد ما إذا كان إجراءٌ ما سيُنفّذ تلقائيًا، أو سيطلب موافقتك، أو سيتم حظره.
...
استخدم `external_directory` للسماح باستدعاءات الأدوات التي تلمس مسارات خارج دليل العمل الذي بدأ منه Bugbee. ينطبق ذلك على أي أداة تأخذ مسارًا كمدخل (مثل `read` و`edit` و`glob` و`grep` والعديد من أوامر `bash`).
...
تُعرَّف أذونات Bugbee بأسماء الأدوات، بالإضافة إلى بعض حواجز الأمان:
...
إذا لم تحدد شيئًا، يبدأ Bugbee بقيم افتراضية متساهلة:
...
عندما يطلب Bugbee الموافقة، تعرض الواجهة ثلاث نتائج ممكنة:

- `once` — الموافقة على هذا الطلب فقط
- `always` — الموافقة على الطلبات المستقبلية المطابقة للأنماط المقترحة (لباقي جلسة Bugbee الحالية)
- `reject` — رفض الطلب
