...
Bugbee Go هو اشتراك منخفض التكلفة — **$5 للشهر الأول**، ثم **$10/شهريًا** — يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في Bugbee. تشترك في Bugbee Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام Bugbee.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع Bugbee.
:::
...
يمنحك Bugbee Go الوصول إلى هذه النماذج مقابل **$5 للشهر الأول**، ثم **$10/شهريًا**.

---

## كيف يعمل

يعمل Bugbee Go مثل أي مزود آخر في Bugbee.

1. تسجّل الدخول إلى **<a href={console}>Bugbee Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `Bugbee Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في Bugbee Go.
:::
...
يتضمن Bugbee Go الحدود التالية:
...
يستخدم [model id](/docs/config/#models) في إعدادات Bugbee لديك التنسيق `bugbee-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K3، ستستخدم `bugbee-go/kimi-k3` في إعداداتك.
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع Bugbee.
