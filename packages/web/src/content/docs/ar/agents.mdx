...
يوجد نوعان من الوكلاء في Bugbee: وكلاء أساسيون ووكلاء فرعيون.
...
يأتي Bugbee مع وكيلين أساسيين مدمجين: **Build** و **Plan**. سنلقي نظرة عليهما أدناه.
...
يأتي Bugbee مع ثلاثة وكلاء فرعيين مدمجين: **General** و **Explore** و **Scout**. سنلقي نظرة على ذلك أدناه.

---

## المدمجة

يأتي Bugbee مع وكيلين أساسيين مدمجين وثلاثة وكلاء فرعيين مدمجين.
...
وكيل للقراءة فقط مخصص للوثائق الخارجية وأبحاث التبعيات. استخدمه عندما تحتاج إلى استنساخ مستودع تبعية داخل ذاكرة التخزين المؤقت المُدارة في Bugbee، أو فحص الشفرة المصدرية لمكتبة، أو إجراء مراجع متقاطعة بين الشفرة المحلية والتنفيذات upstream بدون تعديل مساحة العمل الخاصة بك.
...
هيّئ الوكلاء في ملف إعدادات `bugbee.json`:
...
- عام: `~/.config/bugbee/agents/`
- لكل مشروع: `.bugbee/agents/`
...
إذا لم يتم تحديد temperature، يستخدم Bugbee القيم الافتراضية الخاصة بالنموذج؛ عادة 0 لمعظم النماذج و 0.55 لنماذج Qwen.
...
هذا المسار نسبي لمكان وجود ملف الإعدادات. لذلك يعمل هذا لكل من إعدادات Bugbee العامة وإعدادات المشروع.
...
يستخدم معرّف النموذج في إعدادات Bugbee الصيغة `provider/model-id`. على سبيل المثال، إذا كنت تستخدم [Bugbee Zen](/docs/zen)، فستستخدم `bugbee/gpt-5.1-codex` لـ GPT 5.1 Codex.
...
:::tip
شغّل `bugbee models` لعرض قائمة بالنماذج المتاحة.
:::
...
:::tip
هل لديك وكيل تود مشاركته؟ [قدّم PR](https://github.com/neuralbroker/bugbee).
:::
