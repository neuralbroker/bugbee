...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي Bugbee مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في Bugbee.
...
:::note
هذه الأداة متاحة فقط عند ضبط `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (أو `BUGBEE_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود Bugbee أو عند ضبط متغير البيئة `BUGBEE_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل Bugbee:
