...
مع Bugbee يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Bugbee سمتنا `bugbee`.
...
- يريدون أن يطابق Bugbee مظهر terminal لديهم
...
يدعم Bugbee نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/bugbee/themes/*.json` أو `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.bugbee/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.bugbee/themes/*.json`
