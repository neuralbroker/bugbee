---
title: الإعدادات
description: استخدام ملف إعدادات Bugbee بصيغة JSON.
---

يمكنك ضبط Bugbee باستخدام ملف إعدادات بصيغة JSON.

---

## التنسيق

يدعم Bugbee تنسيقي **JSON** و**JSONC** (JSON مع تعليقات).
...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "bugbee"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/bugbee`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/bugbee/bugbee.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`BUGBEE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`bugbee.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.bugbee`** - الوكلاء، الأوامر، الإضافات
6. **إعدادات ضمنية** (`BUGBEE_CONFIG_CONTENT` env var) - تجاوزات وقت التشغيل
...
:::note
يستخدم الدليلان `.bugbee` و`~/.config/bugbee` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/bugbee`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات Bugbee العامة في `~/.config/bugbee/bugbee.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `bugbee.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.

:::tip
ضع إعدادات المشروع الخاصة في جذر مشروعك.
:::

عند تشغيل Bugbee، يبحث عن ملف إعدادات في الدليل الحالي أو يصعد حتى أقرب دليل Git.
...
حدّد مسار ملف إعدادات مخصصًا باستخدام متغير البيئة `BUGBEE_CONFIG`.
...
حدّد دليل إعدادات مخصصًا باستخدام متغير البيئة `BUGBEE_CONFIG_DIR`.
سيتم البحث داخل هذا الدليل عن الوكلاء والأوامر والأوضاع والإضافات تمامًا مثل
الدليل القياسي `.bugbee`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.bugbee`، لذلك **يمكنه تجاوز** إعداداتها.

---

## المخطط

يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `bugbee serve` و`bugbee web` عبر الخيار `server`.
...
- `mdns` - تفعيل اكتشاف الخدمة عبر mDNS. يتيح ذلك للأجهزة الأخرى على الشبكة اكتشاف خادم Bugbee.
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `bugbee.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك ضبط المزوّدين والنماذج التي تريد استخدامها في إعدادات Bugbee عبر الخيارات `provider` و`model` و`small_model`.
...
يضبط الخيار `small_model` نموذجًا منفصلًا للمهام الخفيفة مثل توليد العناوين. افتراضيًا يحاول Bugbee استخدام نموذج أقل تكلفة إذا كان متاحًا لدى مزوّدك، وإلا فسيعود إلى النموذج الرئيسي.
...
يمكنك ضبط السمة التي تريد استخدامها في إعدادات Bugbee عبر الخيار `theme`.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/bugbee/agents/` أو `.bugbee/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
يجب أن يكون الوكيل الافتراضي وكيلًا أساسيًا (وليس وكيلًا فرعيًا). يمكن أن يكون وكيلًا مدمجًا مثل `"build"` أو `"plan"`، أو [وكيلًا مخصصًا](/docs/agents) قمت بتعريفه. إذا لم يكن الوكيل المحدد موجودًا أو كان وكيلًا فرعيًا، فسيعود Bugbee إلى `"build"` مع تحذير.

ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`bugbee run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/bugbee/commands/` أو `.bugbee/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
سيقوم Bugbee بتنزيل أي تحديثات جديدة تلقائيًا عند بدء التشغيل. يمكنك تعطيل ذلك عبر الخيار `autoupdate`.
...
افتراضيًا يسمح bugbee **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
[الإضافات](/docs/plugins) توسّع Bugbee بأدوات وخُطافات وتكاملات مخصصة.

ضع ملفات الإضافات في `.bugbee/plugins/` أو `~/.config/bugbee/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
...
هذا مفيد عندما تريد تقييد Bugbee لاستخدام مزوّدين محددين بدلًا من تعطيلهم واحدًا تلو الآخر.
