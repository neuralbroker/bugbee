---
title: المزوّدون
description: استخدام أي موفّر LLM في Bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Bugbee ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Bugbee.
...
في `~/.local/share/bugbee/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Bugbee.
...
Bugbee Zen هي قائمة نماذج يوفّرها فريق Bugbee وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Bugbee. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Bugbee Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر bugbee، ثم انتقل إلى [bugbee.dev/auth](https://bugbee.dev/auth).
...
يعمل مثل أي مزوّد آخر في Bugbee واستخدامه اختياري بالكامل.

---

## Bugbee Go

Bugbee Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Bugbee، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Bugbee.

1. شغّل الأمر `/connect` في TUI، واختر `Bugbee Go`، ثم انتقل إلى [bugbee.dev/auth](https://bugbee.dev/zen).
...
لاستخدام Amazon Bedrock مع Bugbee:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل bugbee:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `bugbee.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Bugbee غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين bugbee لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل bugbee بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات bugbee.
...
تخزين الرمز في مساحة تخزين مصادقة bugbee.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Bugbee نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Bugbee
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`bugbee.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth).

##### التهيئة

خصّص عبر `bugbee.json`:
...
لاستخدام اشتراك GitHub Copilot مع bugbee:
...
لاستخدام Google Vertex AI مع Bugbee:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر bugbee، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) لتسجيل كل محادثة Bugbee تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Bugbee مدرجة كجلسة مستقلة.
...
يمكنك ضبط bugbee لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط bugbee لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط bugbee لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Bugbee. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/bugbee) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Bugbee:
...
3. انسخ مفتاح API لاستخدامه في Bugbee.
...
6. **مهم**: قبل استخدام نماذج السحابة في Bugbee، يجب سحب معلومات النموذج محليا:
...
Bugbee Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Bugbee. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Bugbee Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Bugbee Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Bugbee.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات bugbee.
...
5. يمكنك أيضا تخصيصها عبر إعدادات bugbee. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Bugbee:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات bugbee. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع bugbee. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `bugbee.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Bugbee بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `bugbee auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات bugbee وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات bugbee.
