---
title: الخادم
description: تفاعل مع خادم bugbee عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `bugbee serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل bugbee استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `bugbee.local` |
...
عيّن `BUGBEE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `bugbee`، أو عيّن `BUGBEE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `bugbee serve` و `bugbee web`.
...
عند تشغيل `bugbee` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم bugbee للتفاعل مع bugbee برمجيا.
:::

تتيح هذه البنية لـ bugbee دعم عدة عملاء وتمكّنك من التفاعل مع bugbee برمجيا.

يمكنك تشغيل `bugbee serve` لبدء خادم مستقل. إذا كانت واجهة bugbee في terminal (TUI)
قيد التشغيل، فسيبدأ `bugbee serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Bugbee لـ [IDE](/docs/ide).
...
يعرض خادم bugbee واجهات API التالية.
