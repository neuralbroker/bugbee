...
يمكنك إضافة أدوات خارجية إلى Bugbee باستخدام _Model Context Protocol_، أو MCP. يدعم Bugbee خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Bugbee](https://github.com/neuralbroker/bugbee/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/bugbee` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Bugbee تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Bugbee بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Bugbee المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `bugbee mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Bugbee الرموز بشكل آمن في `~/.local/share/bugbee/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Bugbee إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Bugbee مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Bugbee بحساب Sentry الخاص بك.
