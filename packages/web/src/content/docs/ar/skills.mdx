...
تتيح مهارات الوكيل لـ Bugbee اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Bugbee في هذه المواقع:

- إعدادات المشروع: `.bugbee/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/bugbee/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Bugbee صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.bugbee/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/bugbee/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.bugbee/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Bugbee المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `bugbee.json`:
...
**للوكلاء المدمجين** (في `bugbee.json`):
