---
title: LSP Servers
description: Bugbee integrates with your LSP servers.
---

Bugbee can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Bugbee comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `BUGBEE_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and bugbee opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your bugbee config.
