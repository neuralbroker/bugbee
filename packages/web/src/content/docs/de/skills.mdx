...
Agent Skills erlauben Bugbee, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Bugbee sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Bugbee vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.bugbee/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/bugbee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.bugbee/skills/git-release/SKILL.md` zum Beispiel so:
...
Bugbee listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `bugbee.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `bugbee.json`):
