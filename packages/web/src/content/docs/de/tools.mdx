...
Bugbee bringt eingebaute Tools mit und laesst sich ueber [Custom Tools](/docs/custom-tools) oder [MCP-Server](/docs/mcp-servers) erweitern.
...
Hier sind alle in Bugbee verfuegbaren eingebauten Tools.
...
:::note
Dieses Tool ist nur verfuegbar, wenn `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (oder `BUGBEE_EXPERIMENTAL=true`) gesetzt ist.
:::
...
:::note
Dieses Tool ist nur verfuegbar, wenn der Bugbee-Provider genutzt wird oder die Umgebungsvariable `BUGBEE_ENABLE_EXA` auf einen 'truthy' Wert (z. B. `true` oder `1`) gesetzt ist.

Zum Aktivieren beim Start von Bugbee:
