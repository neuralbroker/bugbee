...
Wenn Bugbee Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\bugbee\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `bugbee --log-level DEBUG`.

---

## Speicher

bugbee speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/bugbee/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\bugbee`
...
Bugbee Desktop startet im Hintergrund einen lokalen Bugbee-Server (`bugbee-cli`-Sidecar).
...
- Nur macOS: `Bugbee`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/bugbee/bugbee.jsonc` (oder `~/.config/bugbee/bugbee.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/bugbee/bugbee.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\bugbee\bugbee.jsonc`
...
Bugbee kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/bugbee/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\bugbee\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Bugbee ihn neu aufbauen kann.

1. Beende Bugbee Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/bugbee`
- **Linux**: loesche `~/.cache/bugbee` (oder fuehre aus: `rm -rf ~/.cache/bugbee`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\bugbee`

3. Starte Bugbee Desktop neu.

---

### Server-Verbindungsprobleme beheben

Bugbee Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `bugbee.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `BUGBEE_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `BUGBEE_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Bugbee Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Bugbee-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Bugbee Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Bugbee in den OS-Einstellungen aktiviert sind, und
...
1. Beende Bugbee Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Bugbee Desktop):

- `bugbee.settings.dat` (desktop default server URL)
- `bugbee.global.dat` und `bugbee.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Bugbee hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `bugbee models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\bugbee`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. bugbee installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\bugbee`

2. Starte bugbee neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt bugbee, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
bugbee erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
