---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Bugbee zu erweitern.
---

Mit Plugins können Sie Bugbee erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Bugbee zu ändern.
...
- `.bugbee/plugins/` – Plugins auf Projektebene
- `~/.config/bugbee/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/bugbee/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/bugbee/bugbee.json`)
2. Projektkonfiguration (`bugbee.json`)
3. Globales Plugin-Verzeichnis (`~/.config/bugbee/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.bugbee/plugins/`)
...
Bugbee führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Bugbee-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Bugbee erweitern können.
...
:::note
Wenn Sie die Bugbee-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Bugbee `.env`-Dateien liest:
...
Plugins können Bugbee auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Bugbee aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Bugbee zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://github.com/neuralbroker/bugbee/docs/sdk).
