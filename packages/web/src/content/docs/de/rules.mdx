---
title: Regeln
description: Lege eigene Anweisungen fuer bugbee fest.
---

Du kannst bugbee ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in bugbee den Befehl `/init` aus.
...
So kann bugbee sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht bugbee sie zu erweitern.
...
bugbee kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/bugbee/AGENTS.md` speichern.
Diese gelten in allen bugbee-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Bugbee dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/bugbee/AGENTS.md` existiert)
...
Beim Start sucht bugbee in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/bugbee/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `bugbee.json` oder in `~/.config/bugbee/bugbee.json` angeben.
...
bugbee parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit bugbee.json

Empfohlen ist das `instructions`-Feld in der `bugbee.json`:
...
Du kannst bugbee explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `bugbee.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
