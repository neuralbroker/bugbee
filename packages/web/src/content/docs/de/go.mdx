...
Bugbee Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Bugbee. Du abonnierst Bugbee Go und
...
Bugbee Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Bugbee Go funktioniert wie jeder andere Provider in Bugbee.

1. Du meldest dich bei **<a href={console}>Bugbee Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Bugbee Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Bugbee Go abonnieren.
:::
...
Bugbee Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Bugbee Config verwendet das Format `bugbee-go/<model-id>`. Für Kimi K3 würdest du beispielsweise `bugbee-go/kimi-k3` in deiner Config verwenden.
...
Wir haben Bugbee Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Bugbee zu nutzen.
