---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

Bugbee verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Bugbee-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Bugbee anpassen
config.
...
Bugbee Zen ist eine Liste von Modellen, die vom Bugbee-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Bugbee funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Bugbee Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Bugbee aus und gehen Sie zu [bugbee.dev/auth](https://bugbee.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in Bugbee und ist völlig optional.

---

## Bugbee Go

Bugbee Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Bugbee-Team getestet und verifiziert wurden, dass sie gut mit Bugbee funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Bugbee Go` und gehen Sie zu [bugbee.dev/auth](https://bugbee.dev/zen).
...
So verwenden Sie Amazon Bedrock mit Bugbee:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Bugbee ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `bugbee.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Bugbee wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können bugbee so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Bugbee ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Bugbee-Konfiguration hinzufügen.
...
um das Token im Bugbee-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Bugbee verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Bugbee
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`bugbee.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `bugbee.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Bugbee:
...
So verwenden Sie Google Vertex AI mit Bugbee:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Bugbee ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Bugbee konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session)-Plugin, um jede Bugbee-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Bugbee-Konversation als separate Sitzung aufgeführt.
...
Sie können Bugbee für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Bugbee über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Bugbee über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Bugbee konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/bugbee).
:::
...
So verwenden Sie Ollama Cloud mit Bugbee:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Bugbee.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Bugbee verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Bugbee Zen ist eine Liste getesteter und verifizierter Modelle, die vom Bugbee-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Bugbee Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Bugbee Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Bugbee API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Bugbee-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Bugbee-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Bugbee:
...
Sie können Modelle auch über Ihre Bugbee-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Bugbee verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `bugbee.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Bugbee erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `bugbee auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Bugbee-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Bugbee-Konfiguration übereinstimmt.
