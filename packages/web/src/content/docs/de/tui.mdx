---
title: TUI
description: Arbeite mit Bugbee in der Terminal-Oberflaeche.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Bugbee bietet eine interaktive Terminaloberflaeche (TUI), um mit einem LLM an deinen Projekten zu arbeiten.

Ohne weitere Argumente startet Bugbee die TUI im aktuellen Verzeichnis.
...
In der Bugbee-TUI kannst du `/` plus Befehlsnamen eingeben, um Aktionen schnell auszufuehren.
Zum Beispiel:
...
Fuegt einen Provider zu Bugbee hinzu. Erlaubt die Auswahl aus verfuegbaren Providern und das Hinzufuegen ihrer API-Keys.
...
Beendet Bugbee. _Aliase_: `/quit`, `/q`
...
Dies ist getrennt von `bugbee.json`, welche das Server-/Runtime-Verhalten konfiguriert.
...
- `leader_timeout` - Steuert, wie lange Bugbee nach der leader key wartet. Standard ist `2000`.
...
Verwende `BUGBEE_TUI_CONFIG`, um einen benutzerdefinierten TUI-Konfigurationspfad zu laden.

### Attention

Attention lässt das TUI dich benachrichtigen, wenn Bugbee auf eine Antwort wartet, eine Berechtigung bestätigt werden muss, ein Sitzungsfehler aufgetreten ist oder eine Sitzung abgeschlossen wurde. Aktiviere es mit `attention.enabled`; integrierte Ereignisse spielen beim Auftreten einen Sound ab. Desktop-Benachrichtigungen werden nur gesendet, wenn das Terminalfenster nicht fokussiert ist, und nicht für subagent-Ereignisse verwendet.
...
- `sound_pack` - Zu verwendende sound pack ID. Standard ist `bugbee.default`.
