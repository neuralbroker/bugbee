...
Mit Bugbee kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Bugbee das `bugbee`-Theme.
...
Bugbee liefert mehrere eingebaute Themes mit.
...
- Bugbee optisch ans Terminal anpassen wollen
...
Bugbee unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/bugbee/themes/*.json` oder `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.bugbee/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.bugbee/themes/*.json`
