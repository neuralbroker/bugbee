...
Sie können externe Tools zu Bugbee hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Bugbee unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/bugbee`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Bugbee übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Bugbee Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Bugbee beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `bugbee mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Bugbee die Token sicher in `~/.local/share/bugbee/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Bugbee verfügbar. Sie können sie also wie jedes andere Tool über die Bugbee-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Bugbee mit Ihrem Sentry-Konto zu verbinden.
