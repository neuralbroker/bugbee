---
title: Server
description: Nutze den bugbee-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `bugbee serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein bugbee-Client nutzen kann.
...
Setze `BUGBEE_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `bugbee`, kann aber mit `BUGBEE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `bugbee serve` und `bugbee web`.
...
Wenn du `bugbee` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den bugbee-Server, um bugbee programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `bugbee serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `bugbee serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Bugbee-[IDE](/docs/ide)-Plugins.
...
Der bugbee-Server stellt folgende APIs bereit.
