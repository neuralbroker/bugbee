...
Bugbee Go는 인기 있는 오픈 코딩 모델에 안정적으로 액세스할 수 있게 해주는 저비용 구독 서비스입니다. **첫 달은 $5**, 이후에는 **월 $10**입니다.

Go는 Bugbee의 다른 제공자와 똑같이 작동합니다. Bugbee Go를 구독하고 API 키를 발급받으면 됩니다. 이는 **완전히 선택 사항**이며, Bugbee를 사용하기 위해 꼭 필요하지는 않습니다.
...
:::tip
Bugbee와 잘 맞는 선별된 모델과 제공자 그룹을 테스트했습니다.
:::
...
Bugbee Go를 사용하면 **첫 달은 $5**, 이후에는 **월 $10**으로 이러한 모델에 액세스할 수 있습니다.

---

## 작동 방식

Bugbee Go는 Bugbee의 다른 제공자와 똑같이 작동합니다.

1. **<a href={console}>Bugbee Zen</a>**에 로그인해 Go를 구독하고 API 키를 복사합니다.
2. TUI에서 `/connect` 명령을 실행하고 `Bugbee Go`를 선택한 다음 API 키를 붙여넣습니다.
3. TUI에서 `/models`를 실행해 Go를 통해 사용할 수 있는 모델 목록을 확인합니다.

:::note
workspace당 한 명의 멤버만 Bugbee Go를 구독할 수 있습니다.
:::
...
Bugbee config의 [model id](/docs/config/#models)는 `bugbee-go/<model-id>` 형식을 사용합니다. 예를 들어 Kimi K3의 경우 config에서 `bugbee-go/kimi-k3`를 사용하면 됩니다.
...
4. Bugbee에서 다른 제공자도 함께 사용할 수 있게 해 **종속성 없음**을 유지합니다.
