...
1. TUI에서 `/connect` 명령을 실행하고 `Bugbee Zen`을 선택한 뒤, [bugbee.dev/auth](https://bugbee.dev/auth)로 이동합니다.
...
1. TUI에서 `/connect` 명령을 실행하고 `Bugbee Go`를 선택한 뒤 [bugbee.dev/auth](https://bugbee.dev/auth)로 이동하십시오.
