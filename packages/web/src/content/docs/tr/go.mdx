...
Bugbee Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Bugbee'daki diğer sağlayıcılar gibi çalışır. Bugbee Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Bugbee'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Bugbee Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Bugbee Go, Bugbee'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Bugbee Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Bugbee Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Bugbee Go'ya abone olabilir.
:::
...
Bugbee Go aşağıdaki limitleri içerir:
...
Bugbee yapılandırmanızdaki [model id](/docs/config/#models) formatı `bugbee-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `bugbee-go/kimi-k3` kullanmalısınız.
...
Bugbee Go'yu şu amaçlarla oluşturduk:
...
4. Bugbee ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
