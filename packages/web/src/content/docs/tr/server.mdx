---
title: Sunucu
description: bugbee sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`bugbee serve` komutu, bugbee istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `bugbee.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `BUGBEE_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `bugbee` değeridir; değiştirmek isterseniz `BUGBEE_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `bugbee serve` hem de `bugbee web` için geçerlidir.
...
`bugbee` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
bugbee ile programatik etkileşim için bugbee sunucusunu kullanın.
:::

Bu mimari, bugbee'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`bugbee serve` ile bağımsız bir sunucu başlatabilirsiniz. bugbee TUI açıksa, `bugbee serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum bugbee [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
bugbee sunucusu aşağıdaki API'leri sunar.
