---
title: Yapılandırma
description: bugbee JSON yapılandırması kullanır.
---

bugbee'u JSON yapılandırma dosyası kullanarak yapılandırabilirsiniz.

---

## Format

bugbee hem **JSON** hem de **JSONC** (JSON Yorumlarla birlikte) formatlarını destekler.
...
Örneğin, genel ayarınız `theme: "bugbee"` ve `autoupdate: true`'yi ayarlıyorsa ve proje ayarlarınız `model: "anthropic/claude-sonnet-4-5"`'yi ayarlıyorsa, nihai yapılandırma üç ayarı da içerecektir.
...
1. **Uzak yapılandırma** (`.well-known/bugbee`'dan) - kurumsal varsayılanlar
2. **Genel yapılandırma** (`~/.config/bugbee/bugbee.json`) - kullanıcı tercihleri
3. **Özel yapılandırma** (`BUGBEE_CONFIG` env var) - özel geçersiz kılmalar
4. **Proje yapılandırması** (projedeki `bugbee.json`) - projeye özgü ayarlar
5. **`.bugbee` dizinleri** - agent'lar, komutlar, eklentiler
6. **Satır içi yapılandırma** (`BUGBEE_CONFIG_CONTENT` env var) - çalışma zamanı geçersiz kılmaları
...
:::note
`.bugbee` ve `~/.config/bugbee` dizinleri alt dizinler için **çoğul adlar** kullanır: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` ve `themes/`. Tekil adlar (örneğin, `agent/`) da geriye dönük uyumluluk açısından desteklenir.
:::
...
Kuruluşlar, `.well-known/bugbee` uç noktası aracılığıyla varsayılan yapılandırmayı sağlayabilir. Bu, onu destekleyen bir sağlayıcıyla kimlik doğrulaması yaptığınızda otomatik olarak alınır.
...
Global bugbee yapılandırmanızı `~/.config/bugbee/bugbee.json` içine yerleştirin. Temalar, sağlayıcılar veya tuş atamaları gibi kullanıcı çapındaki tercihler için genel yapılandırmayı kullanın.
...
Proje kökünüze `bugbee.json` ekleyin. Proje yapılandırması, standart yapılandırma dosyaları arasında en yüksek önceliğe sahiptir; hem genel hem de uzak yapılandırmaları geçersiz kılar.

:::tip
Projeye özel yapılandırmayı projenizin köküne yerleştirin.
:::

bugbee başlatıldığında geçerli dizinde bir yapılandırma dosyası arar veya en yakın Git dizinine gider.
...
`BUGBEE_CONFIG` ortam değişkenini kullanarak özel bir yapılandırma dosyası yolu belirtin.
...
`BUGBEE_CONFIG_DIR` kullanarak özel bir yapılandırma dizini belirtin
ortam değişkeni. Bu dizin agent'lar, komutlar için aranacaktır.
modlar ve eklentiler tıpkı standart `.bugbee` dizini gibi olmalıdır ve
...
Özel dizin, genel yapılandırma ve `.bugbee` dizinlerinden sonra yüklenir, böylece bunların ayarlarını **geçersiz kılabilir**.

---

## Şema

Yapılandırma dosyası [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json)'da tanımlanan bir şemaya sahiptir.
...
`bugbee serve` ve `bugbee web` komutları için sunucu ayarlarını `server` seçeneği aracılığıyla yapılandırabilirsiniz.
...
- `mdns` - mDNS hizmet bulmayı etkinleştirin. Bu, ağdaki diğer cihazların bugbee sunucunuzu keşfetmesine olanak tanır.
- `mdnsDomain` - mDNS hizmeti için özel alan adı. Varsayılan olarak `bugbee.local` şeklindedir. Aynı ağda birden fazla örneği çalıştırmak için kullanışlıdır.
...
bugbee yapılandırmanızda kullanmak istediğiniz sağlayıcıları ve modelleri `provider`, `model` ve `small_model` seçenekleri aracılığıyla yapılandırabilirsiniz.
...
`small_model` seçeneği, başlık oluşturma gibi hafif görevler için ayrı bir model yapılandırır. Varsayılan olarak bugbee, sağlayıcınızda mevcutsa daha ucuz bir model kullanmaya çalışır, aksi takdirde ana modelinize geri döner.
...
bugbee yapılandırmanızda kullanmak istediğiniz temayı `theme` seçeneği aracılığıyla yapılandırabilirsiniz.
...
Agent'ları `~/.config/bugbee/agents/` veya `.bugbee/agents/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/agents).
...
Varsayılan agent, birincil agent olmalıdır (alt agent değil). Bu, `"build"` veya `"plan"` gibi yerleşik bir agent ya da tanımladığınız bir [özel agent](/docs/agents) olabilir. Belirtilen agent mevcut değilse veya bir alt agent ise, bugbee bir uyarıyla birlikte `"build"`'ye geri döner.

Bu ayar tüm arayüzler için geçerlidir: TUI, CLI (`bugbee run`), masaüstü uygulaması ve GitHub Action.
...
Komutları `~/.config/bugbee/commands/` veya `.bugbee/commands/` içindeki Markdown dosyalarıyla da tanımlayabilirsiniz. [Daha fazla bilgi](/docs/commands).
...
bugbee başlatıldığında yeni güncellemeleri otomatik olarak indirecektir. Bunu `autoupdate` seçeneğiyle devre dışı bırakabilirsiniz.
...
Varsayılan olarak, bugbee açık bir onay gerektirmeden **tüm işlemlere izin verir**. `permission` seçeneğini kullanarak bunu değiştirebilirsiniz.
...
[Eklentiler](/docs/plugins) bugbee'u özel araçlar, kancalar ve entegrasyonlarla genişletin.

Eklenti dosyalarını `.bugbee/plugins/` veya `~/.config/bugbee/plugins/` içine yerleştirin. Ayrıca eklentileri `plugin` seçeneği aracılığıyla npm'den de yükleyebilirsiniz.
...
Bu, bugbee'u tek tek devre dışı bırakmak yerine yalnızca belirli sağlayıcıları kullanacak şekilde kısıtlamak istediğinizde kullanışlıdır.
