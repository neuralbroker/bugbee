---
title: Eklentiler
description: Bugbee'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Bugbee'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Bugbee'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.bugbee/plugins/` - Proje düzeyinde eklentiler
- `~/.config/bugbee/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/bugbee/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/bugbee/bugbee.json`)
2. Project config (`bugbee.json`)
3. Global eklenti dizini (`~/.config/bugbee/plugins/`)
4. Proje eklenti dizini (`.bugbee/plugins/`)
...
Bugbee bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Bugbee SDK istemcisi.
...
Bugbee'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Bugbee masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Bugbee'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Bugbee'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Bugbee'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Bugbee'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://github.com/neuralbroker/bugbee/docs/sdk) bakın.
