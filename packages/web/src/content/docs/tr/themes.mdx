...
bugbee ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak bugbee kendi `bugbee` temasını kullanır.
...
bugbee birden fazla yerleşik temayla gelir.
...
- bugbee'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
bugbee, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/bugbee/themes/*.json` veya `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.bugbee/themes/*.json`
4. **Geçerli çalışma dizini** - `./.bugbee/themes/*.json`
