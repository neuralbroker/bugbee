...
_Model Bağlam Protokolü_ veya MCP kullanarak bugbee'a harici araçlar ekleyebilirsiniz. bugbee hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/bugbee` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
bugbee, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde bugbee şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa bugbee, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `bugbee mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından bugbee, belirteçleri `~/.local/share/bugbee/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra bugbee'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi bugbee yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve bugbee'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
