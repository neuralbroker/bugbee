---
title: Sağlayıcılar
description: bugbee'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

bugbee, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı bugbee yapılandırmanızda yapılandırın.
...
`~/.local/share/bugbee/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları bugbee'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Bugbee Zen, bugbee ekibi tarafından sağlanan modellerin bir listesidir.
bugbee ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Bugbee Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, bugbee'u seçin ve [bugbee.dev/auth](https://bugbee.dev/auth)'ye gidin.
...
bugbee'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Bugbee Go

Bugbee Go, bugbee ile iyi çalıştığı test edilmiş ve doğrulanmış, bugbee ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Bugbee Go`'yu seçin ve [bugbee.dev/auth](https://bugbee.dev/zen) adresine gidin.
...
Amazon Bedrock'u bugbee ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   bugbee'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `bugbee.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi bugbee'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
bugbee'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   bugbee'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Bugbee yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
bugbee, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. bugbee'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`bugbee.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth) ana sayfasında.

##### Yapılandırma

`bugbee.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi bugbee'la kullanmak için:
...
Google Vertex AI'yi bugbee ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları bugbee çalıştırılırken ayarlayın.
...
Helicone'un bugbee aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her bugbee görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her bugbee görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
bugbee'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla bugbee'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla bugbee'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini bugbee için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/bugbee)'a bakın.
:::
...
Ollama Cloud'u bugbee ile kullanmak için:
...
3. bugbee'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: bugbee'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Bugbee Zen, bugbee ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Bugbee Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Bugbee Zen**'i arayın.

   ```txt
   /connect
   ```

3. bugbee API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   bugbee yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca bugbee yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Bugbee ile kullanmak için:
...
Modelleri bugbee yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı bugbee ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `bugbee.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları bugbee'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `bugbee auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için bugbee yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, bugbee yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
