...
description: Bugbee, LSP sunucularınızla bütünleşir.
---

Bugbee, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Bugbee, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`BUGBEE_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve bugbee bir dosyayı açtığında:
...
LSP sunucularını, bugbee yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
