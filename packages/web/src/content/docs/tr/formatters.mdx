...
description: bugbee dile özgü biçimlendiriciler kullanır.
---

bugbee, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

bugbee, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, bugbee bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

bugbee bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri bugbee yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
