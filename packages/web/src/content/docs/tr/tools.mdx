...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. bugbee yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
bugbee içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (veya `BUGBEE_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca bugbee provider kullanılırken veya `BUGBEE_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

bugbee başlatırken etkinleştirmek için:
