...
description: Bugbee se integra con sus servidores LSP.
---

Bugbee puede integrarse con servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para el agente.

---

## Incorporados

Bugbee viene con varios servidores LSP integrados para idiomas populares:
...
:::note
Puede deshabilitar las descargas automáticas del servidor LSP configurando la variable de entorno `BUGBEE_DISABLE_LSP_DOWNLOAD` en `true`.
:::
...
Cuando LSP está habilitado y bugbee abre un archivo:
...
Puede habilitar y personalizar los servidores LSP a través de la sección `lsp` en su configuración bugbee.
