---
title: Complementos
description: Escriba sus propios complementos para extender Bugbee.
---

Los complementos le permiten extender Bugbee al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Bugbee.
...
- `.bugbee/plugins/` - Complementos a nivel de proyecto
- `~/.config/bugbee/plugins/` - Complementos globales
...
```json title="bugbee.json"
{
  ...
  "plugin": ["bugbee-helicone-session", "bugbee-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/bugbee/node_modules/`.
...
1. Configuración global (`~/.config/bugbee/bugbee.json`)
2. Configuración del proyecto (`bugbee.json`)
3. Directorio global de complementos (`~/.config/bugbee/plugins/`)
4. Directorio de complementos del proyecto (`.bugbee/plugins/`)
...
```json title=".bugbee/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Bugbee ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".bugbee/plugins/my-plugin.ts"
...
```js title=".bugbee/plugins/example.js"
...
- `client`: Un cliente SDK bugbee para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar bugbee.
...
```js title=".bugbee/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "bugbee"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Bugbee, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que bugbee lea archivos `.env`:

```javascript title=".bugbee/plugins/env-protection.js"
...
```javascript title=".bugbee/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a bugbee:

```ts title=".bugbee/plugins/custom-tools.ts"
import { type Plugin, tool } from "@bugbee-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que bugbee puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para bugbee junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://github.com/neuralbroker/bugbee/docs/sdk) para obtener más detalles.
...
```ts title=".bugbee/plugins/compaction.ts"
import type { Plugin } from "@bugbee-ai/plugin"
...
```ts title=".bugbee/plugins/custom-compaction.ts"
import type { Plugin } from "@bugbee-ai/plugin"
