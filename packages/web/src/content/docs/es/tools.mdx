...
Las herramientas permiten que LLM realice acciones en su código base. Bugbee viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Bugbee.
...
:::note
Esta herramienta solo está disponible cuando `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (o `BUGBEE_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Bugbee o cuando la variable de entorno `BUGBEE_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Bugbee:

```bash
BUGBEE_ENABLE_EXA=1 bugbee
```
