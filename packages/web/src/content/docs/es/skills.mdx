...
Las habilidades del agente permiten a Bugbee descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Bugbee busca estas ubicaciones:

- Configuración del proyecto: `.bugbee/skills/<name>/SKILL.md`
- Configuración global: `~/.config/bugbee/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Bugbee sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.bugbee/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/bugbee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.bugbee/skills/git-release/SKILL.md` así:
...
Bugbee enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `bugbee.json`:
...
**Para agentes integrados** (en `bugbee.json`):
