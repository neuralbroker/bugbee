...
Puede agregar herramientas externas a Bugbee utilizando el _Model Context Protocol_ o MCP. Bugbee admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/bugbee`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Bugbee maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Bugbee:
...
Si el servidor requiere autenticación, Bugbee le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `bugbee mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Bugbee almacenará los tokens de forma segura en `~/.local/share/bugbee/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Bugbee, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Bugbee como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Bugbee a su cuenta Sentry.
