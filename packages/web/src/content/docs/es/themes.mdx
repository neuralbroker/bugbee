...
Con Bugbee puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Bugbee usa nuestro propio tema `bugbee`.
...
Bugbee viene con varios temas integrados.
...
- Quiere que Bugbee coincida con la apariencia de su terminal
...
Bugbee admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/bugbee/themes/*.json` o `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.bugbee/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.bugbee/themes/*.json`
