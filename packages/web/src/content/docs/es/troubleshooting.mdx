...
Para depurar problemas con Bugbee, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\bugbee\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `bugbee --log-level DEBUG`.

---

## Almacenamiento

bugbee almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/bugbee/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\bugbee`
...
Bugbee Desktop ejecuta un servidor Bugbee local (el sidecar `bugbee-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Bugbee` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/bugbee/bugbee.jsonc` (o `~/.config/bugbee/bugbee.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/bugbee/bugbee.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\bugbee\bugbee.jsonc`
...
Bugbee también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/bugbee/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\bugbee\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Bugbee pueda reconstruirlo.

1. Salga de Bugbee Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/bugbee`
- **Linux**: eliminar `~/.cache/bugbee` (o ejecutar `rm -rf ~/.cache/bugbee`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\bugbee`

3. Reinicie el escritorio Bugbee.

---

### Solucionar problemas de conexión del servidor

Bugbee Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `bugbee.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `BUGBEE_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `BUGBEE_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Bugbee requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Bugbee.

---

### Notificaciones que no se muestran

Bugbee Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Bugbee en la configuración de su sistema operativo, y
...
1. Salga del escritorio Bugbee.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Bugbee):

- `bugbee.settings.dat` (URL del servidor predeterminado de escritorio)
- `bugbee.global.dat` y `bugbee.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `bugbee upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `bugbee models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\bugbee`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. bugbee instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\bugbee`

2. Reinicie bugbee para reinstalar los paquetes del proveedor más recientes.

Esto obligará a bugbee a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
bugbee detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
