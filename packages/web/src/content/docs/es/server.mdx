---
title: Servidor
description: Interactuar con el servidor bugbee a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `bugbee serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente bugbee puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `bugbee.local` |
...
Configure `BUGBEE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `bugbee`, o configure `BUGBEE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `bugbee serve` como a `bugbee web`.
...
Cuando ejecuta `bugbee`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor bugbee para interactuar con bugbee mediante programación.
:::

Esta arquitectura permite que bugbee admita múltiples clientes y le permite interactuar con bugbee mediante programación.

Puede ejecutar `bugbee serve` para iniciar un servidor independiente. Si tienes el
bugbee TUI ejecutándose, `bugbee serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Bugbee [IDE](/docs/ide).
...
El servidor bugbee expone las siguientes API.
