...
description: Cree herramientas que LLM pueda llamar en bugbee.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de bugbee como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.bugbee/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/bugbee/tools/`.
...
```ts title=".bugbee/tools/database.ts" {1}
...
```ts title=".bugbee/tools/project.ts" {8}
import { tool } from "@bugbee-ai/plugin"
...
```python title=".bugbee/tools/add.py"
import sys
...
```ts title=".bugbee/tools/python-add.ts" {10}
import { tool } from "@bugbee-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".bugbee/tools/add.py")
