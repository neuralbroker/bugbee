---
title: Configuración
description: Usando la configuración Bugbee JSON.
---

Puede configurar Bugbee usando un archivo de configuración JSON.

---

## Formato

Bugbee admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
Por ejemplo, si su configuración global establece `theme: "bugbee"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/bugbee`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/bugbee/bugbee.json`) - preferencias del usuario
3. **Configuración personalizada** (`BUGBEE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`bugbee.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.bugbee`** - agentes, comandos, complementos
6. **Configuración en línea** (`BUGBEE_CONFIG_CONTENT` env var): anulaciones del tiempo de ejecución
...
:::note
Los directorios `.bugbee` y `~/.config/bugbee` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/bugbee`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global Bugbee en `~/.config/bugbee/bugbee.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `bugbee.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia Bugbee, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
Especifique una ruta de archivo de configuración personalizada utilizando la variable de entorno `BUGBEE_CONFIG`.
...
Especifique un directorio de configuración personalizado usando `BUGBEE_CONFIG_DIR`
variable de entorno. En este directorio se buscarán agentes, comandos,
modos y complementos como el directorio estándar `.bugbee`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.bugbee`, por lo que **puede anular** su configuración.

---

## Esquema

El archivo de configuración tiene un esquema definido en [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).
...
Puede configurar los ajustes del servidor para los comandos `bugbee serve` y `bugbee web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor Bugbee.
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `bugbee.local`. Útil para ejecutar múltiples instancias en la misma red.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración Bugbee a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, Bugbee intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración Bugbee a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/bugbee/agents/` o `.bugbee/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, Bugbee recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`bugbee run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/bugbee/commands/` o `.bugbee/commands/`. [Más información aquí](/docs/commands).
...
Bugbee descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
De forma predeterminada, bugbee **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Los [complementos](/docs/plugins) amplían Bugbee con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.bugbee/plugins/` o `~/.config/bugbee/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir Bugbee para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
