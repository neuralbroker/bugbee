---
title: Proveedores
description: Usando cualquier proveedor LLM en Bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

Bugbee usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Bugbee.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Bugbee
configuración.
...
Bugbee Zen es una lista de modelos proporcionados por el equipo Bugbee que han sido
probado y verificado para funcionar bien con Bugbee. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Bugbee Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione bugbee y diríjase a [bugbee.dev/auth](https://bugbee.dev/auth).
...
Funciona como cualquier otro proveedor en Bugbee y su uso es completamente opcional.

---

## Bugbee Go

Bugbee Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Bugbee que han sido
probado y verificado para funcionar bien con Bugbee.

1. Ejecute el comando `/connect` en TUI, seleccione `Bugbee Go` y diríjase a [bugbee.dev/auth](https://bugbee.dev/zen).
...
Para usar Amazon Bedrock con Bugbee:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta bugbee:
   ...
   Para una configuración persistente o específica del proyecto, utilice `bugbee.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Bugbee no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar bugbee para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que bugbee funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración bugbee.
...
para almacenar el token en el almacenamiento de autenticación bugbee.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Bugbee utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Bugbee
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `bugbee.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth).

##### Configuración

Personalizar a través de `bugbee.json`:
...
Para utilizar su suscripción GitHub Copilot con bugbee:
...
Para utilizar Google Vertex AI con Bugbee:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta bugbee.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de bugbee, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) para registrar automáticamente cada conversación Bugbee como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Bugbee enumerada como una sesión separada.
...
Puede configurar bugbee para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar bugbee para usar modelos locales a través de LM Studio.
...
Puedes configurar bugbee para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Bugbee. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/bugbee) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Bugbee:
...
3. Copie la clave API para usarla en Bugbee.
...
6. **Importante**: Antes de usar modelos en la nube en Bugbee, debe obtener la información del modelo localmente:
...
Bugbee Zen es una lista de modelos probados y verificados proporcionada por el equipo Bugbee. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Bugbee Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Bugbee Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Bugbee API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración bugbee.
...
5. También puedes personalizarlos a través de tu configuración bugbee. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Bugbee:
...
También puede personalizar modelos a través de su configuración bugbee. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con bugbee. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `bugbee.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Bugbee comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `bugbee auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración bugbee y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración bugbee.
