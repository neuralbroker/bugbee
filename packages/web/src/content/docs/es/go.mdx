...
Bugbee Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en Bugbee. Te suscribes a Bugbee Go y
...
Bugbee Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Bugbee Go funciona como cualquier otro proveedor en Bugbee.

1. Inicias sesión en **<a href={console}>Bugbee Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Bugbee Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Bugbee Go.
:::
...
Bugbee Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Bugbee
usa el formato `bugbee-go/<model-id>`. Por ejemplo, para Kimi K3, usarías
`bugbee-go/kimi-k3` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Bugbee.
