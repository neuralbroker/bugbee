...
Puoi aggiungere strumenti esterni a Bugbee usando il _Model Context Protocol_, o MCP. Bugbee supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/bugbee`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Bugbee gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Bugbee:
...
Se il server richiede autenticazione, Bugbee ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `bugbee mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Bugbee memorizzera' i token in modo sicuro in `~/.local/share/bugbee/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Bugbee insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Bugbee come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Bugbee al tuo account Sentry.
