...
Le skill degli agenti permettono a Bugbee di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Bugbee cerca in queste posizioni:

- Config di progetto: `.bugbee/skills/<name>/SKILL.md`
- Config globale: `~/.config/bugbee/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Bugbee risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.bugbee/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/bugbee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.bugbee/skills/git-release/SKILL.md` cosi':
...
Bugbee elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `bugbee.json`:
...
**Per agenti integrati** (in `bugbee.json`):
