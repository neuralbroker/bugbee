---
title: Provider
description: Usa qualsiasi provider LLM in Bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

Bugbee usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Bugbee.
...
Bugbee Zen e una lista di modelli forniti dal team Bugbee, testati e
verificati per funzionare bene con Bugbee. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Bugbee Zen.
:::

1. Esegui `/connect` nella TUI, seleziona bugbee e vai su [bugbee.dev/auth](https://bugbee.dev/auth).
...
Funziona come qualsiasi altro provider in Bugbee ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Bugbee:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui bugbee:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `bugbee.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Bugbee non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare bugbee per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché bugbee funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di bugbee.
...
salvare il token nello storage di auth di bugbee.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Bugbee usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Bugbee
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `bugbee.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth).

##### Configurazione

Personalizza tramite `bugbee.json`:
...
Per usare il tuo abbonamento GitHub Copilot con bugbee:
...
Per usare Google Vertex AI con Bugbee:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui bugbee.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da bugbee, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) per loggare automaticamente ogni conversazione di Bugbee come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Bugbee elencata come sessione separata.
...
Puoi configurare bugbee per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare bugbee per usare modelli locali tramite LM Studio.
...
Puoi configurare bugbee per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Bugbee. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/bugbee) per dettagli.
:::
...
Per usare Ollama Cloud con Bugbee:
...
3. Copia la chiave API da usare in Bugbee.
...
6. **Importante**: Prima di usare modelli cloud in Bugbee, devi scaricare le informazioni del modello localmente:
...
Bugbee Zen è una lista di modelli testati e verificati forniti dal team Bugbee. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Bugbee Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Bugbee Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Bugbee.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di bugbee.
...
5. Puoi anche personalizzarli tramite la configurazione di bugbee. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Bugbee:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di bugbee. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con bugbee. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `bugbee.json` nella directory del progetto:
...
I campi `limit` permettono a Bugbee di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `bugbee auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di bugbee e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config bugbee.
