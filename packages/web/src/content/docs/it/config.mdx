...
description: Usa la configurazione JSON di Bugbee.
---

Puoi configurare Bugbee usando un file di configurazione JSON.

---

## Formato

Bugbee supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/bugbee`) - default dell'organizzazione
2. **Config globale** (`~/.config/bugbee/bugbee.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `BUGBEE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`bugbee.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.bugbee`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `BUGBEE_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.bugbee` e `~/.config/bugbee` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/bugbee`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Bugbee in `~/.config/bugbee/bugbee.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/bugbee/tui.json`.
...
Aggiungi `bugbee.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Bugbee si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `BUGBEE_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `BUGBEE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.bugbee` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.bugbee`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).

La configurazione TUI usa [**`bugbee.dev/tui.json`**](https://github.com/neuralbroker/bugbee/tui.json).
...
Usa `BUGBEE_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `bugbee.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `bugbee serve` e `bugbee web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Bugbee.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `bugbee.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Bugbee tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Bugbee prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/bugbee/agents/` o `.bugbee/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Bugbee fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`bugbee run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/bugbee/commands/` o `.bugbee/commands/`. [Scopri di piu](/docs/commands).
...
Bugbee scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, bugbee **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Bugbee con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.bugbee/plugins/` o `~/.config/bugbee/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Bugbee a usare solo alcuni provider invece di disabilitarli uno per uno.
