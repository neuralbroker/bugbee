---
title: Server
description: Interagisci con il server bugbee via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `bugbee serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client bugbee.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `bugbee.local` |
...
Imposta `BUGBEE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `bugbee`, oppure imposta `BUGBEE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `bugbee serve` sia per `bugbee web`.
...
Quando esegui `bugbee` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server bugbee per interagire con bugbee in modo programmatico.
:::

Questa architettura permette a bugbee di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `bugbee serve` per avviare un server standalone. Se la TUI di bugbee e gia in esecuzione, `bugbee serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Bugbee.
...
Il server bugbee espone le seguenti API.
