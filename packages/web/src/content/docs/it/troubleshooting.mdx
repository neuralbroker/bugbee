...
Per diagnosticare problemi con Bugbee, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\bugbee\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `bugbee --log-level DEBUG`.

---

## Archiviazione

bugbee salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/bugbee/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\bugbee`
...
Bugbee Desktop esegue in background un server locale di Bugbee (il sidecar `bugbee-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Bugbee` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/bugbee/bugbee.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\bugbee\bugbee.jsonc`
...
Bugbee puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/bugbee/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\bugbee\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Bugbee possa ricostruirla.

1. Chiudi completamente Bugbee Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/bugbee`
- **Linux**: elimina `~/.cache/bugbee` (oppure esegui `rm -rf ~/.cache/bugbee`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\bugbee`

3. Riavvia Bugbee Desktop.

---

### Risolvi problemi di connessione al server

Bugbee Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `bugbee.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `BUGBEE_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `BUGBEE_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Bugbee Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Bugbee.

---

### Notifiche non visualizzate

Bugbee Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Bugbee nelle impostazioni del sistema operativo, e
...
1. Chiudi Bugbee Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Bugbee Desktop):

- `bugbee.settings.dat` (desktop default server URL)
- `bugbee.global.dat` e `bugbee.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Bugbee:
...
3. Assicurati di avere l'ultima versione con `bugbee upgrade`
...
Per capire a quali modelli hai accesso, esegui `bugbee models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\bugbee`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. bugbee installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\bugbee`

2. Riavvia bugbee per reinstallare i pacchetti provider piu recenti

Questo forzera bugbee a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
bugbee rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
