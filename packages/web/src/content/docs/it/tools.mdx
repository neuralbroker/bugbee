...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Bugbee include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Bugbee.
...
:::note
Questo strumento e' disponibile solo quando `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (oppure `BUGBEE_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Bugbee o quando la variabile d'ambiente `BUGBEE_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Bugbee:
