---
title: Server LSP
description: Bugbee si integra con i tuoi server LSP.
---

Bugbee può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Bugbee include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `BUGBEE_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e bugbee apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di bugbee.
