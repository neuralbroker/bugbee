---
title: Plugin
description: Scrivi plugin per estendere Bugbee.
---

I plugin ti permettono di estendere Bugbee agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Bugbee.
...
- `.bugbee/plugins/` - plugin a livello progetto
- `~/.config/bugbee/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/bugbee/node_modules/`.
...
1. Config globale (`~/.config/bugbee/bugbee.json`)
2. Config di progetto (`bugbee.json`)
3. Directory plugin globale (`~/.config/bugbee/plugins/`)
4. Directory plugin di progetto (`.bugbee/plugins/`)
...
Bugbee esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di bugbee per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere bugbee.
...
:::note
Se usi l'app desktop di Bugbee, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a bugbee di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a bugbee:
...
L'helper `tool` crea uno strumento personalizzato che bugbee può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in bugbee insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://github.com/neuralbroker/bugbee/docs/sdk) per i dettagli.
