...
Bugbee Go 是一項低成本的訂閱服務——**首月 $5 美元**，之後**每月 $10 美元**——讓您能穩定使用受歡迎的開源寫程式模型。

Go 的運作方式與 Bugbee 中的任何其他供應商相同。您訂閱 Bugbee Go 並取得您的 API key。這是**完全可選的**，您不需要使用它也能使用 Bugbee。
...
:::tip
我們測試了一組精選的模型和供應商，它們與 Bugbee 搭配運作良好。
:::
...
Bugbee Go 讓您可以存取這些模型，**首月只需 $5 美元**，之後**每月 $10 美元**。

---

## 運作方式

Bugbee Go 的運作方式與 Bugbee 中的任何其他供應商相同。

1. 您登入 **<a href={console}>Bugbee Zen</a>**，訂閱 Go，然後複製您的 API key。
2. 您在 TUI 中執行 `/connect` 命令，選擇 `Bugbee Go`，然後貼上您的 API key。
3. 在 TUI 中執行 `/models` 即可查看透過 Go 可用的模型清單。

:::note
每個工作區只能有一位成員訂閱 Bugbee Go。
:::
...
您的 Bugbee 設定中的 [model id](/docs/config/#models) 使用 `bugbee-go/<model-id>` 格式。例如，Kimi K3 在設定中應使用 `bugbee-go/kimi-k3`。
...
4. 允許您在 Bugbee 中同時使用任何其他供應商，確保**不被單一廠商綁定**。
