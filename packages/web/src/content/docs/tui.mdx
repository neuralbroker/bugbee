---
title: TUI
description: Using the Bugbee terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Bugbee provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Bugbee starts the TUI for the current directory.
...
When using the Bugbee TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Bugbee. Allows you to select from available providers and add their API keys.
...
This is separate from `bugbee.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Bugbee waits after the leader key. Defaults to `2000`.
...
Use `BUGBEE_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `bugbee.default`.
