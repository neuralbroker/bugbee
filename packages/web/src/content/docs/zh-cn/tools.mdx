...
工具允许 LLM 在您的代码库中执行操作。Bugbee 自带一组内置工具，您也可以通过[自定义工具](/docs/custom-tools)或 [MCP 服务器](/docs/mcp-servers)来扩展它。
...
:::note
该工具仅在设置 `BUGBEE_EXPERIMENTAL_LSP_TOOL=true`（或 `BUGBEE_EXPERIMENTAL=true`）时可用。
:::
...
:::note
该工具仅在使用 Bugbee 提供商时，或当 `BUGBEE_ENABLE_EXA` 环境变量设置为任意真值（例如 `true` 或 `1`）时可用。
