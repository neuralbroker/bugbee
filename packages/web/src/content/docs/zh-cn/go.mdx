...
Bugbee Go 是一项低成本的订阅服务 —— **首月 5 美元**，之后 **每月 10 美元** —— 让你能够稳定地访问流行的开源编程模型。

Go 的工作方式与 Bugbee 中的任何其他提供商（provider）一样。订阅 Bugbee Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 Bugbee 所必需的条件。
...
:::tip
我们测试了一组经过精选且与 Bugbee 配合良好的模型和提供商。
:::
...
Bugbee Go 让你能够访问这些模型，**首月只需 5 美元**，之后 **每月 10 美元**。

---

## 工作原理

Bugbee Go 的工作方式与 Bugbee 中的其他提供商一样。

1. 登录 **<a href={console}>Bugbee Zen</a>**，订阅 Go，然后复制你的 API 密钥。
2. 在 TUI 中运行 `/connect` 命令，选择 `Bugbee Go`，然后粘贴你的 API 密钥。
3. 在 TUI 中运行 `/models` 以查看通过 Go 可用的模型列表。

:::note
每个工作空间只能有一名成员订阅 Bugbee Go。
:::
...
你的 Bugbee 配置中的 [模型 ID](/docs/config/#models) 使用 `bugbee-go/<model-id>` 格式。例如，对于 Kimi K3，你将在配置中使用 `bugbee-go/kimi-k3`。
...
4. **无锁定（no lock-in）**，允许你与 Bugbee 一起使用任何其他提供商。
