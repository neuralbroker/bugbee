...
O Bugbee Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Bugbee. Você assina o Bugbee Go e
...
O Bugbee Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Bugbee Go funciona como qualquer outro provedor no Bugbee.

1. Você entra no **<a href={console}>Bugbee Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Bugbee Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Bugbee Go.
:::
...
O Bugbee Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Bugbee
usa o formato `bugbee-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`bugbee-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Bugbee também.
