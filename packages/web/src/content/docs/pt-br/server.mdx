---
title: Servidor
description: Interaja com o servidor bugbee via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `bugbee serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente bugbee pode usar.
...
```bash
bugbee serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `bugbee.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
bugbee serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `BUGBEE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `bugbee`, ou defina `BUGBEE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `bugbee serve` quanto ao `bugbee web`.

```bash
BUGBEE_SERVER_PASSWORD=your-password bugbee serve
```
...
Quando você executa `bugbee`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor bugbee para interagir com o bugbee programaticamente.
:::

Essa arquitetura permite que o bugbee suporte múltiplos clientes e permite que você interaja com o bugbee programaticamente.

Você pode executar `bugbee serve` para iniciar um servidor autônomo. Se você tiver o TUI do bugbee em execução, `bugbee serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do bugbee [IDE](/docs/ide).
...
O servidor bugbee expõe as seguintes APIs.
