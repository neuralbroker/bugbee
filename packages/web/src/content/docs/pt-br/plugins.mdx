---
title: Plugins
description: Escreva seus próprios plugins para estender o bugbee.
---

Plugins permitem que você estenda o bugbee conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do bugbee.
...
- `.bugbee/plugins/` - Plugins em nível de projeto
- `~/.config/bugbee/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/bugbee/node_modules/`.
...
1. Configuração global (`~/.config/bugbee/bugbee.json`)
2. Configuração do projeto (`bugbee.json`)
3. Diretório de plugins global (`~/.config/bugbee/plugins/`)
4. Diretório de plugins do projeto (`.bugbee/plugins/`)
...
O bugbee executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do bugbee para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o bugbee.
...
:::note
Se você estiver usando o aplicativo desktop bugbee, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o bugbee de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao bugbee:
...
O helper `tool` cria uma ferramenta personalizada que o bugbee pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o bugbee junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://github.com/neuralbroker/bugbee/docs/sdk) para detalhes.
