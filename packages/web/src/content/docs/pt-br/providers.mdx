---
title: Provedores
description: Usando qualquer provedor LLM no bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

bugbee usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do bugbee.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/bugbee/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do bugbee.
...
Bugbee Zen é uma lista de modelos fornecidos pela equipe do bugbee que foram testados e verificados para funcionar bem com o bugbee. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Bugbee Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Bugbee Zen` e acesse [bugbee.dev/zen](https://bugbee.dev/zen).
...
Funciona como qualquer outro provedor no bugbee e é completamente opcional.

---

## Bugbee Go

Bugbee Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do bugbee que foram
testados e verificados para funcionar bem com o bugbee.

1. Execute o comando `/connect` no TUI, selecione `Bugbee Go` e acesse [bugbee.dev/zen](https://bugbee.dev/zen).
...
Para usar o Amazon Bedrock com o bugbee:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o bugbee:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `bugbee.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no bugbee não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o bugbee para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o bugbee funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do bugbee.
...
armazenar o token no armazenamento de autenticação do bugbee.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
bugbee usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o bugbee
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `bugbee.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth).

##### Configuração

Personalize através do `bugbee.json`:
...
Para usar sua assinatura do GitHub Copilot com o bugbee:
...
Para usar o Google Vertex AI com o bugbee:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o bugbee.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do bugbee, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) para registrar automaticamente cada conversa do bugbee como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do bugbee listada como uma sessão separada.
...
Você pode configurar o bugbee para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o bugbee para usar modelos locais através do LM Studio.
...
Você pode configurar o bugbee para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o bugbee. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/bugbee) para detalhes.
:::
...
Para usar o Ollama Cloud com o bugbee:
...
3. Copie a chave da API para uso no bugbee.
...
6. **Importante**: Antes de usar modelos em nuvem no bugbee, você deve puxar as informações do modelo localmente:
...
Bugbee Zen é uma lista de modelos testados e verificados fornecidos pela equipe do bugbee. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Bugbee Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Bugbee Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do bugbee.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do bugbee.
...
5. Você também pode personalizá-los através da sua configuração do bugbee. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o bugbee:
...
Você também pode personalizar modelos através da sua configuração do bugbee. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o bugbee. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no bugbee.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `bugbee.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o bugbee entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `bugbee auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do bugbee e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do bugbee.
