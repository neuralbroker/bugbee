...
Você pode adicionar ferramentas externas ao bugbee usando o _Model Context Protocol_, ou MCP. O bugbee suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do bugbee](https://github.com/neuralbroker/bugbee/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/bugbee`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O bugbee lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o bugbee irá:
...
Se o servidor requer autenticação, o bugbee solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `bugbee mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o bugbee armazenará os tokens de forma segura em `~/.local/share/bugbee/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no bugbee, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do bugbee como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o bugbee à sua conta do Sentry.
