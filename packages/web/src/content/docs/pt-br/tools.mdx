...
As ferramentas permitem que o LLM execute ações em sua base de código. O bugbee vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no bugbee.
...
:::note
Esta ferramenta está disponível apenas quando `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (ou `BUGBEE_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor bugbee ou quando a variável de ambiente `BUGBEE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o bugbee:
