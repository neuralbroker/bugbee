---
title: Configuração
description: Usando a configuração JSON do bugbee.
---

Você pode configurar o bugbee usando um arquivo de configuração JSON.

---

## Formato

O bugbee suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/bugbee`) - padrões organizacionais
2. **Configuração global** (`~/.config/bugbee/bugbee.json`) - preferências do usuário
3. **Configuração personalizada** (`BUGBEE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`bugbee.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.bugbee`** - agentes, comandos, plugins
6. **Configuração inline** (`BUGBEE_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.bugbee` e `~/.config/bugbee` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/bugbee`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do bugbee em `~/.config/bugbee/bugbee.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/bugbee/tui.json`.
...
Adicione `bugbee.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o bugbee é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `BUGBEE_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `BUGBEE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.bugbee`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.bugbee`, então ele **pode substituir** suas configurações.

---

## Esquema

O esquema de configuração do servidor/tempo de execução é definido em [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).

A configuração do TUI usa [**`bugbee.dev/tui.json`**](https://github.com/neuralbroker/bugbee/tui.json).
...
Use `BUGBEE_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `bugbee.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `bugbee serve` e `bugbee web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor bugbee.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `bugbee.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do bugbee através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o bugbee tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/bugbee/agents/` ou `.bugbee/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o bugbee recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`bugbee run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/bugbee/commands/` ou `.bugbee/commands/`. [Saiba mais aqui](/docs/commands).
...
O bugbee fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o bugbee **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o bugbee com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.bugbee/plugins/` ou `~/.config/bugbee/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o bugbee para usar apenas provedores específicos, em vez de desabilitá-los um a um.
