...
As habilidades do agente permitem que o bugbee descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O bugbee pesquisa nesses locais:

- Configuração do projeto: `.bugbee/skills/<name>/SKILL.md`
- Configuração global: `~/.config/bugbee/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o bugbee sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.bugbee/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/bugbee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.bugbee/skills/git-release/SKILL.md` assim:
...
O bugbee lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `bugbee.json`:
...
**Para agentes embutidos** (em `bugbee.json`):
