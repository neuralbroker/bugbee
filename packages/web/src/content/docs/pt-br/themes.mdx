...
Com o bugbee, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o bugbee usa nosso próprio tema `bugbee`.
...
O bugbee vem com vários temas embutidos.
...
- Querem que o bugbee corresponda à aparência do seu terminal
...
O bugbee suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/bugbee/themes/*.json` ou `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.bugbee/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.bugbee/themes/*.json`
