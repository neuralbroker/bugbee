...
Existem dois tipos de agentes no bugbee; agentes primários e subagentes.
...
bugbee vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Bugbee vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Bugbee vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Bugbee, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `bugbee.json`:
...
- Global: `~/.config/bugbee/agents/`
- Por projeto: `.bugbee/agents/`

```markdown title="~/.config/bugbee/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o bugbee usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do bugbee quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do bugbee usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Bugbee Zen](/docs/zen), você usaria `bugbee/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="bugbee.json" {3-6,9-12}
{
  "$schema": "https://github.com/neuralbroker/bugbee/config.json",
  "tools": {
    "write": true,
...
```json title="bugbee.json" {3-5,8-10}
{
  "$schema": "https://github.com/neuralbroker/bugbee/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `bugbee models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/neuralbroker/bugbee).
:::
...
```markdown title="~/.config/bugbee/agents/docs-writer.md"
---
...
```markdown title="~/.config/bugbee/agents/security-auditor.md"
---
