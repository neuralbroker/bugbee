...
description: O bugbee integra-se aos seus servidores LSP.
---

O bugbee pode se integrar a servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para o agente.

---

## Integrados

O bugbee vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `BUGBEE_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o LSP está habilitado e o bugbee abre um arquivo, ele:
...
Você pode habilitar e personalizar os servidores LSP através da seção `lsp` na sua configuração do bugbee.
