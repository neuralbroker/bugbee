...
Para depurar problemas com o bugbee, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\bugbee\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `bugbee --log-level DEBUG`.

---

## Armazenamento

O bugbee armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/bugbee/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\bugbee`
...
O bugbee Desktop executa um servidor bugbee local (o sidecar `bugbee-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `Bugbee` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/bugbee/bugbee.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\bugbee\bugbee.jsonc`
...
O bugbee também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/bugbee/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\bugbee\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o bugbee possa reconstruí-lo.

1. Saia completamente do bugbee Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/bugbee`
- **Linux**: exclua `~/.cache/bugbee` (ou execute `rm -rf ~/.cache/bugbee`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\bugbee`

3. Reinicie o bugbee Desktop.

---

### Corrigindo problemas de conexão com o servidor

O bugbee Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `bugbee.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `BUGBEE_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `BUGBEE_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o bugbee Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do bugbee.

---

### Notificações não aparecendo

O bugbee Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o bugbee nas configurações do seu sistema operacional, e
...
1. Saia do bugbee Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo bugbee Desktop):

- `bugbee.settings.dat` (URL do servidor padrão do desktop)
- `bugbee.global.dat` e `bugbee.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o bugbee:
...
3. Certifique-se de que você tem a versão mais recente com `bugbee upgrade`
...
Para descobrir quais modelos você tem acesso, execute `bugbee models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\bugbee`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O bugbee instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\bugbee`

2. Reinicie o bugbee para reinstalar os pacotes de provedores mais recentes

Isso forçará o bugbee a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O bugbee detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
