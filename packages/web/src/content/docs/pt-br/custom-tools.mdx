...
description: Crie ferramentas que o LLM pode chamar no bugbee.
---

Ferramentas personalizadas são funções que você cria e que o LLM pode chamar durante as conversas. Elas funcionam junto com as [ferramentas integradas](/docs/tools) do bugbee, como `read`, `write` e `bash`.
...
- Localmente, colocando-as no diretório `.bugbee/tools/` do seu projeto.
- Ou globalmente, colocando-as em `~/.config/bugbee/tools/`.
