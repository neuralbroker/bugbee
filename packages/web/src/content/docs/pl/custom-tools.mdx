...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) bugbee, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.bugbee/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/bugbee/tools/`.
