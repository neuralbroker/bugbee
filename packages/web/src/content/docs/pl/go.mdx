...
Bugbee Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w Bugbee. Subskrybujesz Bugbee Go i
...
Bugbee Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

Bugbee Go działa jak każdy inny dostawca w Bugbee.

1. Logujesz się do **<a href={console}>Bugbee Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `Bugbee Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Bugbee Go.
:::
...
Bugbee Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Bugbee
używa formatu `bugbee-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`bugbee-go/kimi-k3` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Bugbee.
