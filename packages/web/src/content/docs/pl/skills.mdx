...
Umiejętności agenta pozwalają bugbee odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
bugbee przeszukuje te lokalizacje:

- Project config: `.bugbee/skills/<name>/SKILL.md`
- Global config: `~/.config/bugbee/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu bugbee przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.bugbee/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/bugbee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.bugbee/skills/git-release/SKILL.md` w ten sposób:
...
bugbee wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `bugbee.json`:
...
**Dla agentów wbudowanych** (w `bugbee.json`):
