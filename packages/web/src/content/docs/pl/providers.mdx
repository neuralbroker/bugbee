---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

bugbee używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji bugbee.
...
w `~/.local/share/bugbee/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim bugbee
config.
...
Bugbee Zen to lista modeli dostarczonych przez zespół bugbee, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z bugbee. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Bugbee Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Bugbee Zen` i przejdź do [bugbee.dev/auth](https://bugbee.dev/zen).
...
Działa jak każdy inny dostawca w bugbee i jest całkowicie opcjonalny w użyciu.

---

## Bugbee Go

Bugbee Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół bugbee, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z bugbee.

1. Uruchom polecenie `/connect` w TUI, wybierz `Bugbee Go` i przejdź do [bugbee.dev/auth](https://bugbee.dev/zen).
...
Aby używać Amazon Bedrock z bugbee:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania bugbee:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `bugbee.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w bugbee nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować bugbee do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod bugbee działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji bugbee.
...
bugbee używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować bugbee
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`bugbee.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem bugbee:
...
Aby używać Google Vertex AI z bugbee:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania bugbee.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą bugbee, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session), aby automatycznie rejestrować każdą konwersację bugbee jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja bugbee będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować bugbee tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować bugbee tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować bugbee tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem bugbee. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/bugbee).
:::
...
Aby korzystać z Ollama Cloud z bugbee:
...
3. Skopiuj klucz API do użycia w bugbee.
...
6. **Ważne**: Przed użyciem modeli chmurowych w bugbee musisz pobrać informacje o modelu lokalnie:
...
Bugbee Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół bugbee. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Bugbee Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Bugbee Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API bugbee.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji bugbee.
...
5. Możesz je także dostosować za pomocą konfiguracji bugbee. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z bugbee:
...
Możesz także dostosować modele za pomocą konfiguracji bugbee. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z bugbee. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `bugbee.json` w katalogu projektu:
...
Pola `limit` pozwalają bugbee zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `bugbee auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację bugbee i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji bugbee.
