---
title: Serwery LSP
description: Bugbee integruje się z serwerami LSP.
---

Bugbee może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

Bugbee posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `BUGBEE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i Bugbee otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji Bugbee.
