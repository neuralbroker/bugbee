---
title: Formatery
description: Bugbee używa formaterów specyficznych dla języka.
---

Bugbee automatycznie formatuje pliki po ich zapisaniu lub edycji przy użyciu formaterów specyficznych dla języka. Zapewnia to, że wygenerowany kod jest zgodny ze stylem kodu Twojego projektu.

---

## Wbudowane

Bugbee zawiera kilka wbudowanych formaterów dla popularnych języków i frameworków. Poniższa tabela zawiera listę formaterów, obsługiwanych plików oraz wymagań konfiguracyjnych.
...
Jeśli więc Twój projekt zawiera `prettier` w `package.json`, Bugbee automatycznie go użyje.

---

## Jak to działa

Kiedy Bugbee zapisuje lub edytuje plik:
...
Możesz dostosować formatery za pomocą sekcji `formatter` w konfiguracji Bugbee.
