...
W Bugbee obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Bugbee zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Bugbee ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Bugbee ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Bugbee, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `bugbee.json`:
...
- Globalnie: `~/.config/bugbee/agents/`
- W projekcie: `.bugbee/agents/`
...
Jeśli nie zostanie skonfigurowana, Bugbee używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Bugbee, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Bugbee w `provider/model-id`. Na przykład, [Bugbee Zen](/docs/zen), użyj `bugbee/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `bugbee models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/neuralbroker/bugbee).
:::
