...
Dzięki bugbee możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie bugbee używa naszego własnego motywu `bugbee`.
...
bugbee ma kilka wbudowanych motywów.
...
- Chcą, aby Bugbee pasował do wyglądu ich terminala
...
bugbee obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/bugbee/themes/*.json` lub `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.bugbee/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.bugbee/themes/*.json`
