...
Możesz dodać dodatkowe informacje do bugbee za pomocą _Model Context Protocol_, w skrócie MCP. bugbee obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/bugbee`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
bugbee automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, bugbee:
...
Jeśli serwer wymaga uwierzytelnienia, bugbee poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `bugbee mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji bugbee bezpieczne przechowa tokeny w `~/.local/share/bugbee/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w bugbee, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie bugbee, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć bugbee z kontem Sentry.
