---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć bugbee.
---

Wtyczki umożliwiające wyłączenie bugbee poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania bugbee.
...
- `.bugbee/plugins/` - Wtyczki na poziomie projektu
- `~/.config/bugbee/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/bugbee/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/bugbee/bugbee.json`)
2. Konfiguracja projektu (`bugbee.json`)
3. Globalny katalog wtyczek (`~/.config/bugbee/plugins/`)
4. Katalog wtyczek projektu (`.bugbee/plugins/`)
...
bugbee uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z bugbee do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do bugbee.
...
:::note
Jeśli korzystasz z aplikacji komputerowej bugbee, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj bugbee czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do bugbee:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod bugbee. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://github.com/neuralbroker/bugbee/docs/sdk).
