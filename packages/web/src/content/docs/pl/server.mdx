---
title: Serwer
description: Interakcja z serwerem bugbee za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `bugbee serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient bugbee.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `bugbee.local` |
...
Ustaw `BUGBEE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `bugbee` lub ustaw `BUGBEE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `bugbee serve`, jak i `bugbee web`.
...
Kiedy uruchomisz `bugbee`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera bugbee do programowej interakcji z bugbee.
:::

Ta architektura umożliwia obsługę wielu klientów przez bugbee i programową interakcję z kodem otwartym.

Możesz uruchomić `bugbee serve`, aby uruchomić samodzielny serwer. Jeśli masz
bugbee TUI działa, `bugbee serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki bugbee [IDE](/docs/ide).
...
Serwer bugbee udostępnia następujące interfejsy API.
