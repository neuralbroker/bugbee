...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Bugbee posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Bugbee.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (lub `BUGBEE_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Bugbee lub gdy zmienna środowiskowa `BUGBEE_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Bugbee:
