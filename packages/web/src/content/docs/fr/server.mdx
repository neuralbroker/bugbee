---
title: Serveur
description: Interagissez avec le serveur bugbee via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `bugbee serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client bugbee peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `bugbee.local` |
...
Définissez `BUGBEE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `bugbee`, ou définissez `BUGBEE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `bugbee serve` et à `bugbee web`.
...
Lorsque vous exécutez `bugbee`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur bugbee pour interagir avec bugbee par programmation.
:::

Cette architecture permet à bugbee de prendre en charge plusieurs clients et vous permet d'interagir avec bugbee par programme.

Vous pouvez exécuter `bugbee serve` pour démarrer un serveur autonome. Si vous avez le
bugbee TUI en cours d'exécution, `bugbee serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Bugbee [IDE](/docs/ide).
...
Le serveur bugbee expose les API suivantes.
