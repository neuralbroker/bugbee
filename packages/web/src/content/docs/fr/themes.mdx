...
Avec Bugbee, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Bugbee utilise notre propre thème `bugbee`.
...
Bugbee est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Bugbee corresponde à l'apparence de leur terminal
...
Bugbee prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/bugbee/themes/*.json` ou `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.bugbee/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.bugbee/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/bugbee/themes
vim ~/.config/bugbee/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .bugbee/themes
vim .bugbee/themes/my-theme.json
```
...
{
  "$schema": "https://github.com/neuralbroker/bugbee/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
