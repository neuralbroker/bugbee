---
title: TUI
description: Utilisation de l’interface utilisateur du terminal Bugbee.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Bugbee fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de Bugbee démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le Bugbee TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à Bugbee. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `bugbee.json`, qui configure le comportement du serveur/d'exécution.
...
- `leader_timeout` - Contrôle la durée pendant laquelle Bugbee attend après la leader key. Valeur par défaut : `2000`.
...
- `sound_pack` - ID du sound pack à utiliser. Valeur par défaut : `bugbee.default`.
