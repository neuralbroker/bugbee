---
title: Configuration
description: Utilisation de la configuration JSON Bugbee.
---

Vous pouvez configurer Bugbee à l'aide d'un fichier de configuration JSON.

---

## Format

Bugbee prend en charge les formats **JSON** et **JSONC** (JSON avec commentaires).
...
1. **Configuration distante** (à partir de `.well-known/bugbee`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/bugbee/bugbee.json`) - préférences utilisateur
3. **Configuration personnalisée** (`BUGBEE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`bugbee.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.bugbee`** - agents, commandes, plugins
6. **Configuration en ligne** (`BUGBEE_CONFIG_CONTENT` env var) - remplacements d'exécution
...
:::note
Les répertoires `.bugbee` et `~/.config/bugbee` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/bugbee`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale Bugbee dans `~/.config/bugbee/bugbee.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/bugbee/tui.json`.
...
Ajoutez `bugbee.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Lorsque Bugbee démarre, il recherche un fichier de configuration dans le répertoire actuel ou remonte jusqu'au répertoire Git le plus proche.
...
Spécifiez un chemin de fichier de configuration personnalisé à l'aide de la variable d'environnement `BUGBEE_CONFIG`.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `BUGBEE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.bugbee`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.bugbee`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).

La configuration TUI utilise [**`bugbee.dev/tui.json`**](https://github.com/neuralbroker/bugbee/tui.json).
...
Utilisez `BUGBEE_TUI_CONFIG` pour pointer vers un fichier de configuration TUI personnalisé.

Les anciennes clés `theme`, `keybinds` et `tui` dans `bugbee.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `bugbee serve` et `bugbee web` via l'option `server`.
...
- `mdns` - Activer la découverte du service mDNS. Cela permet à d'autres appareils du réseau de découvrir votre serveur Bugbee.
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `bugbee.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez configurer les fournisseurs et les modèles que vous souhaitez utiliser dans votre configuration Bugbee via les options `provider`, `model` et `small_model`.
...
L'option `small_model` configure un modèle distinct pour les tâches légères comme la génération de titres. Par défaut, Bugbee essaie d'utiliser un modèle moins cher s'il est disponible auprès de votre fournisseur, sinon il revient à votre modèle principal.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/bugbee/agents/` ou `.bugbee/agents/`. [En savoir plus ici](/docs/agents).
...
L'agent par défaut doit être un agent principal (et non un sous-agent). Il peut s'agir d'un agent intégré tel que `"build"` ou `"plan"`, ou d'un [agent personnalisé](/docs/agents) que vous avez défini. Si l'agent spécifié n'existe pas ou est un sous-agent, Bugbee reviendra à `"build"` avec un avertissement.

Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`bugbee run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/bugbee/commands/` ou `.bugbee/commands/`. [En savoir plus ici](/docs/commands).
...
Bugbee téléchargera automatiquement toutes les nouvelles mises à jour au démarrage. Vous pouvez désactiver cela avec l'option `autoupdate`.
...
Par défaut, bugbee **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
[Plugins](/docs/plugins) étendent Bugbee avec des outils, des hooks et des intégrations personnalisés.

Placez les fichiers du plugin dans `.bugbee/plugins/` ou `~/.config/bugbee/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
...
Ceci est utile lorsque vous souhaitez restreindre Bugbee à l'utilisation de fournisseurs spécifiques plutôt que de les désactiver un par un.
