---
title: SDK
description: Client JS de type sécurisé pour le serveur bugbee.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK bugbee JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler bugbee par programme.
...
Créez une instance de bugbee :

```javascript
import { createBugbee } from "@bugbee-ai/sdk"

const { client } = await createBugbee()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `bugbee.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${bugbee.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de bugbee, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@bugbee-ai/sdk"
```
