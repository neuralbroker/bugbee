...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Bugbee.
...
Voici plusieurs modèles qui fonctionnent bien avec Bugbee, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Bugbee.
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Bugbee Zen](/docs/zen), vous utiliserez `bugbee/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="bugbee.jsonc" {7-12,19-24}
{
  "$schema": "https://github.com/neuralbroker/bugbee/config.json",
  "provider": {
    "openai": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Bugbee est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Bugbee est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
Lorsque Bugbee démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Bugbee.
