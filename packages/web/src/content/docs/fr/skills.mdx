...
Les compétences d'agent permettent à Bugbee de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Bugbee recherche ces emplacements :

- Configuration du projet : `.bugbee/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/bugbee/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Bugbee remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.bugbee/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/bugbee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.bugbee/skills/git-release/SKILL.md` comme ceci :
...
Bugbee répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `bugbee.json` :
...
**Pour les agents intégrés** (dans `bugbee.json`) :
