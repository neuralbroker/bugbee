...
Vous pouvez ajouter des outils externes à Bugbee à l'aide du _Model Context Protocol_ ou MCP. Bugbee prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/bugbee`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
Bugbee gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Bugbee :
...
Si le serveur nécessite une authentification, Bugbee vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `bugbee mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Bugbee stockera les jetons en toute sécurité dans `~/.local/share/bugbee/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Bugbee, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Bugbee comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Bugbee à votre compte Sentry.
