---
title: Plugins
description: Écrivez vos propres plugins pour étendre Bugbee.
---

Les plugins vous permettent d'étendre Bugbee en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Bugbee.
...
- `.bugbee/plugins/` - Plugins au niveau du projet
- `~/.config/bugbee/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/bugbee/node_modules/`.
...
1. Configuration globale (`~/.config/bugbee/bugbee.json`)
2. Configuration du projet (`bugbee.json`)
3. Répertoire global des plugins (`~/.config/bugbee/plugins/`)
4. Répertoire des plugins du projet (`.bugbee/plugins/`)
...
Bugbee exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK bugbee pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre bugbee.
...
:::note
Si vous utilisez l'application de bureau Bugbee, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez bugbee de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à bugbee :
...
L'assistant `tool` crée un outil personnalisé que bugbee peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour bugbee aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://github.com/neuralbroker/bugbee/docs/sdk) pour plus de détails.
