---
title: Config
description: Using the Bugbee JSON config.
---

You can configure Bugbee using a JSON config file.

---

## Format

Bugbee supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="bugbee.jsonc"
{
...
1. **Remote config** (from `.well-known/bugbee`) - organizational defaults
2. **Global config** (`~/.config/bugbee/bugbee.json`) - user preferences
3. **Custom config** (`BUGBEE_CONFIG` env var) - custom overrides
4. **Project config** (`bugbee.json` in project) - project-specific settings
5. **`.bugbee` directories** - agents, commands, plugins
6. **Inline config** (`BUGBEE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/bugbee/` on macOS) - admin-controlled
...
:::note
The `.bugbee` and `~/.config/bugbee` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/bugbee` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/bugbee"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Bugbee config in `~/.config/bugbee/bugbee.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/bugbee/tui.json`.
...
Add `bugbee.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Bugbee starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `BUGBEE_CONFIG` environment variable.

```bash
export BUGBEE_CONFIG=/path/to/my/custom-config.json
bugbee run "Hello world"
```
...
Specify a custom config directory using the `BUGBEE_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.bugbee` directory, and should
follow the same structure.

```bash
export BUGBEE_CONFIG_DIR=/path/to/my/config-directory
bugbee run "Hello world"
```

The custom directory is loaded after the global config and `.bugbee` directories, so it **can override** their settings.
...
Drop an `bugbee.json` or `bugbee.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/bugbee/` |
| Linux    | `/etc/bugbee/`                         |
| Windows  | `%ProgramData%\bugbee`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Bugbee reads managed preferences from the `ai.bugbee.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Bugbee checks these paths:

1. `/Library/Managed Preferences/<user>/ai.bugbee.managed.plist`
2. `/Library/Managed Preferences/ai.bugbee.managed.plist`

The plist keys map directly to `bugbee.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.bugbee.managed` PayloadType. The Bugbee config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.bugbee.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.bugbee.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.bugbee</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).

TUI config uses [**`bugbee.dev/tui.json`**](https://github.com/neuralbroker/bugbee/tui.json).
...
Use `BUGBEE_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `bugbee.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `bugbee serve` and `bugbee web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Bugbee server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `bugbee.local`. Useful for running multiple instances on the same network.
...
If not specified, Bugbee will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Bugbee config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Bugbee tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Bugbee actions on configured resources. Currently, policies can control which providers Bugbee may use.
...
Bugbee normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Bugbee omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/bugbee/agents/` or `.bugbee/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Bugbee will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`bugbee run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/bugbee/commands/` or `.bugbee/commands/`. [Learn more here](/docs/commands).
...
Bugbee uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Bugbee will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, bugbee **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Bugbee with custom tools, hooks, and integrations.

Place plugin files in `.bugbee/plugins/` or `~/.config/bugbee/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="bugbee.json"
{
  "$schema": "https://github.com/neuralbroker/bugbee/config.json",
  "plugin": ["bugbee-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Bugbee to only use specific providers rather than disabling them one by one.
