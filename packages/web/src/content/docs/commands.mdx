...
Create `.bugbee/commands/test.md`:
...
You can add custom commands through the Bugbee config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Bugbee [config](/docs/config):
...
bugbee includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
