---
title: Server
description: Interact with bugbee server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `bugbee serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an bugbee client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `bugbee.local` |
...
Set `BUGBEE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `bugbee`, or set `BUGBEE_SERVER_USERNAME` to override it. This applies to both `bugbee serve` and `bugbee web`.
...
When you run `bugbee` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the bugbee server to interact with bugbee programmatically.
:::

This architecture lets bugbee support multiple clients and allows you to interact with bugbee programmatically.

You can run `bugbee serve` to start a standalone server. If you have the
bugbee TUI running, `bugbee serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Bugbee [IDE](/docs/ide) plugins.
...
The bugbee server exposes the following APIs.
