...
You can add external tools to Bugbee using the _Model Context Protocol_, or MCP. Bugbee supports both local and remote servers.
...
You can define MCP servers in your [Bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/bugbee` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Bugbee automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Bugbee will:
...
If the server requires authentication, Bugbee will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `bugbee mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Bugbee will store the tokens securely in `~/.local/share/bugbee/mcp-auth.json`.
...
Your MCPs are available as tools in Bugbee, alongside built-in tools. So you can manage them through the Bugbee config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Bugbee to your Sentry account.
