---
title: プラグイン
description: Bugbee を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Bugbee を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Bugbee のデフォルトの動作を変更したりできます。
...
- `.bugbee/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/bugbee/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/bugbee/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/bugbee/bugbee.json`)
2. プロジェクト設定 (`bugbee.json`)
3. グローバルプラグインディレクトリ (`~/.config/bugbee/plugins/`)
4. プロジェクトプラグインディレクトリ (`.bugbee/plugins/`)
...
Bugbee は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Bugbee SDK クライアント。
...
ここでは、Bugbee を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://github.com/neuralbroker/bugbee/docs/sdk) を参照してください。
...
Bugbee が `.env` ファイルを読み取らないようにします。
...
プラグインは Bugbee にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Bugbee が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Bugbee で使用できます。
