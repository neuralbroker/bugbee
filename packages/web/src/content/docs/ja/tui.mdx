---
title: TUI
description: Bugbee ターミナルユーザーインターフェイスの使用。
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Bugbee は、LLM を使用してプロジェクトを作業するための対話型ターミナルインターフェイスまたは TUI を提供します。

Bugbee を実行すると、現在のディレクトリの TUI が開始されます。
...
Bugbee TUI を使用する場合、「`/`」に続いてコマンド名を入力すると、アクションをすばやく実行できます。例えば：
...
Bugbee にプロバイダーを追加します。利用可能なプロバイダーから選択し、その API キーを追加できます。
...
これは、サーバー/ランタイムの動作を構成する `bugbee.json` とは別です。
...
- `leader_timeout` - leader key を押したあと、Bugbee が次のキー入力を待つ時間を制御します。デフォルトは `2000` です。
...
カスタム TUI 設定パスをロードするには、`BUGBEE_TUI_CONFIG` を使用します。

### Attention

Attention を有効にすると、Bugbee が回答待ち、権限の承認待ち、セッションエラー、またはセッション完了を知らせたいときに、TUI がサウンドやデスクトップ通知で知らせます。`attention.enabled` で有効化します。組み込みイベントは発生時にサウンドを再生します。デスクトップ通知はターミナルウィンドウにフォーカスがない場合にのみ送信され、subagent イベントには使われません。
...
- `sound_pack` - 使用する sound pack ID です。デフォルトは `bugbee.default` です。
