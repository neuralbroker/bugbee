...
_Model Context Protocol_ (MCP) を使用して、Bugbee に外部ツールを追加できます。 Bugbee は、ローカルサーバーとリモートサーバーの両方をサポートします。
...
MCP サーバーは、`mcp` の下の [Bugbee 設定](https://github.com/neuralbroker/bugbee/docs/config/) で定義できます。各 MCP を一意の名前で追加します。 LLM にプロンプ​​トを表示するときに、その MCP を名前で参照できます。
...
組織は、`.well-known/bugbee` エンドポイント経由でデフォルトの MCP サーバーを提供できます。これらのサーバーはデフォルトで無効になっている場合があり、ユーザーは必要なサーバーにオプトインできます。
...
Bugbee は、リモート MCP サーバーの OAuth 認証を自動的に処理します。サーバーが認証を必要とする場合、Bugbee は次のことを行います。
...
サーバーが認証を必要とする場合、Bugbee を初めて使用しようとすると、認証を求めるプロンプトが表示されます。そうでない場合は、`bugbee mcp auth <server-name>` を使用して [flow](#authenticating) を手動でトリガーできます。
...
`mcp auth` コマンドは、認証のためにブラウザを開きます。承認後、Bugbee はトークンを `~/.local/share/bugbee/mcp-auth.json` に安全に保存します。
...
MCP は、組み込みツールと並んで、Bugbee のツールとして利用できます。したがって、他のツールと同様に、Bugbee 設定を通じてそれらを管理できます。
...
これにより、ブラウザウィンドウが開き、OAuth フローが完了し、Bugbee が Sentry アカウントに接続されます。
