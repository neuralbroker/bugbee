---
title: サーバー
description: HTTP 経由で Bugbee サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`bugbee serve` コマンドは、Bugbee クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `bugbee.local` |
...
HTTP 基本認証でサーバーを保護するには、`BUGBEE_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `bugbee` になるか、`BUGBEE_SERVER_USERNAME` を設定してオーバーライドします。これは、`bugbee serve` と `bugbee web` の両方に当てはまります。
...
`bugbee` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Bugbee サーバーを使用して、プログラムで Bugbee と対話します。
:::

`bugbee serve` を実行してスタンドアロンサーバーを起動できます。すでに Bugbee TUI を実行している場合でも、`bugbee serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Bugbee [IDE](/docs/ide) プラグインによって使用されます。
