...
Создайте `.bugbee/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию bugbee или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем bugbee [config](/docs/config):
...
- Глобальный: `~/.config/bugbee/commands/`
- Для каждого проекта: `.bugbee/commands/`
...
bugbee включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
