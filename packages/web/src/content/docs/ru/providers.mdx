---
title: Провайдеры
description: Использование любого провайдера LLM в bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

bugbee использует [AI SDK](https://ai-sdk.dev/) и [Models.dev](https://models.dev) для поддержки **более 75 поставщиков LLM** и поддерживает запуск локальных моделей.

Чтобы добавить провайдера, вам необходимо:

1. Добавьте ключи API для провайдера с помощью команды `/connect`.
2. Настройте провайдера в вашей конфигурации bugbee.
...
в `~/.local/share/bugbee/auth.json`.

---

### Настройка

Вы можете настроить поставщиков через раздел `provider` в вашем bugbee.
конфиг.
...
Bugbee Zen — это список моделей, предоставленный командой bugbee, которые были
протестировано и проверено на хорошую работу с bugbee. [Подробнее](/docs/zen).

:::tip
Если вы новичок, мы рекомендуем начать с Bugbee Zen.
:::

1. Запустите команду `/connect` в TUI, выберите `Bugbee Zen` и перейдите по адресу [bugbee.dev/auth](https://bugbee.dev/zen).
...
Он работает как любой другой поставщик в bugbee и его использование совершенно необязательно.

---

## Bugbee Go

Bugbee Go — это недорогой план подписки, обеспечивающий надежный доступ к популярным открытым моделям кодирования, предоставляемым командой bugbee, которые были
протестированы и проверены на хорошую работу с bugbee.

1. Запустите команду `/connect` в TUI, выберите `Bugbee Go` и перейдите по адресу [bugbee.dev/auth](https://bugbee.dev/zen).
...
Чтобы использовать Amazon Bedrock с bugbee:
...
2. **Настройте аутентификацию** одним из следующих способов:
   ...
   Установите одну из этих переменных среды при запуске bugbee:
   ...
   Для конкретной или постоянной конфигурации проекта используйте `bugbee.json`:
...
:::info
Использование вашей подписки Claude Pro/Max в bugbee официально не поддерживается [Anthropic](https://anthropic.com).
:::
...
Вы можете настроить bugbee для работы с локальными моделями через [Atomic Chat](https://atomic.chat) — десктопное приложение, которое запускает локальные LLM за OpenAI-совместимым API-сервером (конечная точка по умолчанию `http://127.0.0.1:1337/v1`).
...
2. Перейдите в [Azure AI Foundry](https://ai.azure.com/) и разверните модель.
   ...
   :::примечание
   Для правильной работы bugbee имя развертывания должно совпадать с именем модели.
   :::
...
5. Запустите команду `/models`, чтобы выбрать модель.
   ...
   Вы также можете добавлять модели через конфигурацию bugbee.
...
2. Выберите метод аутентификации:
   ...
   3. Имя: `Bugbee`, Области применения: `api`
...
для хранения токена в хранилище аутентификации bugbee.
:::

##### Самостоятельная GitLab

:::note[примечание о соответствии]
bugbee использует небольшую модель для некоторых задач ИИ, таких как создание заголовка сеанса.
По умолчанию он настроен на использование gpt-5-nano, размещенного на Zen. Чтобы заблокировать bugbee
чтобы использовать только свой собственный экземпляр, размещенный на GitLab, добавьте следующее в свой
`bugbee.json` файл. Также рекомендуется отключить совместное использование сеансов.
...
Дополнительная документация на домашней странице [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth).

##### Конфигурация

Настройте через `bugbee.json`:
...
Чтобы использовать Google Vertex AI с bugbee:
...
2. Установите необходимые переменные среды:
   ...
   Установите их во время запуска bugbee.
...
Если вы видите функцию или модель от Helicone, которая не настраивается автоматически через bugbee, вы всегда можете настроить ее самостоятельно.
...
Функция Helicone [Sessions](https://docs.helicone.ai/features/sessions) позволяет группировать связанные запросы LLM вместе. Используйте плагин [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session), чтобы автоматически регистрировать каждый диалог bugbee как сеанс в Helicone.
...
Плагин вставляет в ваши запросы заголовки `Helicone-Session-Id` и `Helicone-Session-Name`. На странице «Сеансы» Helicone вы увидите каждый диалог bugbee, указанный как отдельный сеанс.
...
Вы можете настроить bugbee для использования локальных моделей с помощью [утилиты llama-server llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Вы можете настроить bugbee для использования локальных моделей через LM Studio.
...
Вы можете настроить bugbee для использования локальных моделей через Ollama.

:::tip
Ollama может автоматически настроиться для bugbee. Подробности см. в документации по интеграции Ollama](https://docs.ollama.com/integrations/bugbee).
:::
...
Чтобы использовать Ollama Cloud с bugbee:
...
3. Скопируйте ключ API для использования в bugbee.
...
6. **Важно**. Перед использованием облачных моделей в bugbee необходимо получить информацию о модели локально:
...
Bugbee Zen — это список протестированных и проверенных моделей, предоставленный командой bugbee. [Подробнее](/docs/zen).

1. Войдите в систему **<a href={console}>Bugbee Zen</a>** и нажмите **Создать ключ API**.

2. Запустите команду `/connect` и найдите **Bugbee Zen**.

   ```txt
   /connect
   ```

3. Введите свой ключ API bugbee.
...
4. Многие модели OpenRouter предварительно загружены по умолчанию. Запустите команду `/models`, чтобы выбрать нужную.
   ...
   Вы также можете добавить дополнительные модели через конфигурацию bugbee.
...
5. Вы также можете настроить их через конфигурацию bugbee. Вот пример указания провайдера
...
Чтобы использовать [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) с bugbee:
...
Вы также можете настраивать модели через конфигурацию bugbee. Ниже приведен пример указания порядка маршрутизации поставщика.
...
4. Создайте или обновите файл `bugbee.json` в каталоге вашего проекта:
...
Поля `limit` позволяют bugbee понять, сколько контекста у вас осталось. Стандартные поставщики автоматически извлекают их из models.dev.
...
1. **Проверьте настройку аутентификации**: запустите `bugbee auth list`, чтобы проверить, верны ли учетные данные.
...
2. Для пользовательских поставщиков проверьте конфигурацию bugbee и:
   - Убедитесь, что идентификатор провайдера, используемый в команде `/connect`, соответствует идентификатору в вашей конфигурации bugbee.
