---
title: LSP-серверы
description: bugbee интегрируется с вашими серверами LSP.
---

bugbee может интегрироваться с серверами Language Server Protocol (LSP), чтобы использовать диагностику как обратную связь для агента.

---

## Встроенные

bugbee поставляется с несколькими встроенными LSP-серверами для популярных языков:
...
:::note
Вы можете отключить автоматическую загрузку LSP-сервера, установив для переменной среды `BUGBEE_DISABLE_LSP_DOWNLOAD` значение `true`.
:::
...
Когда LSP включен и bugbee открывает файл, он:
...
Вы можете включить и настроить серверы LSP через раздел `lsp` в конфигурации bugbee.
