...
Вы можете добавить внешние инструменты в bugbee, используя _Model Context Protocol_ или MCP. bugbee поддерживает как локальные, так и удаленные серверы.
...
Вы можете определить серверы MCP в своем [bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
...
Организации могут предоставлять серверы MCP по умолчанию через свою конечную точку `.well-known/bugbee`. Эти серверы могут быть отключены по умолчанию, что позволяет пользователям выбирать те, которые им нужны.
...
bugbee автоматически обрабатывает аутентификацию OAuth для удаленных серверов MCP. Когда серверу требуется аутентификация, bugbee:
...
Если сервер требует аутентификации, bugbee предложит вам пройти аутентификацию при первой попытке его использования. Если нет, вы можете [вручную запустить поток ](#authenticating) с помощью `bugbee mcp auth <server-name>`.
...
Команда `mcp auth` откроет ваш браузер для авторизации. После того как вы авторизуетесь, bugbee надежно сохранит токены в `~/.local/share/bugbee/mcp-auth.json`.
...
Ваши MCP доступны в виде инструментов bugbee наряду со встроенными инструментами. Таким образом, вы можете управлять ими через конфигурацию bugbee, как и любым другим инструментом.
...
Откроется окно браузера для завершения процесса OAuth и подключения bugbee к вашей учетной записи Sentry.
