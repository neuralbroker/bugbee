...
Вы можете определить серверы MCP в своем [bugbee Config](https://bugbee.dev/docs/config/) в разделе `mcp`. Добавьте каждому MCP уникальное имя. Вы можете обратиться к этому MCP по имени при запросе LLM.
