---
title: Плагины
description: Напишите свои собственные плагины для расширения bugbee.
---

Плагины позволяют расширять bugbee, подключаясь к различным событиям и настраивая поведение. Вы можете создавать плагины для добавления новых функций, интеграции с внешними сервисами или изменения поведения bugbee по умолчанию.
...
- `.bugbee/plugins/` – плагины уровня проекта.
- `~/.config/bugbee/plugins/` — глобальные плагины
...
**Плагины npm** устанавливаются автоматически с помощью Bun при запуске. Пакеты и их зависимости кэшируются в `~/.cache/bugbee/node_modules/`.
...
1. Глобальная конфигурация (`~/.config/bugbee/bugbee.json`)
2. Конфигурация проекта (`bugbee.json`)
3. Глобальный каталог плагинов (`~/.config/bugbee/plugins/`)
4. Каталог плагинов проекта (`.bugbee/plugins/`)
...
bugbee запускает `bun install` при запуске для их установки. Затем ваши плагины и инструменты смогут импортировать их.
...
Вот несколько примеров плагинов, которые вы можете использовать для расширения bugbee.
...
:::note
Если вы используете настольное приложение bugbee, оно может автоматически отправлять системные уведомления, когда ответ готов или когда возникает ошибка сеанса.
:::
...
Плагины также могут добавлять в bugbee собственные инструменты:
...
Помощник `tool` создает собственный инструмент, который может вызывать bugbee. Он принимает функцию схемы Zod и возвращает определение инструмента:
...
Уровни: `debug`, `info`, `warn`, `error`. Подробности см. в документации SDK](https://github.com/neuralbroker/bugbee/docs/sdk).
