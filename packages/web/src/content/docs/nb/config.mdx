---
title: Konfigurasjon
description: Ved å bruke bugbee JSON-konfigurasjonen.
---

Du kan konfigurere Bugbee ved å bruke en JSON konfigurasjonsfil.

---

## Format

Bugbee støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "bugbee"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/bugbee`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/bugbee/bugbee.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`BUGBEE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`bugbee.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.bugbee` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`BUGBEE_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.bugbee` og `~/.config/bugbee` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/bugbee`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Bugbee-konfigurasjon i `~/.config/bugbee/bugbee.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `bugbee.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Bugbee starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `BUGBEE_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `BUGBEE_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.bugbee` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.bugbee`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `bugbee serve` og `bugbee web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Bugbee-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `bugbee.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Bugbee-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Bugbee å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/bugbee/agents/` eller `.bugbee/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Bugbee falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`bugbee run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/bugbee/commands/` eller `.bugbee/commands/`. [Les mer her](/docs/commands).
...
Bugbee vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Bugbee alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Bugbee med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.bugbee/plugins/` eller `~/.config/bugbee/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Bugbee til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
