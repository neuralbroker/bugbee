...
description: Skriv dine egne programtillegg for å utvide Bugbee.
---

Plugins lar deg utvide Bugbee ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Bugbee.
...
- `.bugbee/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/bugbee/node_modules/`.
...
1. Global konfigurasjon (`~/.config/bugbee/bugbee.json`)
2. Prosjektkonfigurasjon (`bugbee.json`)
3. Global plugin-katalog (`~/.config/bugbee/plugins/`)
4. Prosjektpluginkatalog (`.bugbee/plugins/`)
...
Bugbee kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Bugbee SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Bugbee.
...
:::note
Hvis du bruker Bugbee-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Bugbee fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Bugbee:
...
`tool`-hjelperen lager et tilpasset verktøy som Bugbee kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Bugbee sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://github.com/neuralbroker/bugbee/docs/sdk) for detaljer.
