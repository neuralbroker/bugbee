---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

Bugbee bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Bugbee-konfigurasjonen.
...
i `~/.local/share/bugbee/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Bugbee
konfig.
...
Bugbee Zen er en liste over modeller levert av Bugbee-teamet som har vært
testet og verifisert for å fungere godt med Bugbee. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Bugbee Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Bugbee Zen` og gå til [bugbee.dev/auth](https://bugbee.dev/zen).
...
Det fungerer som alle andre leverandører i Bugbee og er helt valgfritt å bruke.

---

## Bugbee Go

Bugbee Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Bugbee-teamet som har vært
testet og verifisert for å fungere godt med Bugbee.

1. Kjør kommandoen `/connect` i TUI, velg `Bugbee Go`, og gå til [bugbee.dev/auth](https://bugbee.dev/zen).
...
Slik bruker du Amazon Bedrock med Bugbee:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Bugbee:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `bugbee.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i bugbee støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere bugbee til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Bugbee skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Bugbee-konfigurasjonen.
...
for å lagre token i Bugbee auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Bugbee bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Bugbee
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`bugbee.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `bugbee.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Bugbee:
...
Slik bruker du Google Vertex AI med Bugbee:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Bugbee.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Bugbee, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) plugin for automatisk å logge hver Bugbee-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Bugbee-samtale oppført som en separat økt.
...
Du kan konfigurere Bugbee for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Bugbee for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Bugbee for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Bugbee. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/bugbee) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Bugbee:
...
3. Kopier API-nøkkelen for bruk i Bugbee.
...
6. **Viktig**: Før du bruker skymodeller i Bugbee, må du hente modellinformasjonen lokalt:
...
Bugbee Zen er en liste over testede og verifiserte modeller levert av Bugbee-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Bugbee Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Bugbee Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Bugbee API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Bugbee-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Bugbee-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Bugbee:
...
Du kan også tilpasse modeller gjennom Bugbee-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Bugbee. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `bugbee.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Bugbee forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `bugbee auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Bugbee-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Bugbee-konfigurasjonen.
