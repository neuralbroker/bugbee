...
Verktøy lar LLM utføre handlinger i kodebasen din. Bugbee kommer med et sett med innebygde verktøy, men du kan utvide det med [egendefinerte verktøy](/docs/custom-tools) eller [MCP servere](/docs/mcp-servers).
...
Her er alle de innebygde verktøyene som er tilgjengelige i Bugbee.
...
:::note
Dette verktøyet er bare tilgjengelig når `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (eller `BUGBEE_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker Bugbee-leverandøren eller når miljøvariabelen `BUGBEE_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

Slik aktiverer du når du starter Bugbee:
