...
Du kan legge til eksterne verktøy i Bugbee ved å bruke _Model Context Protocol_ eller MCP. Bugbee støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="bugbee.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/bugbee`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Bugbee håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Bugbee:
...
Hvis serveren krever autentisering, vil Bugbee be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `bugbee mcp auth <server-name>`.
...
```bash
bugbee mcp auth my-oauth-server
```
...
```bash
bugbee mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Bugbee lagre tokens sikkert i `~/.local/share/bugbee/mcp-auth.json`.
...
bugbee mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Bugbee, sammen med innebygde verktøy. Så du kan administrere dem gjennom Bugbee-konfigurasjonen som alle andre verktøy.
...
```json title="bugbee.json" {11, 14-18}
{
  "$schema": "https://github.com/neuralbroker/bugbee/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
bugbee mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Bugbee til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Bugbee-samtale oppført som en separat økt.
