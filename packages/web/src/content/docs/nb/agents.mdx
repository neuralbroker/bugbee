...
Det er to typer agenter i Bugbee; primære agenter og underagenter.
...
Bugbee kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Bugbee kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Bugbee kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Bugbee sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `bugbee.json`:
...
- Globalt: `~/.config/bugbee/agents/`
- Per prosjekt: `.bugbee/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Bugbee modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Bugbee-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Bugbee-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Bugbee Zen](/docs/zen), vil du bruke `bugbee/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `bugbee models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/neuralbroker/bugbee).
:::
