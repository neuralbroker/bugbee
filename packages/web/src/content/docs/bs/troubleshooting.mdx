...
Da biste otklonili probleme s Bugbee, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\bugbee\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `bugbee --log-level DEBUG`.

---

## Pohrana

Bugbee pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/bugbee/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\bugbee`
...
Bugbee Desktop pokreće lokalni Bugbee server (`bugbee-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Bugbee` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/bugbee/bugbee.jsonc` (ili `~/.config/bugbee/bugbee.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/bugbee/bugbee.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\bugbee\bugbee.jsonc`
...
Bugbee također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/bugbee/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\bugbee\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Bugbee mogao ponovo izgraditi.

1. Potpuno zatvorite Bugbee Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/bugbee`
- **Linux**: obrišite `~/.cache/bugbee` (ili pokrenite `rm -rf ~/.cache/bugbee`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\bugbee`

3. Ponovo pokrenite Bugbee Desktop.

---

### Rješavanje problema sa vezom na serveru

Bugbee Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `bugbee.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `BUGBEE_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `BUGBEE_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Bugbee Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Bugbee karakteristikama.

---

### Obavještenja se ne prikazuju

Bugbee Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Bugbee u postavkama vašeg OS-a, i
...
1. Zatvorite Bugbee Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Bugbee Desktop aplikacije):

- `bugbee.settings.dat` (URL zadanog servera za desktop)
- `bugbee.global.dat` i `bugbee.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `bugbee models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\bugbee`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Bugbee dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\bugbee`

2. Ponovo pokrenite Bugbee da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Bugbee da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Bugbee će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
