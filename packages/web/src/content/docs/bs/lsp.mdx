---
title: LSP serveri
description: Bugbee se integriše sa vašim LSP serverima.
---

Bugbee se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Bugbee dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `BUGBEE_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i bugbee otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj bugbee konfiguraciji.
