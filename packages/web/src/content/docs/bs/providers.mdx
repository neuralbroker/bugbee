---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Bugbee.
---

import config from "../../../../config.mjs"
export const console = config.console

Bugbee koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Bugbee konfiguraciji.
...
u `~/.local/share/bugbee/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Bugbee
config.
...
Bugbee Zen je lista modela koje je obezbedio Bugbee tim koji su bili
testirano i potvrđeno da dobro radi sa Bugbee. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Bugbee Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite bugbee i idite na [bugbee.dev/auth](https://bugbee.dev/auth).
...
Radi kao i svaki drugi provajder u Bugbee i potpuno je opcionalan za korištenje.

---

## Bugbee Go

Bugbee Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Bugbee tim i koji su testirani i verificirani da dobro rade s Bugbee-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Bugbee Go` i idite na [bugbee.dev/auth](https://bugbee.dev/zen).
...
Da biste koristili Amazon Bedrock s Bugbee:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete bugbee:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `bugbee.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Bugbee.
:::
...
Možete konfigurirati bugbee za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi bugbee ispravno radio.
   :::
...
Također možete dodati modele kroz svoju bugbee konfiguraciju.
...
da pohrani token u bugbee auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Bugbee koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Bugbee
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`bugbee.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [bugbee-gitlab-auth](https://www.npmjs.com/package/bugbee-gitlab-auth).

##### Konfiguracija

Prilagodite putem `bugbee.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s bugbee:
...
Za korištenje Google Vertex AI s Bugbee:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće bugbee.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem bugbeea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [bugbee-helicone-session](https://github.com/H2Shami/bugbee-helicone-session) da automatski prijavite svaki Bugbee razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Bugbee razgovor naveden kao zasebna sesija.
...
Možete konfigurirati bugbee za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati bugbee za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati bugbee za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Bugbee. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/bugbee) za detalje.
:::
...
Da biste koristili Ollama Cloud s Bugbee:
...
3. Kopirajte API ključ za korištenje u Bugbee.
...
6. **Važno**: Prije upotrebe modela oblaka u Bugbee, morate lokalno povući informacije o modelu:
...
Bugbee Zen je lista testiranih i verifikovanih modela koju je obezbedio Bugbee tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Bugbee Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Bugbee Zen**.

```txt
   /connect
```

3. Unesite svoj Bugbee API ključ.
...
Također možete dodati dodatne modele putem vaše bugbee konfiguracije.
...
5. Također ih možete prilagoditi putem vaše bugbee konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Bugbeeom:
...
Također možete prilagoditi modele kroz svoju bugbee konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s bugbee-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `bugbee.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Bugbee da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `bugbee auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Bugbee konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj bugbee konfiguraciji.
