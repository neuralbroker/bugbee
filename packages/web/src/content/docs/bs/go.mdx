...
Bugbee Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u Bugbee-u. Pretplatite se na Bugbee Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Bugbee.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa Bugbee-om.
:::
...
Bugbee Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Bugbee Go radi kao bilo koji drugi provajder u Bugbee-u.

1. Prijavite se na **<a href={console}>Bugbee Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Bugbee Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Bugbee Go.
:::
...
Bugbee Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Bugbee konfiguraciji
koristi format `bugbee-go/<model-id>`. Na primjer, za Kimi K3, koristili biste
`bugbee-go/kimi-k3` u svojoj konfiguraciji.
...
Napravili smo Bugbee Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Bugbee koristite i bilo kojeg drugog provajdera.
