...
Agent skills omogucavaju Bugbee da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Bugbee pretrazuje ove lokacije:

- Konfiguracija projekta: `.bugbee/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/bugbee/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Bugbee ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.bugbee/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/bugbee/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.bugbee/skills/git-release/SKILL.md` ovako:
...
Bugbee navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `bugbee.json`:
...
**Za ugradene agente** (u `bugbee.json`):
