...
Možete dodati vanjske alate u Bugbee koristeći _Model Context Protocol_, ili MCP. Bugbee podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Bugbee Config](https://github.com/neuralbroker/bugbee/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/bugbee`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Bugbee automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Bugbee će:
...
Ako server zahtijeva autentifikaciju, Bugbee će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `bugbee mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Bugbee sigurno čuva tokene u `~/.local/share/bugbee/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Bugbee, zajedno s ugrađenim alatima. Možete njima upravljati kroz Bugbee konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Bugbee sa vašim Sentry nalogom.
