---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Bugbee.
---

Dodaci vam omogućavaju da proširite Bugbee spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Bugbee.
...
- `.bugbee/plugins/` - Dodaci na nivou projekta
- `~/.config/bugbee/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/bugbee/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/bugbee/bugbee.json`)
2. Konfiguracija projekta (`bugbee.json`)
3. Globalni direktorij dodataka (`~/.config/bugbee/plugins/`)
4. Direktorij dodataka projekta (`.bugbee/plugins/`)
...
Bugbee pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Bugbee SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Bugbee.
...
Spriječite bugbee da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u bugbee:
...
Pomoćnik `tool` kreira prilagođeni alat koji bugbee može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za bugbee zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://github.com/neuralbroker/bugbee/docs/sdk) za detalje.
