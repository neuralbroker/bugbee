---
title: Server
description: Komunicirajte s bugbee serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `bugbee serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji bugbee klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `bugbee.local` |
...
Postavite `BUGBEE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `bugbee`, ili postavite `BUGBEE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `bugbee serve` i za `bugbee web`.
...
Kada pokrenete `bugbee`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite bugbee server za programsku interakciju sa bugbee-om.
:::

Ova arhitektura omogucava bugbee podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `bugbee serve` da startate standalone server. Ako je bugbee TUI vec pokrenut, `bugbee serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Bugbee [IDE](/docs/ide) pluginovi.
...
bugbee server izlaže sljedece API-je.
