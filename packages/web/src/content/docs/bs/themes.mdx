...
U Bugbee mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Bugbee koristi nasu `bugbee` temu.
...
Bugbee dolazi sa vise ugradenih tema.
...
- Zele da Bugbee odgovara izgledu njihovog terminala
...
Bugbee podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/bugbee/themes/*.json` ili `$XDG_CONFIG_HOME/bugbee/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.bugbee/themes/*.json`
4. **Trenutni radni direktorij** - `./.bugbee/themes/*.json`
