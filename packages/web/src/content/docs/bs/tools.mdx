...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Bugbee dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Bugbee.
...
:::note
Ovaj alat je dostupan samo kada je `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (ili `BUGBEE_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Bugbee provajdera ili kada je varijabla `BUGBEE_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Bugbee:
