---
title: Konfiguracija
description: Korištenje Bugbee JSON konfiguracije.
---

Možete konfigurirati Bugbee koristeći JSON konfiguracijski fajl.

---

## Format

Bugbee podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/bugbee`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/bugbee/bugbee.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`BUGBEE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`bugbee.json` u projektu) - postavke specifične za projekat
5. **`.bugbee` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`BUGBEE_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.bugbee` i `~/.config/bugbee` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/bugbee` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Bugbee konfiguraciju u `~/.config/bugbee/bugbee.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/bugbee/tui.json`.
...
Dodajte `bugbee.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Bugbee pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `BUGBEE_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `BUGBEE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.bugbee` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.bugbee`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`bugbee.dev/config.json`**](https://github.com/neuralbroker/bugbee/config.json).

TUI konfiguracija koristi [**`bugbee.dev/tui.json`**](https://github.com/neuralbroker/bugbee/tui.json).
...
Koristite `BUGBEE_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `bugbee.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `bugbee serve` i `bugbee web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Bugbee server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `bugbee.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Bugbee konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Bugbee pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/bugbee/agents/` ili `.bugbee/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Bugbee će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`bugbee run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/bugbee/commands/` ili `.bugbee/commands/`. [Saznajte više ovdje](/docs/commands).
...
Bugbee će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Bugbee **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Bugbee sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.bugbee/plugins/` ili `~/.config/bugbee/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Bugbee da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
