...
Tools allow the LLM to perform actions in your codebase. Bugbee comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Bugbee.
...
:::note
This tool is only available when `BUGBEE_EXPERIMENTAL_LSP_TOOL=true` (or `BUGBEE_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Bugbee provider or when the `BUGBEE_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Bugbee:
