...
import { createBugbee, type ToolPart } from "@bugbee-ai/sdk"
...
console.log("🚀 Starting bugbee server...")
const bugbee = await createBugbee({
  port: 0,
})
console.log("✅ Bugbee server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await bugbee.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new bugbee session...")
    const { client, server } = bugbee
    ...
    console.log("✅ Created bugbee session:", createResult.data.id)
  ...
  console.log("📝 Sending to bugbee:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Bugbee response:", JSON.stringify(result, null, 2))
