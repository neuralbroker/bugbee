...
import { type Session } from "@bugbee-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("bugbee://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("bugbee://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("bugbee://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("bugbee://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("bugbee://open-project")).toBeUndefined()
    expect(parseDeepLink("bugbee://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "bugbee://open-project?directory=/a",
      "bugbee://other?directory=/b",
      "bugbee://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("bugbee://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("bugbee://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("bugbee://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("bugbee://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "bugbee://new-session?directory=/a",
      "bugbee://open-project?directory=/b",
      "bugbee://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __BUGBEE__: {
        deepLinks: ["bugbee://open-project?directory=/a"],
      },
    } as unknown as Window & { __BUGBEE__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["bugbee://open-project?directory=/a"])
