...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Bugbee needs attention", () => {
    expect(wslBugbeeAction(undefined)).toBeUndefined()
    expect(
      wslBugbeeAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    expect(
      wslBugbeeAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/bugbee",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when Bugbee is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        bugbeeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.bugbee/bin/bugbee",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installBugbee" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-bugbee")
  })
