...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-bugbee" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslBugbeeAction(check?: WslBugbeeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install Bugbee"
  if (check.matchesDesktop === false) return "Update Bugbee"
}
...
export function addServerViewModel(input: {
  ...
  const bugbeeCheck = selectedDistro ? (state?.bugbeeChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.bugbeeChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateBugbee" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.bugbeeMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installBugbee" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  bugbeeCheck: WslBugbeeCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingBugbee =
    input.state?.job?.kind === "install-bugbee" && input.state.job.distro === input.selectedDistro
  if (!ready || probingBugbee) {
    return {
      variant: "contrast",
  ...
  if (!addServerBugbeeReady(input.bugbeeCheck)) {
    const update = !!input.bugbeeCheck?.resolvedPath && input.bugbeeCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingBugbee
        ? { key: "wsl.onboarding.updatingBugbee" }
        : update
          ? { key: "wsl.onboarding.updateBugbee" }
          : { key: "wsl.onboarding.installBugbee" },
      disabled: !!input.state?.job || input.adding,
      action: "install-bugbee",
      loading: installingBugbee,
...
function addServerBugbeeReady(check: WslBugbeeCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.bugbeeChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.bugbeeChecks[item.name]) return [`bugbee:${item.name}`]
    return []
  })
