...
import { Tag } from "@bugbee-ai/ui/v2/badge-v2"
...
import { wslBugbeeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.bugbeeChecks[item.config.distro]
          const bugbeeAction = () => wslBugbeeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-bugbee" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={bugbeeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installBugbee(item.config.distro))}
                    >
