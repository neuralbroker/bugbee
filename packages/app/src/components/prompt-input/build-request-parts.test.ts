...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.bugbeeComment &&
          (part.metadata.bugbeeComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("preserves an external attachment source path for the model", () => {
    const result = buildRequestParts({
      ...
      images: [
        {
          type: "image",
          id: "img_external",
          filename: "bugbee.global.dat",
          sourcePath: "C:\\Users\\Luke\\AppData\\Roaming\\ai.bugbee.desktop.beta\\bugbee.global.dat",
          mime: "text/plain",
      ...
      sessionDirectory: "C:\\Repos\\sst\\bugbee",
    })

    expect(result.requestParts.find((part) => part.type === "file")?.filename).toBe(
      "C:\\Users\\Luke\\AppData\\Roaming\\ai.bugbee.desktop.beta\\bugbee.global.dat",
    )
  })
  ...
  test("handles macOS paths correctly", () => {
    ...
    const result = buildRequestParts({
      ...
      messageID: "msg_mac_1",
      sessionID: "ses_mac_1",
      sessionDirectory: "/Users/kelvin/Projects/bugbee",
    })
    ...
    if (filePart?.type === "file") {
      ...
      expect(() => new URL(filePart.url)).not.toThrow()
      // Should be a normal Unix path
      expect(filePart.url).toBe("file:///Users/kelvin/Projects/bugbee/README.md")
    }
  })
