import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@bugbee-ai/sdk/v2/client"
...
import { List, type ListRef } from "@bugbee-ai/ui/list"
import { ProviderIcon } from "@bugbee-ai/ui/provider-icon"
import { Spinner } from "@bugbee-ai/ui/spinner"
import { Tag } from "@bugbee-ai/ui/tag"
import { TextField } from "@bugbee-ai/ui/text-field"
import { ButtonV2 } from "@bugbee-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@bugbee-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@bugbee-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "bugbee-go") return language.t("dialog.provider.bugbeeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "bugbee"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.bugbee.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["bugbee", "bugbee-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "bugbee" || provider.id === "bugbee-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "bugbee"
                                ? "dialog.provider.bugbee.tagline"
                                : "dialog.provider.bugbeeGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "bugbee"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.bugbeeZen.line1")}</div>
              <div>{language.t("provider.connect.bugbeeZen.line2")}</div>
              <div>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "bugbee"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.bugbeeZen.line1")}</div>
