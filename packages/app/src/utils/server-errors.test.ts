...
import type { SessionNotFoundError } from "@bugbee-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em bugbee.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
