  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://bugbee.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://bugbee.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://bugbee.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://bugbee.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://bugbee.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://bugbee.example.com"),
      ),
    ).toEqual({
