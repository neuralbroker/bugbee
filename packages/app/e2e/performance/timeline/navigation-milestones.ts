    ...
    ({ triggerSelector, milestones }) => {
          ...
          setTimeout(() => {
            ...
            Object.entries(current).forEach(([name, value]) => {
              ...
              if (!marked.has(`${name}.first`)) {
                performance.mark(`bugbee.navigation.${name}.first`)
                marked.add(`${name}.first`)
              }
              const streak = (streaks.get(name) ?? 0) + 1
              streaks.set(name, streak)
              if (streak === 3) performance.mark(`bugbee.navigation.${name}.stable`)
            })
            ...
            if (all && !marked.has("all.first")) {
              performance.mark("bugbee.navigation.all.first")
              marked.add("all.first")
            }
            if (allStreak === 3) performance.mark("bugbee.navigation.all.stable")
            sample()
          }, 0)
        })
      }
      document.addEventListener(
        ...
        (event) => {
          ...
          performance.mark("bugbee.navigation.click")
          sample()
        },
