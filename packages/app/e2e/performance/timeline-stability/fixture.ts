...
import { Event } from "@bugbee-ai/schema/event"
import { SessionStatusEvent } from "@bugbee-ai/schema/session-status-event"
import { SessionV1 } from "@bugbee-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockBugbeeServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/Bugbee/TimelineStability"
...
export const model = { providerID: "bugbee", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("bugbee.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["bugbee"],
    default: { providerID: "bugbee", modelID: "claude-opus-4-6" },
  }
}
