...
export async function startChromeTrace(page: Page, name: string) {
  const directory = process.env.BUGBEE_PERFORMANCE_TRACE_DIR
  if (!directory) return

  const selectors = process.env.BUGBEE_PERFORMANCE_SELECTOR_TRACE === "1"
...
export async function prepareChromeTrace(
  ...
  const run = process.env.BUGBEE_PERFORMANCE_RUN_ID ?? "manual"
