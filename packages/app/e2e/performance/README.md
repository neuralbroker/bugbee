...
Set `BUGBEE_PERFORMANCE_TRACE_DIR` to emit a standard Chrome DevTools trace for every benchmark page automatically:

```sh
BUGBEE_PERFORMANCE_TRACE_DIR=/tmp/bugbee-performance-traces \
...
INP analysis requires a trace with a supported navigation/interaction insight. Selector statistics require a trace captured with `BUGBEE_PERFORMANCE_SELECTOR_TRACE=1`.
