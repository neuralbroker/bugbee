...
const directory = "C:/Bugbee/FileBrowserSidebar"
...
async function setup(page: Page) {
  await mockBugbeeServer(page, {
    ...
    provider: {
      ...
      connected: ["bugbee"],
      default: { providerID: "bugbee", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
  ...
  await page.addInitScript(
    ({ directory, server, sessionID }) => {
      ...
      localStorage.setItem(
        "bugbee.global.dat:layout",
        JSON.stringify({ review: { diffStyle: "split", panelOpened: true } }),
      )
      localStorage.setItem(
        "bugbee.global.dat:review-panel-v2",
        JSON.stringify({ sidebarOpened: true, sidebarWidth: 240, expandMode: "collapse" }),
      )
      localStorage.setItem(
        "bugbee.window.browser.dat:tabs",
