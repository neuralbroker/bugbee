...
import type { UpdaterState } from "@bugbee-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@bugbee-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.BUGBEE_CHANNEL === "prod" &&
...
const deepLinkEvent = "bugbee:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__BUGBEE__ ??= {}
  const pending = window.__BUGBEE__.deepLinks ?? []
  window.__BUGBEE__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `bugbee.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://bugbee.dev/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("bugbee.global.dat").getItem("language")
