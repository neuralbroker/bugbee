...
import { expectBugbeeVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveBugbee?: typeof resolveWslBugbee
...
export function createWslServersController(
  ...
  const setBugbeeCheck = (distro: string, check: WslBugbeeCheck) => {
    setState({
      bugbeeChecks: {
        ...state.bugbeeChecks,
        [distro]: check,
  ...
  const checkBugbee = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveBugbee ?? resolveWslBugbee)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return bugbeeCheck(distro, resolved, version, appVersion)
  }

  const refreshBugbeeCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setBugbeeCheck(distro, await checkBugbee(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const bugbeeChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.bugbeeChecks[distro])
        .map(async (distro) => [distro, await checkBugbee(distro, opts)] as const),
    )
    if (bugbeeChecks.length) {
      setState({ bugbeeChecks: { ...state.bugbeeChecks, ...Object.fromEntries(bugbeeChecks) } })
    }
  }
  ...
  const refreshBugbeeCheckBackground = (id: string, distro: string) => {
    void checkBugbee(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setBugbeeCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl bugbee check failed", { id, distro, message })
      })
  }

  const refreshBugbeeChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkBugbee(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setBugbeeCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl bugbee check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshBugbeeCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installBugbee(name: string) {
      await runJob({ kind: "install-bugbee", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslBugbee(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "Bugbee installation failed")
        }
        await refreshBugbeeCheck(name, { signal: abort.signal })
        expectBugbeeVersion(state.bugbeeChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.bugbeeChecks, distro) : {}),
      })
    },
...
function bugbeeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "bugbee is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "bugbee is installed but could not run",
    }
  }
  return {
    distro,
