...
import { type WslCommandLine, resolveWslBugbee, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const bugbee = await resolveWslBugbee(distro)
  if (!bugbee) throw new Error(`Bugbee is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "bugbee"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export BUGBEE_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export BUGBEE_CLIENT=desktop",
    `export BUGBEE_SERVER_USERNAME=${shellEscape(username)}`,
    `export BUGBEE_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(bugbee)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
