import type { WslDistroProbe, WslBugbeeCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  bugbeeChecks: Record<string, WslBugbeeCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextBugbeeChecks = { ...bugbeeChecks }
  delete nextDistroProbes[distro]
  delete nextBugbeeChecks[distro]
  return { distroProbes: nextDistroProbes, bugbeeChecks: nextBugbeeChecks }
}
