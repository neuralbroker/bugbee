...
export function expectBugbeeVersion(installed: string | null, expected: string, distro = "Debian") {
  if (installed === expected) return
  throw new Error(
    `Bugbee update finished but ${distro} still reports ${installed ?? "no version"}; expected ${expected}`,
  )
}
