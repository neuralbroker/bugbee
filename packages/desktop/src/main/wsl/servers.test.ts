...
let releaseBugbeeResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectBugbeeVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectBugbeeVersion("1.14.35", "1.16.2")).toThrow(
    "Bugbee update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Bugbee", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.bugbee/bin/bugbee",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, bugbeeChecks: {} })
})
...
test("ignores stale background Bugbee checks after removing a WSL server", async () => {
  persistedServers = []
  releaseBugbeeResolve = undefined
  ...
  await waitFor(() => !!releaseBugbeeResolve)
  await controller.removeServer("wsl:Debian")
  releaseBugbeeResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().bugbeeChecks).toEqual({})
})

test("ignores stale startup Bugbee checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Bugbee", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveBugbee: async (distro) => {
      bugbee.push(distro)
      return "/home/me/.bugbee/bin/bugbee"
    },
  })
  ...
  expect(bugbee).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(bugbee).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().bugbeeChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Bugbee in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const bugbee: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(bugbee).toEqual(["Debian"])
  expect(Object.keys(controller.getState().bugbeeChecks)).toEqual(["Debian"])
})
