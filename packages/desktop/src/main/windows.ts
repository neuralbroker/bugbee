...
import type { DesktopTheme } from "@bugbee-ai/ui/theme/types"
...
function windowDataFile(id: string) {
  return `bugbee.window.${id.replace(/[^a-zA-Z0-9._-]/g, "-")}.dat`
}
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "Bugbee window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("Bugbee is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
