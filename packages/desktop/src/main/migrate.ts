...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.bugbee.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `bugbee.settings.dat` is special: it maps to the `bugbee.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"bugbee.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // bugbee.settings.dat → the electron settings store ("bugbee.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "bugbee.global.dat").
  const storeName = filename === "bugbee.settings.dat" ? "bugbee.settings" : filename
