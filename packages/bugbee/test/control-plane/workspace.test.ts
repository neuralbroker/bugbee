...
import { EventSequenceTable } from "@bugbee-ai/core/event/sql"
...
import { WorkspaceV2 } from "@bugbee-ai/core/workspace"
import { WorkspaceTable } from "@bugbee-ai/core/control-plane/workspace.sql"
...
import { Ripgrep } from "@bugbee-ai/core/ripgrep"
import { AppNodeBuilder } from "@bugbee-ai/core/effect/app-node-builder"
import { LayerNode } from "@bugbee-ai/core/effect/layer-node"

const originalEnv = {
  BUGBEE_AUTH_CONTENT: process.env.BUGBEE_AUTH_CONTENT,
  BUGBEE_EXPERIMENTAL_WORKSPACES: process.env.BUGBEE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.BUGBEE_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@bugbee.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.BUGBEE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=bugbee-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.BUGBEE_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.BUGBEE_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.BUGBEE_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=bugbee-test")
