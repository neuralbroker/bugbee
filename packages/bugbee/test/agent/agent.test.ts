...
it.instance("plan agent denies edits except .bugbee/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".bugbee/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".bugbee", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.BUGBEE_TEST_HOME
      process.env.BUGBEE_TEST_HOME = test.directory
