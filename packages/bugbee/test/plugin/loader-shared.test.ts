...
const { Npm } = await import("@bugbee-ai/core/npm")
...
function load(dir: string, flags?: Parameters<typeof RuntimeFlags.layer>[0]) {
  const source = path.join(dir, "bugbee.json")
  return Effect.gen(function* () {
  ...
  it.live("loads npm server plugin from package ./server export", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "bugbee.json"), JSON.stringify({ plugin: ["acme-plugin@1.0.0"] }, null, 2))
  ...
  it.live("rejects npm server export that resolves outside plugin directory", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "bugbee.json"), JSON.stringify({ plugin: ["acme-plugin"] }, null, 2))
    ...
    withTmp(
      async (dir) => {
        await Bun.write(
          ...
          JSON.stringify(
            {
              plugin: ["bugbee-openai-codex-auth@1.0.0", "bugbee-copilot-auth@1.0.0", "regular-plugin@1.0.0"],
            },
            null,
            2,
          ),
      ...
      (_tmp) =>
        Effect.gen(function* () {
          ...
          try {
            ...
            expect(pkgs).not.toContain("bugbee-openai-codex-auth@1.0.0")
            expect(pkgs).not.toContain("bugbee-copilot-auth@1.0.0")
          } finally {
            install.mockRestore()
          }
        }),
  ...
  it.live("continues loading plugins when plugin init throws", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "bugbee.json"), JSON.stringify({ plugin: [file, ok] }, null, 2))
  ...
  it.live("continues loading plugins when plugin import fails", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(path.join(dir, "bugbee.json"), JSON.stringify({ plugin: [missing, ok] }, null, 2))
...
`,
        ...
        await Bun.write(path.join(dir, "bugbee.json"), JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2))
