...
const paid = (providers: Record<string, { models: Record<string, { cost: { input: number } }> }>) => {
  const item = providers[ProviderV2.ID.make("bugbee")]
...
it.instance("ModelNotFoundError suggests catalog models for unloaded providers", () =>
  Effect.gen(function* () {
    yield* remove("BUGBEE_API_KEY")
    const error = yield* Provider.use
      .getModel(ProviderV2.ID.bugbee, ModelV2.ID.make("claude-haiku-fake-model"))
      .pipe(Effect.flip)
...
it.instance(
  ...
  {
    config: {
      provider: { "cloudflare-ai-gateway": { options: { metadata: { invoked_by: "test", project: "bugbee" } } } },
    },
  },
)
...
it.effect("plugin config providers persist after instance dispose", () =>
  Effect.gen(function* () {
    ...
    const configDir = path.join(dir, ".bugbee")
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const configDir = path.join(instance.directory, ".bugbee")
    const root = path.join(configDir, "plugin")
    yield* Effect.promise(() => mkdir(root, { recursive: true }))
    yield* Effect.promise(() => markPluginDependenciesReady(configDir))
    yield* Effect.promise(() =>
...
it.effect("bugbee loader keeps paid models when config apiKey is present", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped({
      config: { provider: { bugbee: { options: { apiKey: "test-key" } } } },
    })
...
it.effect("bugbee loader keeps paid models when auth exists", () =>
  Effect.gen(function* () {
    const noneDir = yield* tmpdirScoped()
    const keyedDir = yield* tmpdirScoped()
    ...
    yield* Effect.acquireRelease(
      Effect.promise(() => Filesystem.write(authPath, JSON.stringify({ bugbee: { type: "api", key: "test-key" } }))),
      () =>
