...
describe("bugbee read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // BUGBEE_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ bugbee }) =>
      Effect.gen(function* () {
        const r = yield* bugbee.spawn(["mcp", "list"])
        bugbee.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bugbee }) =>
      Effect.gen(function* () {
        const r = yield* bugbee.spawn(["providers", "list"])
        bugbee.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ bugbee }) =>
      Effect.gen(function* () {
        const r = yield* bugbee.spawn(["models"])
        bugbee.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ bugbee }) =>
      Effect.gen(function* () {
        const r = yield* bugbee.spawn(["agent", "list"])
        bugbee.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh BUGBEE_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ bugbee }) =>
      Effect.gen(function* () {
        const r = yield* bugbee.spawn(["session", "list"])
        bugbee.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bugbee }) =>
      Effect.gen(function* () {
        const r = yield* bugbee.spawn(["stats"])
        bugbee.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under BUGBEE_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ bugbee }) =>
      Effect.gen(function* () {
        const r = yield* bugbee.spawn(["db", "path"])
        bugbee.expectExit(r, 0, "db path")
