...
import type { QuestionRequest } from "@bugbee-ai/sdk/v2"
import { BugbeeKeymapProvider, registerBugbeeKeymap } from "@bugbee-ai/tui/keymap"
import {
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerBugbeeKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerBugbeeKeymap(keymap, renderer, tuiConfig)
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "bugbee", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerBugbeeKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "bugbee", modelID: "gpt-5" })
