...
import { BugbeeClient, type GlobalEvent } from "@bugbee-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<BugbeeClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<BugbeeClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<BugbeeClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<BugbeeClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<BugbeeClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://bugbee.test"),
...
function sdk(
  input: {
    ...
    subscribe?: BugbeeClient["event"]["subscribe"]
    globalEvent?: BugbeeClient["global"]["event"]
    promptAsync?: BugbeeClient["session"]["promptAsync"]
    status?: BugbeeClient["session"]["status"]
    messages?: BugbeeClient["session"]["messages"]
    children?: BugbeeClient["session"]["children"]
    permissions?: BugbeeClient["permission"]["list"]
    questions?: BugbeeClient["question"]["list"]
  } = {},
) {
  const client = new BugbeeClient()

  const subscribe: BugbeeClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: BugbeeClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: BugbeeClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: BugbeeClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: BugbeeClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: BugbeeClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: BugbeeClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: BugbeeClient["question"]["list"] = input.questions ?? (() => ok([]))
