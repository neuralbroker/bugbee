// Subprocess integration tests for `bugbee run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `bugbee.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `BUGBEE_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("bugbee run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bugbee.run("say hi")
        bugbee.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bugbee.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* bugbee.run("think", { extraArgs: ["--thinking"] })
        bugbee.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* bugbee.run("think again")
        bugbee.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bugbee.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bugbee.run("say hi", { format: "json" })
        bugbee.expectExit(result, 0)

        const events = bugbee.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ bugbee }) =>
      Effect.gen(function* () {
        const result = yield* bugbee.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bugbee.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const result = yield* bugbee.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* bugbee.run("request permission", { permission: { bash: "ask" } })
        bugbee.expectExit(denied, 0)
        ...
        const allowed = yield* bugbee.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        bugbee.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* bugbee.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        bugbee.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, bugbee }) =>
      Effect.gen(function* () {
        ...
        const server = yield* bugbee.serve()

        const result = yield* bugbee.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, bugbee }) =>
      Effect.gen(function* () {
        const result = yield* bugbee.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, bugbee }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* bugbee.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
