...
import { createTuiAttention } from "@bugbee-ai/tui/attention"
...
describe("createTuiAttention", () => {
  ...
  test("supports focused-only requests", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "bugbee", message: "focused" }])
  })
  ...
  test("notification can deliver while focused when requested", async () => {
    ...
    expect(audio.playCalls).toBe(1)
    expect(renderer.notifications).toEqual([{ title: "bugbee", message: "hello" }])
  })
  ...
  test("notifies while blurred", async () => {
    ...
    expect(await attention.notify({ title: "bugbee", message: "hello", sound: false })).toEqual({
      ok: true,
      notification: true,
      sound: false,
    })
  ...
  test("can play sound always while notification is blurred-only", async () => {
    ...
    expect(renderer.notifications).toEqual([{ title: "bugbee", message: "hello again" }])
  })
  ...
  test("plays named sounds from the active sound pack", async () => {
    ...
    dispose()
    expect(attention.soundboard.current()).toBe("bugbee.default")
  })
