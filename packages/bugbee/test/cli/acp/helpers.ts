...
export function createAcpClient(input: Pick<CliFixture, "bugbee">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.bugbee.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "bugbee-local-acp", version: "0.1.0" },
      }),
    )
  })
}
