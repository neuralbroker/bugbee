...
describe("bugbee acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ bugbee }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ bugbee }))
        ...
        expect(initialized.agentInfo?.name).toBe("Bugbee")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bugbee }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ bugbee })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("bugbee-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "bugbee-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.BUGBEE_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ bugbee }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("bugbee-login")
