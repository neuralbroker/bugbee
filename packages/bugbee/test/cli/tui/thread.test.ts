...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ bugbee }) =>
    Effect.gen(function* () {
      const result = yield* bugbee.spawn(["--replay-limit", "10"])

      bugbee.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ bugbee }) =>
    Effect.gen(function* () {
      const result = yield* bugbee.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ bugbee }) =>
    Effect.gen(function* () {
      const result = yield* bugbee.spawn(["--mini", "--port", "4096"])
