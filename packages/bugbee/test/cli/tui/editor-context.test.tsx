...
import { EditorContextProvider, useEditorContext, type EditorIntegration } from "@bugbee-ai/tui/context/editor"
import { tmpdir } from "../../fixture/fixture"
import { FakeWebSocket } from "../../lib/websocket"
import { TestTuiContexts } from "../../fixture/tui-environment"
import { discoverEditorConnection } from "@bugbee-ai/tui/editor"

const originalClaudePort = process.env.CLAUDE_CODE_SSE_PORT
const originalBugbeePort = process.env.BUGBEE_EDITOR_SSE_PORT

afterEach(() => {
  process.env.CLAUDE_CODE_SSE_PORT = originalClaudePort
  process.env.BUGBEE_EDITOR_SSE_PORT = originalBugbeePort
})
...
function mountEditorContext(WebSocketImpl?: typeof WebSocket) {
  ...
  createRoot((nextDispose) => {
    ...
    const value = process.env.CLAUDE_CODE_SSE_PORT || process.env.BUGBEE_EDITOR_SSE_PORT
    return (
...
test("useEditorContext reconnect switches editor server by session directory", async () => {
  ...
  process.env.BUGBEE_EDITOR_SSE_PORT = undefined
...
test("useEditorContext connects with BUGBEE_EDITOR_SSE_PORT", async () => {
  await using tmp = await tmpdir()
  process.env.CLAUDE_CODE_SSE_PORT = undefined
  process.env.BUGBEE_EDITOR_SSE_PORT = "4020"
