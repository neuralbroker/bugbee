...
// terminal sizes. The default bugbee tui command is excluded —
// `bugbee --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `bugbee --help` prints today;
...
// top-level help on `--help` and exits 1; not a real bugbee command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("bugbee CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ bugbee }) =>
      Effect.gen(function* () {
        const topLevel = yield* bugbee.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* bugbee.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`bugbee ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`bugbee ${argv.join(" ")} --help`)
        }
