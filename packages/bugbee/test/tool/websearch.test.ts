...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.BUGBEE_WEBSEARCH_PROVIDER

    try {
      process.env.BUGBEE_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.BUGBEE_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.BUGBEE_WEBSEARCH_PROVIDER
      else process.env.BUGBEE_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for bugbee or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.bugbee, { exa: false, parallel: false })).toBe(true)
