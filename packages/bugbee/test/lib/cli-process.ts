// Subprocess test harness for the bugbee CLI. Spawns the real binary against
...
//
// Configuration flows through bugbee's built-in test affordances:
//   - BUGBEE_CONFIG_CONTENT      : provider config inline, no files to find
//   - BUGBEE_TEST_HOME           : pins os.homedir() → tmpdir
//   - BUGBEE_DISABLE_PROJECT_CONFIG : skip walking up for bugbee.json
//   - BUGBEE_PURE                : skip external plugin discovery + install
//   - BUGBEE_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `bugbee.run` is fully wired. The shape supports adding more
// builders (`bugbee.serve(opts)`, `bugbee.acp(opts)`, `bugbee.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of BugbeeCli.
...
import { AppProcess } from "@bugbee-ai/core/process"
...
const bugbeeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(bugbeeRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    BUGBEE_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    BUGBEE_CONFIG_CONTENT: configJson,
    BUGBEE_DISABLE_PROJECT_CONFIG: "1",
    BUGBEE_PURE: "1",
    BUGBEE_DISABLE_AUTOUPDATE: "1",
    BUGBEE_DISABLE_AUTOCOMPACT: "1",
    BUGBEE_DISABLE_MODELS_FETCH: "1",
    BUGBEE_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `bugbee run`. New flags should
...
// `bugbee serve` is a long-lived process — it never exits on its own.
...
// `bugbee acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type BugbeeCli = {
  ...
  // Spawn `bugbee serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `bugbee acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("bugbee.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          BUGBEE_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("bugbee.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("bugbee.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "bugbee server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `bugbee serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("bugbee.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const bugbee: BugbeeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, bugbee })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `bugbee.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "bugbee") {
...
// every call site. Use this for any test that needs the bugbee CLI fixture.
//
...
// resources (e.g. `bugbee.serve`) without an extra `Effect.scoped` wrapper —
