...
import type { Event, BugbeeClient } from "@bugbee-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<BugbeeClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "bugbee-acp-permission-"))
