...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "bugbee.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "bugbee.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "bugbee.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "bugbee.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["bugbee.json", "bugbee.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "bugbee.json"),
        path.join(project, "bugbee.jsonc"),
      ])
    })
  })
