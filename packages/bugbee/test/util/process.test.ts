...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.BUGBEE_TEST ?? "")'), {
      env: {
        BUGBEE_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "BUGBEE_TEST_SHELL"], {
      shell: true,
      env: {
        BUGBEE_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("BUGBEE_TEST_SHELL=ok")
  })
