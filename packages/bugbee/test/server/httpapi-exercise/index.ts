 ...
 *
 * The script intentionally isolates `BUGBEE_DB` before importing modules that touch
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "bugbee.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "bugbee.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-bugbee-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-bugbee-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "bugbee", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "x-bugbee-ticket": "1" },
    }))
    .json(404, object, "status"),
  http.protected
  ...
  http.protected
    ...
    .at((ctx) => ({
      ...
      headers: { ...ctx.headers(), "content-type": "application/json" },
      body: { model: { providerID: "bugbee", id: "big-pickle" } },
    }))
    .status(204, undefined, "none"),
  http.protected
