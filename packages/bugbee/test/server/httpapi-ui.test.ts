  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("BUGBEE_SERVER_PASSWORD", original.envPassword)
        restoreEnv("BUGBEE_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
...
function authConfigLayer(input?: { password?: string; username?: string }) {
  return ServerAuth.Config.configLayer({
    ...
    username: input?.username ?? "bugbee",
  })
}
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            BUGBEE_SERVER_PASSWORD: input?.password,
            BUGBEE_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  it.live("serves the web UI through the HTTP API app", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        client: httpClient(
          new Response("<html>bugbee</html>", { headers: { "content-type": "text/html" } }),
          (request) => {
            proxiedUrl = request.url
          },
        ),
      ...
      expect(yield* responseText(response)).toBe("<html>bugbee</html>")
      expect(proxiedUrl).toBe("https://app.bugbee.ai/")
    }),
  )
  ...
  it.live("strips upstream transfer encoding headers from proxied assets", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(200)
      expect(proxiedUrl).toBe("https://app.bugbee.ai/assets/app.js")
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        username: "bugbee",
        disableEmbeddedWebUi: true,
        client: httpClient(new Response("<html>bugbee</html>", { headers: { "content-type": "text/html" } })),
      }).request(`/?auth_token=${btoa("bugbee:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("bugbee:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("bugbee:sec:ret")}` },
      })
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "bugbee" }).request("/", {
        method: "OPTIONS",
        headers: {
