...
const original = {
  ...
  envPassword: process.env.BUGBEE_SERVER_PASSWORD,
  envUsername: process.env.BUGBEE_SERVER_USERNAME,
}
const auth = { username: "bugbee", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.BUGBEE_SERVER_PASSWORD
  else process.env.BUGBEE_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.BUGBEE_SERVER_USERNAME
  else process.env.BUGBEE_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.BUGBEE_SERVER_PASSWORD = auth.password
  Flag.BUGBEE_SERVER_USERNAME = auth.username
  process.env.BUGBEE_SERVER_PASSWORD = auth.password
  process.env.BUGBEE_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.BUGBEE_SERVER_USERNAME = auth.username
  delete process.env.BUGBEE_SERVER_PASSWORD
  process.env.BUGBEE_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-bugbee-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-bugbee-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-bugbee-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.BUGBEE_DISABLE_DEFAULT_PLUGINS
    process.env.BUGBEE_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.BUGBEE_DISABLE_DEFAULT_PLUGINS
      else process.env.BUGBEE_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-bugbee-ticket": "1" },
      })
