...
import { AccountV2 } from "@bugbee-ai/core/account"
import { AccountTable } from "@bugbee-ai/core/account/sql"
...
function withCreatedWorktree(
  ...
  return Effect.acquireUseRelease(
    Effect.gen(function* () {
      ...
      expect(info).toMatchObject({ name, branch: "bugbee/api-test" })
      ...
      Effect.gen(function* () {
        ...
        const [consoleState, consoleOrgs, toolList, toolIDs, worktrees, resources] = yield* Effect.all(
          [
            ...
            request(`${ExperimentalPaths.tool}?provider=bugbee&model=gpt-5`, directory),
