...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const bugbeeFile = path.join(tmp, ".git", "bugbee")
      expect(yield* Effect.promise(() => Bun.file(bugbeeFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "bugbee")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "bugbee")
      const wrongCache = path.join(parentDir, ".git", "bugbee")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "bugbee")
      const cacheB = path.join(bareB, "bugbee")
      const wrongCache = path.join(parentDir, ".git", "bugbee")
