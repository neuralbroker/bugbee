    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe(`bugbee/${info.name}`)
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(info.branch).toBe("bugbee/my-feature")
        }),
      { git: true },
    )
    ...
    it.instance(
      ...
      () =>
        Effect.gen(function* () {
          ...
          const svc = yield* Worktree.Service
          yield* git(test.directory, ["branch", "bugbee/my-feature"])
    ...
    it.instance(
      ...
      () =>
        withCreatedWorktree(undefined, ({ info }) =>
          Effect.gen(function* () {
            expect(info.name).toBeDefined()
            expect(info.branch ?? "").toStartWith("bugbee/")
      ...
      () =>
        withCreatedWorktree({ name: "test-workspace" }, ({ info }) =>
          Effect.gen(function* () {
            ...
            expect(info.branch).toBe("bugbee/test-workspace")
          }),
        ),
      { git: true },
    )
