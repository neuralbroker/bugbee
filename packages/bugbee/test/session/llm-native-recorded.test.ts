...
import { SessionV1 } from "@bugbee-ai/core/v1/session"
import { ModelsDev } from "@bugbee-ai/core/models-dev"
import { HttpRecorder } from "@bugbee-ai/http-recorder"
import { HttpRecorderInternal } from "@bugbee-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@bugbee-ai/llm"
import { RequestExecutor } from "@bugbee-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.bugbee.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.BUGBEE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["bugbee", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("BUGBEE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("BUGBEE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["bugbee", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "bugbee-proxy",
    name: "Bugbee proxy",
    providerID: ProviderV2.ID.bugbee,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["bugbee", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.BUGBEE_RECORD_CONSOLE_TOKEN && process.env.BUGBEE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.bugbee,
        name: "Bugbee Zen",
        env: ["BUGBEE_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.BUGBEE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.BUGBEE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.BUGBEE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["bugbee", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("BUGBEE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("BUGBEE_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("BUGBEE_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set BUGBEE_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the bugbee LLM stack remain real.
  const metadata = {
