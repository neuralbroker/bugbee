...
import type { NamedError } from "@bugbee-ai/core/util/error"
...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to Go upsell action", () => {
    ...
    expect(SessionRetry.retryable(error, "bugbee")).toEqual({
      message: SessionRetry.GO_UPSELL_MESSAGE,
      action: {
        reason: "free_tier_limit",
        provider: "bugbee",
        title: "Free limit reached",
        message: "Subscribe to Bugbee Go for reliable access to the best open-source models, starting at $5/month.",
        label: "subscribe",
  ...
  test("maps Go subscription limits to workspace PAYG upsell", () => {
    ...
    expect(SessionRetry.retryable(error, "bugbee-go")).toEqual({
      message:
        "5 hour usage limit reached. It will reset in 5 hours 23 minutes. To continue using this model now, enable usage from your available balance - https://bugbee.dev/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      action: {
        ...
        label: "open settings",
        link: "https://bugbee.dev/workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      },
    })
  })
  ...
  test("maps Go subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "bugbee-go")?.action?.message).toBe(
