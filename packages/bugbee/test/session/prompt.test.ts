...
import { SessionMessageTable } from "@bugbee-ai/core/session/sql"
...
import { Shell } from "@bugbee-ai/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@bugbee-ai/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://github.com/neuralbroker/bugbee/config.json", ...config }),
  )
})
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/bugbee/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("bugbee"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
