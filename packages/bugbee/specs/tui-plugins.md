...
- Author package entrypoint is `@bugbee-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/bugbee-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@bugbee-ai/plugin/tui`.
- `@bugbee-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@bugbee-ai/plugin/tui"
...
- The value is a semver range checked against the running Bugbee version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.bugbee` is absent, no check is performed (backward compatible).
...
- `bugbee plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `bugbee plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.bugbee` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.bugbee`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.bugbee`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.bugbee` dir or `BUGBEE_CONFIG_DIR`, Bugbee installs `@bugbee-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@bugbee-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Bugbee's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Bugbee registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Bugbee pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@bugbee-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"bugbee"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `bugbee.default`.
...
- Local plugins persist installed themes under the local `.bugbee/themes` area near the plugin config source.
...
- `--pure` / `BUGBEE_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.bugbee/plugins/tui-smoke.tsx`
- Local vim plugin: `.bugbee/plugins/tui-vim.tsx`
- Local smoke config: `.bugbee/tui.json`
- Local smoke theme: `.bugbee/plugins/smoke-theme.json`
