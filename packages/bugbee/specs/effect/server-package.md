...
Practical reference for a future `packages/server` split after the bugbee
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/bugbee`.
...
Until enough shared service code lives outside `packages/bugbee`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/bugbee`

It should not import `packages/bugbee` services while `packages/bugbee`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on bugbee-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/bugbee` implementation details.
