...
Trim `packages/bugbee/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/bugbee/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/bugbee`.
- `[x]` `bun typecheck` from `packages/bugbee`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/bugbee/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/bugbee`.
- `[x]` `bun dev generate > /tmp/bugbee-openapi.json` from `packages/bugbee`.
...
- Edit `packages/bugbee/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/bugbee-openapi.json` from `packages/bugbee`.
...
- Check source schemas in files like `packages/bugbee/src/session/schema.ts`, `packages/bugbee/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/bugbee/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/bugbee/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
