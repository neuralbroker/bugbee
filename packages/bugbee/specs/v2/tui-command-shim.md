...
Delete `packages/bugbee/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/bugbee/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/bugbee/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/bugbee`
- TUI plugin loader tests in `packages/bugbee` if runtime plugin API wiring changed
