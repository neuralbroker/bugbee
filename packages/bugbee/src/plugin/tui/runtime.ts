...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@bugbee-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@bugbee-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@bugbee-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@bugbee-ai/tui/context/theme"
import { Global } from "@bugbee-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@bugbee-ai/core/util/flock"
import { Flag } from "@bugbee-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@bugbee-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@bugbee-ai/core/v1/config/plugin"
import { createCommandShim } from "@bugbee-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@bugbee-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".bugbee"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".bugbee", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".bugbee", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.BUGBEE_PURE ? [] : pluginOrigins
    if (Flag.BUGBEE_PURE && pluginOrigins.length) {
    }
