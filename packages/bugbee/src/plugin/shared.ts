...
export const DEPRECATED_PLUGIN_PACKAGES = ["bugbee-openai-codex-auth", "bugbee-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, bugbeeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(bugbeeVersion) || semver.major(bugbeeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.bugbee
  if (typeof range !== "string") return
  if (!semver.satisfies(bugbeeVersion, range)) {
    throw new Error(`Plugin requires bugbee ${range} but running ${bugbeeVersion}`)
  }
}
