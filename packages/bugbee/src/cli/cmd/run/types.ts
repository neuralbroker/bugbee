// Shared type vocabulary for the direct interactive mode (`bugbee --mini`).
//
...
import type { BugbeeClient, PermissionRequest, QuestionRequest, ToolPart } from "@bugbee-ai/sdk/v2"
import type { TuiConfig } from "@bugbee-ai/tui/config"
...
type PromptModel = Parameters<BugbeeClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<BugbeeClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<BugbeeClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<BugbeeClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<BugbeeClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<BugbeeClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<BugbeeClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<BugbeeClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<BugbeeClient["question"]["reject"]>[0]
