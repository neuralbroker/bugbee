...
//
// Enable with BUGBEE_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/bugbee/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.BUGBEE_DIRECT_TRACE) {
    state = false
    return undefined
  }
