import type { PermissionV1 } from "@bugbee-ai/core/v1/permission"
import { FSUtil } from "@bugbee-ai/core/fs-util"
// CLI entry point for `bugbee run` and `bugbee --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`bugbee --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`bugbee --mini --attach`): connects to a running
//      bugbee server and runs interactive mode against it.
//
...
import { createBugbeeClient, type BugbeeClient, type ToolPart } from "@bugbee-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<BugbeeClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run bugbee with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running bugbee server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to BUGBEE_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to BUGBEE_SERVER_USERNAME or 'bugbee')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: BugbeeClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: BugbeeClient, sessionID: string) {
      ...
      async function current(sdk: BugbeeClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: BugbeeClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: BugbeeClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: BugbeeClient) {
        ...
        async function loop(client: BugbeeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createBugbeeClient({
        baseUrl: "http://bugbee.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
