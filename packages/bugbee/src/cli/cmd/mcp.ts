...
import { ConfigMCPV1 } from "@bugbee-ai/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: bugbee mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in bugbee.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .bugbee/ subdirectory too)
  const candidates = [path.join(baseDir, "bugbee.json"), path.join(baseDir, "bugbee.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".bugbee", "bugbee.json"), path.join(baseDir, ".bugbee", "bugbee.jsonc"))
  }
  ...
  // Default to bugbee.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., bugbee x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "bugbee-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
