...
const AGENT_USERNAME = "bugbee-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/bugbee.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/bugbee-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.bugbee.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
jobs:
  bugbee:
    if: |
      ...
      startsWith(github.event.comment.body, '/bugbee')
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.bugbee.ai"
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/bugbee,/oc")
        .split(",")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("bugbee-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `bugbee/${type}-${hex}-${timestamp}`
      }
      return `bugbee/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      const image = (() => {
        ...
        return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/bugbee-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
      })()
      const shareUrl = shareId ? `[bugbee session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the bugbee infrastructure after your response",
