...
import { McpEvent } from "@bugbee-ai/schema/mcp-event"
...
function createClient(directory: string) {
  const client = new Client({ name: "bugbee", version: InstallationVersion }, CLIENT_OPTIONS)
...
export class Service extends Context.Service<Service, Interface>()("@bugbee/MCP") {}
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: bugbee mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
