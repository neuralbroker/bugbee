...
const TUI_SCHEMA_URL = "https://github.com/neuralbroker/bugbee/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from bugbee.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const bugbee = await bugbeeFiles(input)
  for (const file of bugbee) {
...
async function bugbeeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "bugbee"),
    ...(await Filesystem.findUp(["bugbee.json", "bugbee.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "bugbee"))
  }
  if (Flag.BUGBEE_CONFIG) files.push(Flag.BUGBEE_CONFIG)
