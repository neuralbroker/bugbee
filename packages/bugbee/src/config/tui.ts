...
import { isRecord } from "@bugbee-ai/tui/util/record"
...
import { TuiKeybind } from "@bugbee-ai/tui/config/keybind"
...
import { TuiConfig } from "@bugbee-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@bugbee/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old bugbee.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.bugbee`
  // folders between cwd and home, and BUGBEE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.BUGBEE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit BUGBEE_TUI_CONFIG override, if set.
  if (Flag.BUGBEE_TUI_CONFIG) {
    const configFile = Flag.BUGBEE_TUI_CONFIG
  ...
  // 4. `.bugbee` directories (and BUGBEE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".bugbee") || dir === Flag.BUGBEE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".bugbee") && dir !== Flag.BUGBEE_CONFIG_DIR) continue
