...
import type * as ModelsDev from "@bugbee-ai/core/models-dev"
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "bugbee" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    ...
    if (input.model.providerID.startsWith("bugbee")) {
      result["promptCacheKey"] = input.sessionID
      result["include"] = INCLUDE_ENCRYPTED_REASONING
      result["reasoningSummary"] = "auto"
    }
...
export function schema(model: Provider.Model, schema: JSONSchema7): JSONSchema7 {
  ...
  */
  ...
  if (model.api.npm === "@ai-sdk/openai" || model.api.npm === "@ai-sdk/azure") {
    ...
    // Codex also applies lossy compaction above 4 KB; defer that until Bugbee needs the same schema budget.
  }
