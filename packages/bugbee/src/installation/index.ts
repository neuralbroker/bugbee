...
import { makeRuntime } from "@bugbee-ai/core/effect/runtime"
import semver from "semver"
import { InstallationChannel, InstallationVersion } from "@bugbee-ai/core/installation/version"
import { NpmConfig } from "@bugbee-ai/core/npm-config"
import { InstallationEvent } from "@bugbee-ai/schema/installation-event"
...
export function userAgent(client = "cli") {
  return `bugbee/${InstallationChannel}/${InstallationVersion}/${client}`
}
...
export class Service extends Context.Service<Service, Interface>()("@bugbee/Installation") {}
  ...
  Effect.gen(function* () {
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        if (process.execPath.includes(path.join(".bugbee", "bin"))) return "curl" as Method
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          { name: "brew", command: () => text(["brew", "list", "--formula", "bugbee"]) },
          { name: "scoop", command: () => text(["scoop", "list", "bugbee"]) },
          { name: "choco", command: () => text(["choco", "list", "--limit-output", "bugbee"]) },
        ]
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "brew") {
          ...
          const response = yield* httpOk.execute(
            HttpClientRequest.get("https://formulae.brew.sh/api/formula/bugbee.json").pipe(
        ...
        if (detectedMethod === "choco") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27bugbee%27%20and%20IsLatestVersion&$select=Version",
        ...
        if (detectedMethod === "scoop") {
          const response = yield* httpOk.execute(
            HttpClientRequest.get(
              "https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/bugbee.json",
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "choco":
            upgradeResult = yield* run(["choco", "upgrade", "bugbee", `--version=${target}`, "-y"])
            break
          case "scoop":
            upgradeResult = yield* run(["scoop", "install", `bugbee@${target}`])
            break
          default:
