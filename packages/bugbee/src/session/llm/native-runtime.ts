...
import type { LLMClientShape } from "@bugbee-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("bugbee"))
    return { type: "unsupported", reason: "provider is not openai, bugbee, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @bugbee-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains bugbee-owned. The native runtime only adapts
      // the @bugbee-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
