...
import type { AssistantMessage, Message, BugbeeClient, SessionMessageResponse } from "@bugbee-ai/sdk/v2"
...
export const AuthMethodID = "bugbee-login"
...
export class Service extends Context.Service<Service, Interface>()("@bugbee/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `bugbee auth login` in the terminal",
      name: "Login with bugbee",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: BugbeeClient) {
...
function makeUsageService(sdk: BugbeeClient) {
...
async function loadDirectorySnapshot(sdk: BugbeeClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, bugbee provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const bugbeeProvider = providers[ProviderV2.ID.make("bugbee")]
  const bugbeeModel = bugbeeProvider ? Provider.sort(Object.values(bugbeeProvider.models))[0] : undefined
  if (bugbeeProvider && bugbeeModel) return { providerID: bugbeeProvider.id, modelID: bugbeeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Bugbee service failure", service })
}
