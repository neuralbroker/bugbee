...
import { AbsolutePath, Agent, Location, Model, Bugbee, Prompt, Session, SessionMessage } from "../src/effect"
...
test("sessions.get returns the decoded Effect projection", async () => {
  ...
  const result = await Effect.gen(function* () {
    const client = yield* Bugbee.make({ baseUrl: "http://localhost:3000" })
