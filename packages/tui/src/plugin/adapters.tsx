import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@bugbee-ai/plugin/tui"
...
import type { useBugbeeKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useBugbeeKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getBugbeeModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getBugbeeModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
