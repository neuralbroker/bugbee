...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@bugbee-ai/sdk/v2"
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__bugbee_custom_provider__"
    ...
    ...pipe(
      ...
      map((provider) => ({
        ...
        description: {
          ...
          "bugbee-go": "Low cost subscription for everyone",
        }[provider.id],
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in bugbee.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      description={() =>
        ({
          bugbee: (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Bugbee Zen gives you access to all the best coding models at the cheapest prices with a single API
                key.
              </text>
              <text fg={theme.text}>
          ...
          "bugbee-go": (
            <box gap={1}>
              <text fg={theme.textMuted}>
                Bugbee Go is a $10 per month subscription that provides reliable access to popular open coding models
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in bugbee.json to use it.`,
          })
          dialog.clear()
          return
        }
