...
function View(props: { api: TuiPluginApi; sessionID: string }) {
  ...
  const has = createMemo(() =>
    props.api.state.provider.some(
      (item) => item.id !== "bugbee" || Object.values(item.models).some((model) => model.cost?.input !== 0),
    ),
  )
      ...
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>Bugbee includes free models so you can start immediately.</text>
