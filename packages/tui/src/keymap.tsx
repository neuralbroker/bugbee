...
export const BUGBEE_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const BUGBEE_MODE_KEY = "bugbee.mode"

export const BugbeeKeymapProvider = KeymapProvider
export const useBugbeeKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type BugbeeModeStack = ReturnType<typeof createBugbeeModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, BugbeeModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createBugbeeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(BUGBEE_MODE_KEY, BUGBEE_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(BUGBEE_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(BUGBEE_MODE_KEY, stack.at(-1)?.mode ?? BUGBEE_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? BUGBEE_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(BUGBEE_MODE_KEY, undefined)
...
export function useBugbeeModeStack() {
  return getBugbeeModeStack(useBugbeeKeymap())
}

export function getBugbeeModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Bugbee mode stack is not registered for this keymap")
  return value
}
...
export function registerBugbeeKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createBugbeeModeStack(keymap)
