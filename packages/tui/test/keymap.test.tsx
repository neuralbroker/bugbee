...
import { getBugbeeModeStack, BUGBEE_BASE_MODE, BugbeeKeymapProvider, registerBugbeeKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerBugbeeKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when bugbee mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getBugbeeModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getBugbeeModeStack(keymap).push("autocomplete")
