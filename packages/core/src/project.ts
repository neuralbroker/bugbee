...
export interface Interface {
   ...
   *
   * This exists while the old bugbee project service and this core project
...
export class Service extends Context.Service<Service, Interface>()("@bugbee/ProjectV2") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const cached = Effect.fnUntraced(function* (dir: string) {
      return yield* fs.readFileString(path.join(dir, "bugbee")).pipe(
    ...
    const commit = Effect.fn("Project.commit")(function* (input: { store: AbsolutePath; id: ID }) {
      yield* fs.writeFileString(path.join(input.store, "bugbee"), input.id).pipe(Effect.ignore)
    })
