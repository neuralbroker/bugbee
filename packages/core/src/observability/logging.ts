...
export function fileLogger(file = path.join(Global.Path.log, "bugbee.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.BUGBEE_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.BUGBEE_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
