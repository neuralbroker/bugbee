...
import { ToolFailure } from "@bugbee-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@bugbee/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.BUGBEE_WEBSEARCH_PROVIDER === "exa" || process.env.BUGBEE_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.BUGBEE_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("BUGBEE_EXPERIMENTAL") || truthy("BUGBEE_ENABLE_EXA") || truthy("BUGBEE_EXPERIMENTAL_EXA"),
    enableParallel: truthy("BUGBEE_ENABLE_PARALLEL") || truthy("BUGBEE_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `bugbee/${InstallationVersion}`,
