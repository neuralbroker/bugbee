...
import type { IntegrationOAuthMethodRegistration } from "@bugbee-ai/plugin/v2/effect/integration"
import { define } from "@bugbee-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@bugbee-ai/sdk/v2/types"
...
const defaultServer = "https://console.bugbee.ai"
const clientID = "bugbee-cli"
...
export const BugbeePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "bugbee",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("BugbeePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("bugbee")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Bugbee provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("bugbee", (integration) => {
        integration.name = "Bugbee"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "bugbee", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("bugbee")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("bugbee")
      ...
      const item = catalog.provider.get(ProviderV2.ID.bugbee)
      if (!item) return
      const hasKey = Boolean(process.env.BUGBEE_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("bugbee")),
