...
async function generate() {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "bugbee-core-migration-"))
...
async function check() {
  const temporary = await fs.mkdtemp(path.join(os.tmpdir(), "bugbee-core-migration-check-"))
  const incremental = path.join(temporary, "incremental")
  const full = path.join(temporary, "full")
  try {
    await fs.mkdir(incremental)
