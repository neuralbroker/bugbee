...
import { Watcher } from "@bugbee-ai/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    BUGBEE_EXPERIMENTAL_FILEWATCHER: "true",
    BUGBEE_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@bugbee.test`.cwd(tmp.path).quiet()
