...
// test/preload.ts pins BUGBEE_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.BUGBEE_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.BUGBEE_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.BUGBEE_MODELS_PATH = undefined
  Flag.BUGBEE_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.BUGBEE_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.BUGBEE_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.BUGBEE_DISABLE_MODELS_FETCH = false
        }),
