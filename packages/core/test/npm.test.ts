...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@bugbee/acme")).toBe("@bugbee/acme")
    expect(Npm.sanitize("@bugbee/acme@1.0.0")).toBe("@bugbee/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/bugbee/acme.git"
    const expected = win ? "acme@git+https_//github.com/bugbee/acme.git" : spec
