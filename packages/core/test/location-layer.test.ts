...
import { define } from "@bugbee-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@bugbee-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "bugbee.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "bugbee.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
