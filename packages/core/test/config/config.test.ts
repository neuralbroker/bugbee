...
import { Policy } from "@bugbee-ai/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads bugbee JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "bugbee.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "bugbee.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "bugbee.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "bugbee.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "bugbee.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "bugbee.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .bugbee configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".bugbee"), { recursive: true })
            await fs.mkdir(path.join(directory, ".bugbee"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "bugbee.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "bugbee.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "bugbee.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "bugbee.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "bugbee.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".bugbee", "bugbee.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".bugbee", "bugbee.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".bugbee")),
              AbsolutePath.make(path.join(directory, ".bugbee")),
            ])
