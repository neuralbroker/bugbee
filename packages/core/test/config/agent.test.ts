...
import { ConfigAgentPlugin } from "@bugbee-ai/core/config/plugin/agent"
...
describe("ConfigAgentPlugin.Plugin", () => {
  it.effect("matches POSIX paths against home-relative permissions", () =>
    Effect.gen(function* () {
      ...
      expect(PermissionV2.evaluate("external_directory", "/home/test/p/bugbee/src/*", permissions).effect).toBe(
        "allow",
      )
  ...
  it.effect("matches Windows paths against home-relative permissions", () =>
    Effect.gen(function* () {
      ...
      expect(
        PermissionV2.evaluate("external_directory", "C:\\Users\\test\\p\\bugbee\\src\\*", permissions).effect,
      ).toBe("allow")
