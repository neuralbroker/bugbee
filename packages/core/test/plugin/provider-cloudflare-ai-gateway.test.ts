...
import { CloudflareAIGatewayPlugin } from "@bugbee-ai/core/plugin/provider/cloudflare-ai-gateway"
      ...
      Effect.gen(function* () {
        ...
        yield* aisdk.runSDK({
          ...
          options: {
            ...
            metadata: { invoked_by: "test", project: "bugbee" },
            cacheTtl: 300,
            cacheKey: "cache-key",
            skipCache: true,
            collectLog: false,
        ...
        expect(aiGatewayCalls[0]).toEqual({
          ...
          options: {
            ...
            collectLog: false,
            headers: {
              "User-Agent": expect.stringContaining("bugbee/"),
            },
          },
        })
      }),
      ...
      Effect.gen(function* () {
        ...
        yield* aisdk.runSDK({
          ...
          options: {
            name: "cloudflare-ai-gateway",
            headers: {
              "cf-aig-metadata": JSON.stringify({ invoked_by: "header", project: "bugbee" }),
            },
          },
        })

        expect(aiGatewayCalls[0]?.options).toMatchObject({
          metadata: { invoked_by: "header", project: "bugbee" },
        })
      }),
    ),
  )
