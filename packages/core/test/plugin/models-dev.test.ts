...
import { ModelsDevPlugin } from "@bugbee-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.BUGBEE_MODELS_PATH,
          disabled: Flag.BUGBEE_DISABLE_MODELS_FETCH,
        }
        Flag.BUGBEE_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.BUGBEE_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.BUGBEE_MODELS_PATH = previous.path
          Flag.BUGBEE_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
