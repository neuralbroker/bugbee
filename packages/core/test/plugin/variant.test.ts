...
import { VariantPlugin } from "@bugbee-ai/core/plugin/variant"
  ...
  it.effect("adds GLM 5.2 variants after catalog sources", () =>
    Effect.gen(function* () {
      ...
      yield* service.transform((catalog) => {
        catalog.provider.update(ProviderV2.ID.bugbee, (provider) => {
          provider.api = { type: "aisdk", package: "@ai-sdk/openai-compatible" }
        })
        catalog.model.update(ProviderV2.ID.bugbee, ModelV2.ID.make("glm-5.2"), (model) => {
          model.api = {
      ...
      expect((yield* service.model.get(ProviderV2.ID.bugbee, ModelV2.ID.make("glm-5.2")))?.variants).toEqual([
