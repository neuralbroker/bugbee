...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "bugbee-core-test-"))
  return {
    path: dir,
  ...
  describe("Windows-specific", () => {
    fx.effect(
      ...
      Effect.gen(function* () {
        ...
        const out = yield* ChildProcessSpawner.ChildProcessSpawner.use((svc) =>
          svc.string(
            ChildProcess.make("set", ["BUGBEE_TEST_SHELL"], {
              shell: true,
              extendEnv: true,
              env: { BUGBEE_TEST_SHELL: "ok" },
            }),
          ),
        )
        expect(out).toContain("BUGBEE_TEST_SHELL=ok")
      }),
    )
